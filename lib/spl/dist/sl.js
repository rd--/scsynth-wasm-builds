...
const context = {
    ...
    packageSymCounter: 0,
    methodBodyInitialSourceTable: new Map()
};
function deleteUnderscores(text) {
    return text.replaceAll('_', '');
}
function deleteLeadingZeroes(s) {
    return s.replace(/^0+(?!\.|$)/, '');
}
function isArityQualifiedName(s) {
    return s.includes(':/');
}
function initContext(name) {
    context.packageName = name;
    context.packageSymCounter = 0;
    context.methodBodyInitialSourceTable.clear();
}
function genPackageSym(prefix) {
...
function rewriteMethodListToCore(nList, bList) {
...
function rewriteTypeOrTraitExtensionToCore(t, n, b) {
...
const asJs = {
    ...
    MethodDefinitions (_p, _l, n, _r, _lc, mn, mb, _rc) {
        return makeMethodList('extendTypeOrTraitWithMethod', n.asIteration().children.map((c)=>c.sourceString), mn.children, mb.children);
    },
    ...
    Statements (nonFinal, __final) {
        return nonFinal.asJs + __final.asJs;
    },
    ...
    TypeDefinition (n, h, t, _l, v, mn, mb, _r) {
        return makeTypeDefinition(h.sourceString === '!', n.sourceString, t.asJs, v.asJs, mn.children, mb.children);
    },
    ValueApply (p, _d, a) {
...
const asSl = {
    ...
    ApplyWithTrailingClosuresSyntax (name, args, trailing) {
        return `${name.asSl}(${commaListSl(args.children.concat(trailing.children))})`;
    },
    Arguments (a, _p) {
    ...
    Statements (nonFinal, __final) {
        return nonFinal.asSl + __final.asSl;
    },
    ...
    floatLiteral (s, i, _dot, f) {
        return s.sourceString + deleteLeadingZeroes(deleteUnderscores(i.sourceString)) + '.' + f.sourceString;
    },
    ...
    radixIntegerLiteral (s, b, _r, i) {
        const r = Number.parseInt(s.sourceString + deleteUnderscores(i.sourceString), Number.parseInt(b.sourceString, 10));
        if (Number.isNaN(r)) {
...
const asAst = {
    ...
    Statements (nonFinal, __final) {
        return nonFinal.children.map((x)=>x.asAst).concat([
            __final.asAst
        ]);
    },
    ValueApply (p, _d, a) {
...
function procList(nodeArray, fn, sep) {
...
export { parse as parseIni, stringify as encodeIni };
...
function copyTraitMethodsToType(traitName, typeName) {
    if (!traitExists(traitName)) {
        throw new Error(`copyTraitMethodsToType: trait does not exist: ${traitName}`);
    }
    if (!typeExists(typeName)) {
        throw new Error(`copyTraitMethodsToType: type does not exist: ${typeName}`);
    }
    const trait = getTrait(traitName);
