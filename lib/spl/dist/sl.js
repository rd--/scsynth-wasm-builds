...
function evaluateForSignalling(packageName, slText) {
    if (onlyBlanks(slText)) {
        ...
        let jsText = rewriteStringFor(packageName, slText);
...
function evaluateFor(packageName, slText) {
    try {
        ...
        console.error(`evaluateFor: ${packageName}: "${slText}": ${err.message}: ${err.cause}`);
        return err;
    }
}
...
function objectType(anObject) {
    return anObject instanceof Array ? 'List' : anObject instanceof Map ? 'Map' : anObject instanceof Set ? 'Set' : anObject instanceof Promise ? 'Promise' : anObject instanceof PriorityQueue ? 'PriorityQueue' : anObject instanceof Uint8Array ? 'ByteArray' : anObject instanceof Float64Array ? 'Float64Array' : anObject instanceof Error ? 'Error' : anObject instanceof WeakMap ? 'WeakMap' : anObject._type || (isRecord(anObject) ? 'Record' : objectNameByConstructor(anObject));
}
...
function dispatchByType(name, arity, typeTable, parameterArray) {
    if (arity === 0) {
        ...
        if (typeMethod) {
            ...
            throw new Error(`dispatchByType: no method ${name}:/${arity} for ${receiverType}`);
        }
    }
}
...
function preferencesRead(preferences, key) {
    const configuration = preferences['Configuration'];
    const configurationEntry = preferences[configuration][key];
    if (configurationEntry) {
        return configurationEntry;
    } else {
        const defaultEntry = preferences['Default'][key];
        if (defaultEntry) {
            return defaultEntry;
        } else {
