...
function addType(isHostType, typeName, packageName, traitList, slotNames) {
    if (!typeExists(typeName) || preinstalledTypes.includes(typeName)) {
        ...
        const defInitializeSlots = isHostType ? '' : `addMethod('${typeName}', '${packageName}', 'initializeSlots', ${JSON.stringify([
            'self'
...
function primitiveReadLocalFile(fileName) {
...
function addLoadUrlMethods() {
    ...
    addMethod('String', 'Kernel', 'primitiveReadLocalFile', [
        'self'
    ], primitiveReadLocalFile, '<primitive: file reader>');
}
...
export { primitiveReadLocalFile as primitiveReadLocalFile };
