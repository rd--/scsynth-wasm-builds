...
const nonOperatorPunctuationCharacters = '"\'(),:;.[]_`{}';
...
function isNonOperatorPunctuationCharacter(character) {
...
function isPunctuationToken(name) {
    for (const character of name){
...
const punctuationCharacterNameTable = {
    '!': 'exclamationMark',
    '#': 'numberSign',
    '$': 'dollarSign',
    '%': 'percentSign',
...
function punctuationTokenName(operator) {
    const words = [
        ...operator
    ].map((letter)=>punctuationCharacterNameTable[letter]);
...
export { nonOperatorPunctuationCharacters as nonOperatorPunctuationCharacters };
export { punctuationCharacters as punctuationCharacters };
export { isOperatorCharacter as isOperatorCharacter };
export { isNonOperatorPunctuationCharacter as isNonOperatorPunctuationCharacter };
export { isPunctuationCharacter as isPunctuationCharacter };
export { isOperator as isOperator };
export { isPunctuationToken as isPunctuationToken };
export { punctuationCharacterNameTable as punctuationCharacterNameTable };
export { punctuationTokenName as punctuationTokenName };
...
const slOptions = {
    ...
    multipleNamesForLocalBlocks: false
};
...
function genListRange(start, end) {
...
const asJs = {
    TypeExtension (_plus, typeName, _leftBrace, methodName, methodBlock, _rightBrace) {
        ...
        ], methodName.children.map((c)=>c.sourceString), methodBlock.children);
    },
    ...
    TypeDefinition (typeName, trait, _leftBrace, instanceVariables, methodName, methodBlock, _rightBrace) {
        return makeTypeDefinition(false, typeName.sourceString, trait.asJs, instanceVariables.asJs, methodName.children.map((c)=>c.sourceString), methodBlock.children);
    },
    HostTypeDefinition (typeName, _isHostType, trait, _leftBrace, instanceVariables, methodName, methodBlock, _rightBrace) {
        return makeTypeDefinition(true, typeName.sourceString, trait.asJs, instanceVariables.asJs, methodName.children.map((c)=>c.sourceString), methodBlock.children);
    },
    ...
    TraitListExtension (_plus, _at, _leftBracket, traitNameList, _rightBracket, _leftBrace, methodName, methodBlock, _rightBrace) {
        const traitNameArray = traitNameList.asIteration().children.map((c)=>c.sourceString);
        return makeMethodList('extendTraitWithMethod', traitNameArray, methodName.children.map((c)=>c.sourceString), methodBlock.children);
    },
    ...
    TemporaryBlockLiteralInitializer (name, _equals, blk) {
        ...
        const qualifiedName = `${genName(unqualifiedName, blk.parametersOf.length)}`;
        const binding = `${qualifiedName} = ${blk.asJs}`;
        const reBinding = slOptions.multipleNamesForLocalBlocks ? `, ${unqualifiedName} = ${qualifiedName}` : '';
        return `${binding}${reBinding}`;
    },
    TemporaryExpressionInitializer (name, _equals, exp) {
        return `${name.asJs} = ${exp.asJs}`;
    },
    TemporaryDictionaryInitializer (_leftParen, lhs, _rightParen, _equals, rhs) {
    ...
    TemporaryListInitializer (_leftBracket, lhs, _rightBracket, _equals, rhs) {
        const namesArray = lhs.asIteration().children.map((c)=>c.asJs);
        const rhsName = genSym();
        const slots = namesArray.map((name, index)=>`${name} = _${genName('at', 2)}(${rhsName}, ${index + 1})`).join(', ');
    ...
    ListAssignment (_leftBracket, lhs, _rightBracket, _colonEquals, rhs) {
        ...
        const slots = namesArray.map((name, index)=>`${name} = _${genName('at', 2)}(${rhsListName}, ${index + 1})`).join(';\n');
    ...
    DictionaryAssignment (_leftParen, lhs, _rightParen, _colonEquals, rhs) {
        ...
        const slots = namesArray.map((name)=>`_${name} = _${genName('at', 2)}(${rhsDictionaryName}, '${name}')`).join(';\n');
    ...
    operatorWithUnaryAdverb (op, _dot, adverb) {
        return `_${genName(adverb.sourceString, 1)}(_${genName(op.sourceString, 2)})`;
    },
    operatorWithBinaryAdverb (op, _dot, adverb, _openParen, parameter, _closeParen) {
    ...
    ListRangeThenSyntax (_leftBracket, start, _comma_, then, _dotDot, end, _rightBracket) {
        return `_${genName('asList', 1)}(_${genName('thenTo', 3)}(${start.asJs}, ${then.asJs}, ${end.asJs}))`;
    },
    RangeSyntax (_leftParen, start, _dotDot, end, _rightParen) {
        return genRange(start, end);
    },
    RangeThenSyntax (_leftParen, start, _comma_, then, _dotDot, end, _rightParen) {
    ...
    VectorSyntaxRange (start, _dotDot_, end) {
        return `_${genName('asList', 1)}(_${genName('upTo', 2)}(${start.asJs}, ${end.asJs}))`;
    },
...
function makeMethod(slProc, typeNameArray, methodName, methodBlock) {
...
function makeMethodList(slProc, typeNameArray, methodNames, methodBlocks) {
    let methodList = '';
...
function rewriteStringFor(packageName, slText) {
    let jsText;
...
export { rewriteStringFor as rewriteStringFor };
...
function evaluateForSignalling(packageName, slText) {
    if (onlyBlanks(slText)) {
        ...
        const jsText = rewriteStringFor(packageName, slText);
...
function addTraitMethod(traitName, packageName, methodName, parameterNames, block, sourceCode) {
    if (traitExists(traitName)) {
...
function dispatchVoid(name, typeTable) {
...
function dispatchByType(name, typeTable, parameterArray) {
...
function addMethod(typeName, packageName, methodName, parameterNames, block, sourceCode) {
...
function addType(isHostType, typeName, packageName, traitList, slotNames) {
    if (!typeExists(typeName) || preinstalledTypes.includes(typeName)) {
        ...
        const defNewType = isHostType ? '' : `sl.addMethod('Void', '${packageName}', 'new${typeName}', [], function() { return {_type: '${typeName}', ${nilSlots} }; }, '<primitive: constructor>')`;
        const defInitializeSlots = isHostType ? '' : `sl.addMethod('${typeName}', '${packageName}', 'initializeSlots', ${JSON.stringify([
            'self'
        ...
        const defPredicateTrue = `sl.addMethod('${typeName}', '${packageName}', 'is${typeName}', ['self'], function(anInstance) { return true; }, '<primitive: predicate>')`;
        const defSlotAccess = slotNames.map((each)=>`sl.addMethod('${typeName}', '${packageName}', '${each}', ['self'], function(anInstance) { return anInstance.${each} }, '<primitive: accessor>');`).join('; ');
        const defSlotMutate = slotNames.map((each)=>`sl.addMethod('${typeName}', '${packageName}', '${each}', ['self', 'anObject'], function(anInstance, anObject) { anInstance.${each} = anObject; return anObject; }, '<primitive: mutator>');`).join('; ');
...
function shiftRightUnsigned(lhs, rhs) {
    return lhs >>> rhs;
}
function signExponentMantissa(x) {
    const __float = new Float64Array(1);
    const bytes = new Uint8Array(__float.buffer);
    __float[0] = x;
...
async function primitiveLoadPackageSequence(packageNames) {
    ...
    packageNames.forEach((name)=>{
        ...
        if (pkg == undefined) {
            throw new Error(`primitiveLoadPackageSequence: no such package: ${name}, ${pkg}`);
        } else {
            pkg.isLoaded = true;
            packageArray.push(pkg);
        }
...
async function dotLayout(dotSource, outputFormat, layoutEngine) {
...
export { shiftRightUnsigned as shiftRightUnsigned };
export { signExponentMantissa as signExponentMantissa };
...
function addLoadUrlMethods() {
    addMethod('List', 'Kernel', 'primitiveReadLocalPackages', [
        'self'
    ], primitiveReadLocalPackages, '<primitive: package reader>');
    addMethod('List', 'Kernel', 'primitiveLoadPackageSequence', [
        'self'
    ], primitiveLoadPackageSequence, '<primitive: package loader>');
    addMethod('String', 'Kernel', 'primitiveReadLocalBinaryFile', [
        'self'
