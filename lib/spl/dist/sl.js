...
export { syntaxCharacters as syntaxCharacters };
export { punctuationCharacters as punctuationCharacters };
export { isOperatorCharacter as isOperatorCharacter };
export { isSyntaxCharacter as isSyntaxCharacter };
export { isPunctuationCharacter as isPunctuationCharacter };
export { isOperatorToken as isOperatorToken };
export { isSyntaxToken as isSyntaxToken };
...
function genDotTrailing(lhs, name, args, trailing, fn) {
    const numArgs = args ? args.arityOf : 0;
    const qualifiedName = `${genName(fn(name), 1 + numArgs + trailing.children.length)}`;
    return `${qualifiedName}(${commaList([
        lhs
    ].concat(args ? args.children : [], trailing.children), fn)})`;
}
function genApplyTrailing(name, args, trailing, fn) {
    const numArgs = (args ? args.arityOf : 0) + trailing.children.length;
    const qualifiedName = `${genName(fn(name), numArgs)}`;
    return `${qualifiedName}(${commaList((args ? args.children : []).concat(trailing.children), fn)})`;
}
...
const asJs = {
    ...
    TemporaryListInitializer (_leftBracket, lhs, _rightBracket, _equals, rhs) {
        ...
        const slots = namesArray.map((name, index)=>`${name} = _at_2(${rhsName}, ${index + 1})`).join(', ');
    ...
    ListAssignment (_leftBracket, lhs, _rightBracket, _colonEquals, rhs) {
        ...
        const slots = namesArray.map((name, index)=>`${name} = _at_2(${rhsListName}, ${index + 1})`).join(';\n');
    ...
    AtPutSyntax (c, _leftBracket, k, _rightBracket, _equals, v) {
        ...
        return `_${genName('atPut', 2 + elem.length)}(${c.asJs}, ${commaListJs(elem)}, ${v.asJs})`;
    },
...
const asAst = {
    ...
    Block (_l, arg, tmp, prm, stm, _r) {
        return [
            ...
            [
                arg.asAst
            ].concat(tmp.asAst, prm.asAst, stm.asAst).flat(1)
        ];
    },
    EmptyListSyntax (_l, _r) {
        return [
