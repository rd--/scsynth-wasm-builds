...
function makeTypeDefinition(isHostType, typeName, traits, instanceVariables, methodNames, methodBlocks) {
    ...
    const instanceVariablesList = instanceVariablesSource === '' ? [] : slTemporariesSyntaxNames(instanceVariablesSource).map((name)=>`'${name}'`);
...
function intervalSyntax(start, end) {
    return `_${genName('upOrDownTo', 2)}(${start.asJs}, ${end.asJs})`;
}
function arrayIntervalSyntax(start, end) {
...
const asJs = {
    ...
    TemporaryArrayInitializer (_leftBracket, lhs, _rightBracket, _equals, rhs) {
    ...
    ArrayAssignment (_leftBracket, lhs, _rightBracket, _colonEquals, rhs) {
        ...
        const slots = namesArray.map((name, index)=>`${name} = _${genName('at', 2)}(${rhsArrayName}, ${index + 1})`).join(';\n');
    ...
    VectorSyntaxRange (start, _dotDot_, end) {
        return `_${genName('asArray', 1)}(_${genName('upTo', 2)}(${start.asJs}, ${end.asJs}))`;
    },
...
function addLoadUrlMethods() {
    addMethod('Array', 'Kernel', 'primitiveReadLocalPackages', [
        'self'
    ], primitiveReadLocalPackages, '<primitive: package reader>');
    addMethod('Array', 'Kernel', 'primitiveLoadPackageSequence', [
        'self'
