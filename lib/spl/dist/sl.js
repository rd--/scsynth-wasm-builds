...
function genDotTrailing(lhs, name, args, trailing) {
    ...
    const qualifiedName = `${genName(name.asJs, 1 + numArgs + trailing.children.length)}`;
    return `${qualifiedName}(${commaList([
        lhs
    ].concat(args ? args.children : [], trailing.children))})`;
}
function genApplyTrailing(name, args, trailing) {
