...
function rewriteMethodListToCore(n, b) {
    ...
    for(let i = 0; i < k; i++){
        const bSl = bArray[i];
        const bCore = bSl.asSl;
        context.methodBodySavedText.set(bCore, bSl.sourceString);
...
function makeMethod(slProc, typeOrTraitNameArray, methodName, methodBlock) {
    ...
    const blkInitialSource = context.methodBodySavedText.get(blkCoreSource);
