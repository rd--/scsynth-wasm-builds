...
sl.addMethodToExistingType(
  ...
  "{ :self :maxDepth | let __SplVar1 = assertIsOfSize(shape(contents(self)), 2); let rowCount = at(__SplVar1, 1); let columnCount = at(__SplVar1, 2); let header = ['P2', format('% %',[columnCount, rowCount]), printString(maxDepth)]; let rows = collect(contents(self), { :each | unwords(collect(each, { :level | if(betweenAnd(level,0, 1), { printString(round((*(level, maxDepth)))) }, { error(self,'asPgm: not (0,1) matrix') }) })) }); unlines((++(++(header, rows), ['']))) }",
);
