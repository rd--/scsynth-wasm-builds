...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _maxDepth) {
    ...
    let __SPL264 = _assertIsOfSize_2(_shape_1(_contents_1(_self)), 2);
    let _rowCount = _at_2(__SPL264, 1);
    let _columnCount = _at_2(__SPL264, 2);
    let _header = [
      "P2",
  ...
  "{ :self :maxDepth | let __SPL264 = assertIsOfSize(shape(contents(self)), 2); let rowCount = at(__SPL264, 1); let columnCount = at(__SPL264, 2); let header = ['P2', format('% %',[columnCount, rowCount]), printString(maxDepth)]; let rows = collect(contents(self), { :each | unwords(collect(each, { :level | if(betweenAnd(level,0, 1), { printString(rounded((*(level, maxDepth)))) }, { error(self,'asPgm: not (0,1) matrix') }) })) }); unlines((++(++(header, rows), ['']))) }",
);
