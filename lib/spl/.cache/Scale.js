...
sl.addMethodToExistingType(
  ...
  "{ :self | let sum = startIndex(self); let answer = [sum]; let tuningSize = tuningSize(self); allButLastDo(intervals(self), { :each | sum := +((%((+(-(sum, 1), each)), tuningSize)), 1); add(answer,sum) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_generator, _period, _maxDepth) {
    ...
    let _nextPair_1 = sl.annotateFunction(function (_pair) {
      ...
      let __SPL278 = _assertIsOfSize_2(_pair, 2);
      let _i = _at_2(__SPL278, 1);
      let _j = _at_2(__SPL278, 2);
      /* Statements */
      return _if_3(
    ...
    _whileTrue_1(sl.annotateFunction(function () {
      ...
      let _k = _max_2(_i, _j);
      let __SPL279 = _assertIsOfSize_2(_nextPair_1([_i, _j]), 2);
  ...
  "{ :generator :period :maxDepth | let i = generator; let j = -(period, generator); let nextPair = { :pair | let __SPL278 = assertIsOfSize(pair, 2); let i = at(__SPL278, 1); let j = at(__SPL278, 2); if((<(i, j)), { [i, -(j, i)] }, { [-(i, j), j] }) }; let depth = 1; let answer = [[i, j]]; whileTrue({ let previousLevel = last(answer); let k = max(i,j); let __SPL279 = assertIsOfSize(nextPair([i, j]), 2); let p = at(__SPL279, 1); let q = at(__SPL279, 2); let nextLevel = copyReplaceAllWith(previousLevel,[k], [p, q]); ({ let __SPL280 = [p, q]; i := at(__SPL280, 1); j := at(__SPL280, 2) } . ()); depth := +(depth, 1); if((|(|(|(~(i, j), { ~(i, 0) }), { ~(j, 0) }), { >(depth, maxDepth) })), { false }, { add(answer,nextLevel); true }) }); answer }",
);
