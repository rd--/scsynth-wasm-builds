...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :generator :period :maxDepth | let i = generator; let j = -(period, generator); let nextPair = { :pair | let __SplVar1 = assertIsOfSize(pair, 2); let i = at(__SplVar1, 1); let j = at(__SplVar1, 2); if((<(i, j)), { [i, -(j, i)] }, { [-(i, j), j] }) }; let depth = 1; let answer = [[i, j]]; whileTrue({ let previousLevel = last(answer); let k = max(i,j); let __SplVar2 = assertIsOfSize(nextPair([i, j]), 2); let p = at(__SplVar2, 1); let q = at(__SplVar2, 2); let nextLevel = copyReplaceAllWith(previousLevel,[k], [p, q]); ({ let __SplVar3 = [p, q]; i := at(__SplVar3, 1); j := at(__SplVar3, 2) } . ()); depth := +(depth, 1); if((|(|(|(~(i, j), { ~(i, 0) }), { ~(j, 0) }), { >(depth, maxDepth) })), { false }, { add(answer,nextLevel); true }) }); answer }",
);
