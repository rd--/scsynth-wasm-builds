...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_generator, _period, _maxDepth) {
    ...
    let _nextPair_1 = sl.annotateFunction(function (_pair) {
      ...
      let __SPL482 = _assertIsOfSize_2(_pair, 2);
      let _i = _at_2(__SPL482, 1);
      let _j = _at_2(__SPL482, 2);
      /* Statements */
      return _if_3(
    ...
    _whileTrue_1(sl.annotateFunction(function () {
      ...
      let _k = _max_2(_i, _j);
      let __SPL483 = _assertIsOfSize_2(_nextPair_1([_i, _j]), 2);
      let _p = _at_2(__SPL483, 1);
      let _q = _at_2(__SPL483, 2);
  ...
  "{ :generator :period :maxDepth | let i = generator; let j = -(period, generator); let nextPair = { :pair | let __SPL482 = assertIsOfSize(pair, 2); let i = at(__SPL482, 1); let j = at(__SPL482, 2); if((<(i, j)), { [i, -(j, i)] }, { [-(i, j), j] }) }; let depth = 1; let answer = [[i, j]]; whileTrue({ let previousLevel = last(answer); let k = max(i,j); let __SPL483 = assertIsOfSize(nextPair([i, j]), 2); let p = at(__SPL483, 1); let q = at(__SPL483, 2); let nextLevel = copyReplaceAllWith(previousLevel,[k], [p, q]); ({ let __SPL484 = [p, q]; i := at(__SPL484, 1); j := at(__SPL484, 2) } . ()); depth := +(depth, 1); if((|(|(|(~(i, j), { ~(i, 0) }), { ~(j, 0) }), { >(depth, maxDepth) })), { false }, { add(answer,nextLevel); true }) }); answer }",
);
