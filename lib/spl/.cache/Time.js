...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let _m = _at_2(__SPL6, 2);
        let _s = _at_2(__SPL6, 3);
        /* Statements */
        return _format_2("%:%:%.%", [
  ...
  "{ :self | let seconds = seconds(self); if((&(>=(seconds, 0), { <(seconds, (*(*(24, 60), 60))) })), { let __SPL6 = assertIsOfSize(mixedRadixEncode(seconds,[60, 60]), 3); let h = at(__SPL6, 1); let m = at(__SPL6, 2); let s = at(__SPL6, 3); format('%:%:%.%',[padLeft(asString(h),[2], '0'), padLeft(asString(m),[2], '0'), padLeft(asString(truncated(s)),[2], '0'), padLeft(asString(rounded((*(fractionPart(s), 1000)))),[3], '0')]) }, { error(self,'timeString: invalid time') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _elseClause_0) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL7 = _assertIsOfSize_2(_splitBy_2(_self, ":"), 3);
        let _h = _at_2(__SPL7, 1);
        let _m = _at_2(__SPL7, 2);
        let _s = _at_2(__SPL7, 3);
        /* Statements */
  ...
  "{ :self :elseClause:/0 | if(isTimeString(self), { let __SPL7 = assertIsOfSize(splitBy(self,':'), 3); let h = at(__SPL7, 1); let m = at(__SPL7, 2); let s = at(__SPL7, 3); h := parseDecimalInteger(h); m := parseDecimalInteger(m); s := parseNumber(s); if((|(<(h, 24), { |(<(m, 60), { <(s, 60) }) })), { Time(+(+((*(*(h, 60), 60)), (*(m, 60))), s)) }, { elseClause() }) }, { elseClause() }) }",
);
