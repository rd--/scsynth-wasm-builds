...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL434 = _assertIsOfSize_2(
          _mixedRadixEncode_2(_seconds, [60, 60]),
          3,
        );
        let _h = _at_2(__SPL434, 1);
        let _m = _at_2(__SPL434, 2);
        let _s = _at_2(__SPL434, 3);
        /* Statements */
        return _format_2("%:%:%.%", [
          _padLeft_3(_asString_1(_h), [2], "0"),
          _padLeft_3(_asString_1(_m), [2], "0"),
          _padLeft_3(_asString_1(_truncated_1(_s)), [2], "0"),
          _padLeft_3(
            _asString_1(_rounded_1(_asterisk_2(_fractionPart_1(_s), 1000))),
            [3],
            "0",
          ),
        ]);
  ...
  "{ :self | let seconds = seconds(self); if((&(>=(seconds, 0), { <(seconds, (*(*(24, 60), 60))) })), { let __SPL434 = assertIsOfSize(mixedRadixEncode(seconds,[60, 60]), 3); let h = at(__SPL434, 1); let m = at(__SPL434, 2); let s = at(__SPL434, 3); format('%:%:%.%',[padLeft(asString(h),[2], '0'), padLeft(asString(m),[2], '0'), padLeft(asString(truncated(s)),[2], '0'), padLeft(asString(rounded((*(fractionPart(s), 1000)))),[3], '0')]) }, { error(self,'timeString: invalid time') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _elseClause_0) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL435 = _assertIsOfSize_2(_splitBy_2(_self, ":"), 3);
        let _h = _at_2(__SPL435, 1);
        let _m = _at_2(__SPL435, 2);
        let _s = _at_2(__SPL435, 3);
        /* Statements */
  ...
  "{ :self :elseClause:/0 | if(isTimeString(self), { let __SPL435 = assertIsOfSize(splitBy(self,':'), 3); let h = at(__SPL435, 1); let m = at(__SPL435, 2); let s = at(__SPL435, 3); h := parseDecimalInteger(h); m := parseDecimalInteger(m); s := parseNumber(s); if((|(<(h, 24), { |(<(m, 60), { <(s, 60) }) })), { Time(+(+((*(*(h, 60), 60)), (*(m, 60))), s)) }, { elseClause() }) }, { elseClause() }) }",
);
