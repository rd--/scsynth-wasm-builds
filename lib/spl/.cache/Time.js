...
sl.addMethodToExistingType(
  ...
  "{ :self | let seconds = seconds(self); if((&(>=(seconds, 0), { <(seconds, (*(*(24, 60), 60))) })), { let __SPL1 = assertIsOfSize(mixedRadixEncode(seconds,[60, 60]), 3); let h = at(__SPL1, 1); let m = at(__SPL1, 2); let s = at(__SPL1, 3); format('%:%:%.%',[padLeft(asString(h),[2], '0'), padLeft(asString(m),[2], '0'), padLeft(asString(truncate(s)),[2], '0'), padLeft(asString(round((*(fractionalPart(s), 1000)))),[3], '0')]) }, { error(self,'timeString: invalid time') }) }",
);
