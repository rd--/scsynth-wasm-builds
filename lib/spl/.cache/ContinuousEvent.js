...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :part :voice :voiceBlock:/1 | collect(Span(1, voice, 1), { :each | let bus = voicerVoiceAddress(part,each); voiceBlock(asContinuousEvent(ControlIn(8, bus))) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :part :numVoices :voiceBlock:/0 | collect(Span(1, numVoices, 1), { :voice | ControlOut(voicerVoiceAddress(part,voice), asList(asContinuousEvent(voiceBlock()))) }) }",
);
