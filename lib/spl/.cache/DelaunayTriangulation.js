...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(shape(self), 2); let m = at(__SplVar1, 1); let n = at(__SplVar1, 2); if((=(n, 2)), { let coordinateVector = Float64Array(*(size(self), 2)); let index = 1; do(self, { :each | let __SplVar2 = assertIsOfSize(each, 2); let x = at(__SplVar2, 1); let y = at(__SplVar2, 2); atPut(coordinateVector, index, x); atPut(coordinateVector, +(index, 1), y); index := +(index, 2) }); initializeSlots(newDelaunayTriangulation(),self, basicDelaunayTriangulation(coordinateVector)) }, { error(self,'DelaunayTriangulation: not two column matrix') }) }",
);
