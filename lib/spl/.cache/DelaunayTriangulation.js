...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    let __SPL73 = _assertIsOfSize_2(_shape_1(_self), 2);
    let _m = _at_2(__SPL73, 1);
    let _n = _at_2(__SPL73, 2);
    /* Statements */
    return _if_3(_equalsSign_2(_n, 2), function () {
      ...
      _do_2(_self, function (_each) {
        ...
        let _x = _at_2(__SPL74, 1);
        let _y = _at_2(__SPL74, 2);
        /* Statements */
  ...
  "{ :self | let __SPL73 = assertIsOfSize(shape(self), 2); let m = at(__SPL73, 1); let n = at(__SPL73, 2); if((=(n, 2)), { let coordinateVector = Float64Array(*(size(self), 2)); let index = 1; do(self, { :each | let __SPL74 = assertIsOfSize(each, 2); let x = at(__SPL74, 1); let y = at(__SPL74, 2); atPut(coordinateVector, index, x); atPut(coordinateVector, +(index, 1), y); index := +(index, 2) }); initializeSlots(newDelaunayTriangulation(),self, basicDelaunayTriangulation(coordinateVector)) }, { error(self,'DelaunayTriangulation: not two column matrix') }) }",
);
