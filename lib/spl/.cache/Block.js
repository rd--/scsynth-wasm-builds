...
sl.addMethodToExistingType(
  ...
  "clip",
  ["aBlock:/1", "min", "max"],
  sl.annotateFunction(function (_aBlock_1, _min, _max) {
    /* ArityCheck */
    if (arguments.length !== 3) {
      const errorMessage = "Arity: expected 3, _aBlock_1, _min, _max";
      throw new Error(errorMessage);
    } /* Statements */
    return sl.annotateFunction(function (_x) {
      ...
      return _clip_5(_aBlock_1(_x), _min, _max, _min, _max);
    }, ["x"]);
  }, ["aBlock:/1", "min", "max"]),
  "{ :aBlock:/1 :min :max | { :x | clip(aBlock(x),min, max, min, max) } }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _errorHandlerBlock_1) {
    /* ArityCheck */
    if (arguments.length !== 2) {
...
sl.addMethodToExistingType(
  ...
  "{ :self:/1 :requireImmediate | let table = if(requireImmediate, { Map() }, { EqualityDictionary() }); { :input | atIfAbsent(table, input, { let answer = self(input); atPut(table,input, answer); answer }) } }",
);
