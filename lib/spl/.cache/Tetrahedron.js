...
sl.addMethodToExistingType(
  ...
  "{ :self | let v = vertexCoordinates(self); let a = determinant(arrayPad(v,[[0, 0], [0, 1]], 1)); let vSquareSum = collect(square(v), { :each | enclose(sum(each)) }); let c = join([vSquareSum, v],2); let d = arrayPad(c,[[0, 0], [0, 1]], 1); let f = { :m :i | *(determinant(submatrix(d,asList(upOrDownTo(1, 4)), without(asList(upOrDownTo(1, 5)),+(i, 1)))), m) }; let dx = f(1, 1); let dy = f(-1, 2); let dz = f(1, 3); let r = /(sqrt((-(+(+(square(dx), square(dy)), square(dz)), (*(*(4, a), determinant(c)))))), (*(2, abs(a)))); Sphere(/([dx, dy, dz], (*(a, 2))), r) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL276 = _assertIsOfSize_2(_vertexCoordinates_1(_self), 4);
    let _a = _at_2(__SPL276, 1);
    let _b = _at_2(__SPL276, 2);
    let _c = _at_2(__SPL276, 3);
    let _d = _at_2(__SPL276, 4);
    /* Statements */
    return _toBarycentricCoordinates_4(_a, _b, _c, _d);
  }, ["self"]),
  "{ :self | let __SPL276 = assertIsOfSize(vertexCoordinates(self), 4); let a = at(__SPL276, 1); let b = at(__SPL276, 2); let c = at(__SPL276, 3); let d = at(__SPL276, 4); toBarycentricCoordinates(a, b, c, d) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL277 = _assertIsOfSize_2(_shape_1(_self), 2);
    let _m = _at_2(__SPL277, 1);
    let _n = _at_2(__SPL277, 2);
    /* Statements */
    return _if_3(
      _ampersand_2(
        _equalsSign_2(_m, 4),
  ...
  "{ :self | let __SPL277 = assertIsOfSize(shape(self), 2); let m = at(__SPL277, 1); let n = at(__SPL277, 2); if((&(=(m, 4), { =(n, 3) })), { initializeSlots(newTetrahedron(),self) }, { error(self,'Tetrahedron: invalid matrix') }) }",
);
