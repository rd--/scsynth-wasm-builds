...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _parameter) {
    ...
    let _elapsedTime = _at_2(__SPL2, 1);
    let _answer = _at_2(__SPL2, 2);
    /* Statements */
    return _postLine_1(
      _stringJoin_1([
        _name_1(_self),
  ...
  "{ :self :parameter | let __SPL2 = assertIsOfSize(timing({ value(benchmark(self),parameter) }), 2); let elapsedTime = at(__SPL2, 1); let answer = at(__SPL2, 2); postLine(stringJoin([name(self), ': Parameter = ', parameter, ', Verified = ', =(answer, at(verification(self), parameter)), ', Elapsed Time = ', elapsedTime])) }",
);
