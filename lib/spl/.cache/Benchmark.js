...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _parameter) {
    ...
    let _elapsedTime = _at_2(__SPL1, 1);
    let _answer = _at_2(__SPL1, 2);
    /* Statements */
    return _postLine_1(
      _stringJoin_1([
        _name_1(_self),
  ...
  "{ :self :parameter | let __SPL1 = assertIsOfSize(timing({ value(benchmark(self),parameter) }), 2); let elapsedTime = at(__SPL1, 1); let answer = at(__SPL1, 2); postLine(stringJoin([name(self), ': Parameter = ', parameter, ', Verified = ', =(answer, at(verification(self), parameter)), ', Elapsed Time = ', elapsedTime])) }",
);
