...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _aTimeSeries, _resolveConflict_2) {
    ...
    _do_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL17 = _assertIsOfSize_2(_each, 2);
        let _t1 = _at_2(__SPL17, 1);
        let _v1 = _at_2(__SPL17, 2);
        /* Statements */
        _whileTrue_2(
          sl.annotateFunction(function () {
            /* ArityCheck */
  ...
  "{ :self :aTimeSeries :resolveConflict:/2 | let lhs = path(self); let rhs = path(aTimeSeries); let i2 = 1; let t2 = at(at(rhs, 1), 1); let k2 = size(rhs); let nextRhs = { let x = at(at(rhs, i2), 2); i2 := +(i2, 1); ifTrue((<=(i2, k2)), { t2 := at(at(rhs, i2), 1) }); x }; let answer = []; do(lhs, { :each | let __SPL17 = assertIsOfSize(each, 2); let t1 = at(__SPL17, 1); let v1 = at(__SPL17, 2); whileTrue({ &(<(t2, t1), { <=(i2, k2) }) }, { add(answer,[t2, nextRhs()]) }); if((=(t1, t2)), { let r = resolveConflict(v1, nextRhs()); add(answer,[t1, r]) }, { add(answer,[t1, v1]) }) }); toDo(i2, size(rhs), { :i | add(answer,at(rhs, i)) }); TimeSeries(answer) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL19 = _assertIsOfSize_2(_transpose_1(_self), 2);
        let _t = _at_2(__SPL19, 1);
        let _v = _at_2(__SPL19, 2);
        /* Statements */
  ...
  "{ :self | let __SPL18 = assertIsOfSize(shape(self), 2); let m = at(__SPL18, 1); let n = at(__SPL18, 2); if((=(n, 2)), { let __SPL19 = assertIsOfSize(transpose(self), 2); let t = at(__SPL19, 1); let v = at(__SPL19, 2); TimeSeries(v, t) }, { error(self,'TimeSeries: not two-column matrix') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { true }, { if(isMatrix(self), { let __SPL20 = assertIsOfSize(shape(self), 2); let m = at(__SPL20, 1); let n = at(__SPL20, 2); if((=(n, 2)), { if((<=(m, 2)), { true }, { let z = -(at(at(self, 2), 1), at(at(self, 1), 1)); allSatisfy(upOrDownTo(3, m), { :i | =(-(at(at(self, i), 1), at(at(self, -(i, 1)), 1)), z) }) }) }, { error(self,'isRegularlySampled: not two column matrix') }) }, { error(self,'isRegularlySampled: not vector or two-column matrix') }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { 1 }, { if(isMatrix(self), { let __SPL21 = assertIsOfSize(shape(self), 2); let m = at(__SPL21, 1); let n = at(__SPL21, 2); if((=(n, 2)), { minimumDifferenceBy(self,first:/1) }, { error(self,'minimumTimeIncrement: not two-column matrix') }) }, { error(self,'minimumTimeIncrement: not vector or two-column matrix') }) }) }",
);
