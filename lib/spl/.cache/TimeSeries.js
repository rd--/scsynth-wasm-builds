...
sl.addMethodToExistingType(
  ...
  "{ :self :aTimeSeries :resolveConflict:/2 | let lhs = path(self); let rhs = path(aTimeSeries); let i2 = 1; let t2 = at(at(rhs, 1), 1); let k2 = size(rhs); let nextRhs = { let x = at(at(rhs, i2), 2); i2 := +(i2, 1); ifTrue((<=(i2, k2)), { t2 := at(at(rhs, i2), 1) }); x }; let answer = []; do(lhs, { :each | let __SplVar1 = assertIsOfSize(each, 2); let t1 = at(__SplVar1, 1); let v1 = at(__SplVar1, 2); whileTrue({ &(<(t2, t1), { <=(i2, k2) }) }, { add(answer,[t2, nextRhs()]) }); if((=(t1, t2)), { let r = resolveConflict(v1, nextRhs()); add(answer,[t1, r]) }, { add(answer,[t1, v1]) }) }); toDo(i2, size(rhs), { :i | add(answer,at(rhs, i)) }); TimeSeries(answer) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(shape(self), 2); let m = at(__SplVar2, 1); let n = at(__SplVar2, 2); if((=(n, 2)), { let __SplVar3 = assertIsOfSize(transpose(self), 2); let t = at(__SplVar3, 1); let v = at(__SplVar3, 2); TimeSeries(v, t) }, { error(self,'TimeSeries: not two-column matrix') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { true }, { if(isMatrix(self), { let __SplVar4 = assertIsOfSize(shape(self), 2); let m = at(__SplVar4, 1); let n = at(__SplVar4, 2); if((=(n, 2)), { if((<=(m, 2)), { true }, { let z = -(at(at(self, 2), 1), at(at(self, 1), 1)); allSatisfy(upOrDownTo(3, m), { :i | =(-(at(at(self, i), 1), at(at(self, -(i, 1)), 1)), z) }) }) }, { error(self,'isRegularlySampled: not two column matrix') }) }, { error(self,'isRegularlySampled: not vector or two-column matrix') }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { 1 }, { if(isMatrix(self), { let __SplVar5 = assertIsOfSize(shape(self), 2); let m = at(__SplVar5, 1); let n = at(__SplVar5, 2); if((=(n, 2)), { minimumDifferenceBy(self,first:/1) }, { error(self,'minimumTimeIncrement: not two-column matrix') }) }, { error(self,'minimumTimeIncrement: not vector or two-column matrix') }) }) }",
);
