...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    return (function (_aVector) {
      ...
      let __SPL50 = _assertIsOfSize_2(_dot_2(_matrix, _aVector), 3);
      let _x = _at_2(__SPL50, 1);
      let _y = _at_2(__SPL50, 2);
      let __genSym52 = _at_2(__SPL50, 3);
      /* Statements */
      return [_x, _y];
    });
  },
  "{ :self | let matrix = asTransformationMatrix(self); { :aVector | let __SPL50 = assertIsOfSize(dot(matrix,aVector), 3); let x = at(__SPL50, 1); let y = at(__SPL50, 2); let _ = at(__SPL50, 3); [x, y] } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    let __SPL51 = _assertIsOfSize_2(_self, 3);
    let _x = _at_2(__SPL51, 1);
    let _y = _at_2(__SPL51, 2);
    let _z = _at_2(__SPL51, 3);
    /* Statements */
    return _initializeSlots_7(
      _newAxonometricProjection_0(),
      _at_2(_x, 1),
  ...
  "{ :self | let __SPL51 = assertIsOfSize(self, 3); let x = at(__SPL51, 1); let y = at(__SPL51, 2); let z = at(__SPL51, 3); initializeSlots(newAxonometricProjection(),at(x, 1), at(x, 2), at(y, 1), at(y, 2), at(z, 1), at(z, 2)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :alpha | caseOfOtherwise(self, [->({ 'CabinetOblique' }, { AxonometricProjection(alpha, 0, 0, /(1, 2), 1, 1) })], { error(self,'namedAxonometricProjection: alpha') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | caseOfOtherwise(self, [->({ 'CavalierOblique' }, { AxonometricProjection(pi(Fraction(1L, 4L)), 0, 0, 1, 1, 1) }), ->({ 'Isometric' }, { AxonometricProjection(pi(Fraction(1L, 6L)), 0, pi(Fraction(1L, 6L)), 1, 1, 1) }), ->({ 'Planometric' }, { AxonometricProjection(pi(Fraction(1L, 5L)), 0, pi(Fraction(1L, 4L)), 1, 1, 1) })], { error(self,'namedAxonometricProjection') }) }",
);
