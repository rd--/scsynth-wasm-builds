...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return sl.annotateFunction(function (_aVector) {
      ...
      let __SPL94 = _assertIsOfSize_2(_dot_2(_matrix, _aVector), 3);
      let _x = _at_2(__SPL94, 1);
      let _y = _at_2(__SPL94, 2);
      let __genSym96 = _at_2(__SPL94, 3);
      /* Statements */
      return [_x, _y];
    }, ["aVector"]);
  }, ["self"]),
  "{ :self | let matrix = asTransformationMatrix(self); { :aVector | let __SPL94 = assertIsOfSize(dot(matrix,aVector), 3); let x = at(__SPL94, 1); let y = at(__SPL94, 2); let _ = at(__SPL94, 3); [x, y] } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL95 = assertIsOfSize(self, 3); let x = at(__SPL95, 1); let y = at(__SPL95, 2); let z = at(__SPL95, 3); initializeSlots(newAxonometricProjection(),at(x, 1), at(x, 2), at(y, 1), at(y, 2), at(z, 1), at(z, 2)) }",
);
