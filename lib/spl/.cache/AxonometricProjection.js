...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return sl.annotateFunction(function (_aVector) {
      ...
      let __SPL91 = _assertIsOfSize_2(_dot_2(_matrix, _aVector), 3);
      let _x = _at_2(__SPL91, 1);
      let _y = _at_2(__SPL91, 2);
      let __genSym93 = _at_2(__SPL91, 3);
      /* Statements */
      return [_x, _y];
    }, ["aVector"]);
  }, ["self"]),
  "{ :self | let matrix = asTransformationMatrix(self); { :aVector | let __SPL91 = assertIsOfSize(dot(matrix,aVector), 3); let x = at(__SPL91, 1); let y = at(__SPL91, 2); let _ = at(__SPL91, 3); [x, y] } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL92 = _assertIsOfSize_2(_self, 3);
    let _x = _at_2(__SPL92, 1);
    let _y = _at_2(__SPL92, 2);
    let _z = _at_2(__SPL92, 3);
    /* Statements */
    return _initializeSlots_7(
      _newAxonometricProjection_0(),
      _at_2(_x, 1),
  ...
  "{ :self | let __SPL92 = assertIsOfSize(self, 3); let x = at(__SPL92, 1); let y = at(__SPL92, 2); let z = at(__SPL92, 3); initializeSlots(newAxonometricProjection(),at(x, 1), at(x, 2), at(y, 1), at(y, 2), at(z, 1), at(z, 2)) }",
);
