...
sl.addMethodToExistingType(
  ...
  "{ :self | let matrix = asTransformationMatrix(self); { :aVector | let __SplVar1 = assertIsOfSize(dot(matrix,aVector), 3); let x = at(__SplVar1, 1); let y = at(__SplVar1, 2); let _ = at(__SplVar1, 3); [x, y] } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(self, 3); let x = at(__SplVar2, 1); let y = at(__SplVar2, 2); let z = at(__SplVar2, 3); initializeSlots(newAxonometricProjection(),at(x, 1), at(x, 2), at(y, 1), at(y, 2), at(z, 1), at(z, 2)) }",
);
