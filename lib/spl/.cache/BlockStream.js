...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self:/1 :anObject :aBlock:/2 | let previous = nil; nextUntil(iterate(self:/1,anObject), { :each | let continue = &(isNotNil(previous), { aBlock(previous, each) }); previous := each; continue }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :lhs :rhs | adaptToStreamAndApply(rhs,lhs, round:/2) }",
);
