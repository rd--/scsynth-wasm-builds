...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | caseOf(size(self), [->(2, { let __SplVar1 = assertIsOfSize(self, 2); let x = at(__SplVar1, 'x'); let y = at(__SplVar1, 'y'); CartesianCoordinates([x, y]) }), ->(3, { let __SplVar2 = assertIsOfSize(self, 3); let x = at(__SplVar2, 'x'); let y = at(__SplVar2, 'y'); let z = at(__SplVar2, 'z'); CartesianCoordinates([x, y, z]) }), ->(4, { let __SplVar3 = assertIsOfSize(self, 4); let x = at(__SplVar3, 'x'); let y = at(__SplVar3, 'y'); let z = at(__SplVar3, 'z'); let w = at(__SplVar3, 'w'); CartesianCoordinates([x, y, z, w]) })], { error(self,'asCartesianCoordinates: not x,y or x,y,z or x,y,z,w') }) }",
);
