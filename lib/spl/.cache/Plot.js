...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL12 = assertIsOfSize(self, 2); let b = at(__SPL12, 1); let y = at(__SPL12, 2); let x = adjacentPairsCollect(b, { :i :j | +(i, (/((-(j, i)), 2))) }); discretePlot(transposed([x, y])) }",
);
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __genSym29 = _at_2(__SPL23, 3);
        let _p = _collect_2(
          _Span_3(1, _m, 1),
          sl.annotateFunction(function (_i) {
            /* ArityCheck */
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :aBlock:/1 | let k = 100; let __SPL25 = assertIsOfSize(log(minMax(self)), 2); let a = at(__SPL25, 1); let b = at(__SPL25, 2); let h = /((-(b, a)), (-(k, 1))); let x = collect(upOrDownTo(0, -(k, 1)), { :i | exp((+(a, (*(i, h))))) }); let y = collect(x, { :each | aBlock(each) }); linePlot(y) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _xInterval, _yInterval) {
    ...
    let __SPL26 = _assertIsOfSize_2(_minMax_1(_xInterval), 2);
    let _x1 = _at_2(__SPL26, 1);
    let _x2 = _at_2(__SPL26, 2);
    let __SPL27 = _assertIsOfSize_2(_minMax_1(_yInterval), 2);
    let _y1 = _at_2(__SPL27, 1);
    let _y2 = _at_2(__SPL27, 2);
    let _n = 16;
  ...
  "{ :self :xInterval :yInterval | let __SPL26 = assertIsOfSize(minMax(xInterval), 2); let x1 = at(__SPL26, 1); let x2 = at(__SPL26, 2); let __SPL27 = assertIsOfSize(minMax(yInterval), 2); let y1 = at(__SPL27, 1); let y2 = at(__SPL27, 2); let n = 16; let xStep = /((-(x2, x1)), n); let yStep = /((-(y2, y1)), n); let minStep = min(xStep,yStep); let coordinateList = flatten(table({ :i :j | let x = +((+(x1, (*(i, xStep)))), (*((%(j, 2)), (/(xStep, 2))))); let y = +(y1, (*(j, yStep))); [x, y] },asList(upOrDownTo(0, -(n, 1))), asList(upOrDownTo(0, -(n, 1)))),1); vectorPlot(coordinateList,self, *(minStep, 0.35)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _aBlock_2, _scalar) {
    ...
    return _LineDrawing_1(_collect_2(
      ...
      sl.annotateFunction(function (_xy) {
        ...
        let __SPL28 = _assertIsOfSize_2(_xy, 2);
        let _x = _at_2(__SPL28, 1);
        let _y = _at_2(__SPL28, 2);
  ...
  "{ :self :aBlock:/2 :scalar | LineDrawing(collect(self, { :xy | let __SPL28 = assertIsOfSize(xy, 2); let x = at(__SPL28, 1); let y = at(__SPL28, 2); let uv = *(normalize(aBlock(x, y)), scalar); [Point(+(xy, uv)), Line([-(xy, uv), +(xy, uv)])] })) }",
);
