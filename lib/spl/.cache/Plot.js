...
sl.addMethodToExistingType(
        ...
        _hyphenMinusGreaterThanSign_2(function () {
          ...
          return (function (_p) {
            ...
            return _collect_2(_p, function (_each) {
              ...
              let __SPL155 = _assertIsOfSize_2(_each, 2);
              let _x = _at_2(__SPL155, 1);
              let _y = _at_2(__SPL155, 2);
              /* Statements */
        ...
        return _if_3(_equalsSign_2(_format_1(_self), "line"), function () {
          ...
          let _t_1 = function (_list) {
            ...
            return _collect_2(_list, function (_each) {
              ...
              let __SPL156 = _assertIsOfSize_2(_each, 3);
              let _x = _at_2(__SPL156, 1);
              let _y = _at_2(__SPL156, 2);
              let _z = _at_2(__SPL156, 3);
              /* Statements */
            ...
            _Plot_2(
              _collect_2(_pages_1(_self), function (_each) {
                ...
                return _withIndexCollect_2(_each, function (_item, _x) {
                  ...
                  let __SPL157 = _assertIsOfSize_2(_item, 1);
                  let _y = _at_2(__SPL157, 1);
                  /* Statements */
                  return [_x, _y];
                });
              }),
  ...
  "{ :self | let columnCount = columnCount(self); if((=(columnCount, 2)), { let r = asRectangle(coordinateBoundingBox(concatenation(pages(self)))); let w = width(r); let h = height(r); let dataRatio = (/(w, h)); let aspectRatio = if((<(abs((-(dataRatio, 1))), 0.075)), { dataRatio }, { goldenRatio(1) }); let xScalar = /(aspectRatio, (/(w, h))); let scaledPages = collect(pages(self), { :each | *(each, [[xScalar, 1]]) }); let items = []; let gen:/1 = caseOf(format(self),[->({ 'line' }, { { :p | [Line(p)] } }), ->({ 'scatter' }, { { :p | [PointCloud(p)] } }), ->({ 'discrete' }, { { :p | collect(p, { :each | let __SPL155 = assertIsOfSize(each, 2); let x = at(__SPL155, 1); let y = at(__SPL155, 2); Line([[x, 0], [x, y]]) }) } })]); let includesXAxis = &(<=(lower(r), 0), { >=(upper(r), 0) }); let includesYAxis = &(<=(left(r), 0), { >=(right(r), 0) }); ifTrue(includesXAxis, { add(items,Point([*(left(r), xScalar), 0])) }); ifTrue(includesYAxis, { add(items,Point([0, upper(r)])) }); do(scaledPages, { :each | addAll(items,gen(each)) }); LineDrawing(items) }, { if((=(columnCount, 3)), { if((=(format(self), 'line')), { let p:/1 = asUnaryBlock(AxonometricProjection(pi(Fraction(1L, 6L)), 0, 0, 0.5, 1, 1)); let r = [[-1, -1, 0], [+1, -1, 0], [+1, +1, 0], [-1, +1, 0]]; let t = { :list | collect(list, { :each | let __SPL156 = assertIsOfSize(each, 3); let x = at(__SPL156, 1); let y = at(__SPL156, 2); let z = at(__SPL156, 3); p([negated(x), z, negated(y)]) }) }; let l = collect(pages(self), { :each | Line(t(each)) }); LineDrawing([l]) }, { error(self,'n×3 matrix: format must be line') }) }, { if((=(columnCount, 1)), { asLineDrawing(Plot(collect(pages(self), { :each | withIndexCollect(each, { :item :x | let __SPL157 = assertIsOfSize(item, 1); let y = at(__SPL157, 1); [x, y] }) }), format(self))) }, { error(self,++('Multiple plots not implemented: ', columnCount)) }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    let __SPL158 = _assertIsOfSize_2(_pages_1(_self), 1);
    let _graph = _at_2(__SPL158, 1);
    /* Statements */
    return _draw_1(_dotDrawing_2(_graph, _options_1(_self)));
  },
  "{ :self | let __SPL158 = assertIsOfSize(pages(self), 1); let graph = at(__SPL158, 1); draw(dotDrawing(graph,options(self))) }",
);
...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    let __SPL159 = _assertIsOfSize_2(_pages_1(_self), 1);
    let _contents = _at_2(__SPL159, 1);
      ...
      function () {
        ...
        return _if_3(_equalsSign_2(_d, 1), function () {
          ...
          return _if_3(_equalsSign_2(_d, 2), function () {
            ...
            let __SPL160 = _assertIsOfSize_2(_shape, 2);
            let _m = _at_2(__SPL160, 1);
            let _n = _at_2(__SPL160, 2);
            /* Statements */
  ...
  "{ :self | let __SPL159 = assertIsOfSize(pages(self), 1); let contents = at(__SPL159, 1); let shape = shape(contents); let d = size(shape); let a = 'x'; let c = [0]; let plotData = if(includes(['array', 'matrix'],format(self)), { a := 'matrix'; c := []; reversed(contents) }, { if((=(d, 1)), { transposed([contents]) }, { if((=(d, 2)), { let __SPL160 = assertIsOfSize(shape, 2); let m = at(__SPL160, 1); let n = at(__SPL160, 2); if((=(n, 1)), { contents }, { if((=(n, 2)), { a := 'xy'; c := [0, 1]; contents }, { if((=(n, 3)), { a := 'xyz'; c := [0, 1, 2]; contents }, { error(contents,'cliDraw: matrix columns > 3') }) }) }) }, { error(contents,'cliPlot: array dimensions > 2') }) }) }); let fileName = '/tmp/listPlot.json'; writeTextFile(fileName,asJson(plotData)); systemCommand(system,'hsc3-plot', ++(['json', a, ++('--format=', format(self)), fileName], collect(c,asString:/1))) }",
);
...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    let _counts = _nub_1(_collect_2(_pages_1(_self), function (_each) {
      ...
      let __SPL161 = _assertIsOfSize_2(_shape_1(_each), 2);
  ...
  "{ :self | let counts = nub(collect(pages(self), { :each | let __SPL161 = assertIsOfSize(shape(each), 2); let rowCount = at(__SPL161, 1); let columnCount = at(__SPL161, 2); columnCount })); if((=(size(counts), 1)), { anyOne(counts) }, { error(self,'columnCount: pages have differing column counts') }) }",
);
...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    return _caseOfOtherwise_3(_format_1(_self), [
      _hyphenMinusGreaterThanSign_2("array", function () {
        ...
        let __SPL162 = _assertIsOfSize_2(_pages_1(_self), 1);
        let _contents = _at_2(__SPL162, 1);
        /* Statements */
      ...
      _hyphenMinusGreaterThanSign_2("graph", function () {
        ...
        let __SPL163 = _assertIsOfSize_2(_pages_1(_self), 1);
        let _graph = _at_2(__SPL163, 1);
        /* Statements */
        return _draw_1(_dotDrawing_2(_graph, _options_1(_self)));
      }),
      _hyphenMinusGreaterThanSign_2("matrix", function () {
        ...
        let __SPL164 = _assertIsOfSize_2(_pages_1(_self), 1);
        let _contents = _at_2(__SPL164, 1);
        /* Statements */
  ...
  "{ :self | caseOfOtherwise(format(self), [->('array', { let __SPL162 = assertIsOfSize(pages(self), 1); let contents = at(__SPL162, 1); draw(asColourSvg(contents)) }), ->('graph', { let __SPL163 = assertIsOfSize(pages(self), 1); let graph = at(__SPL163, 1); draw(dotDrawing(graph,options(self))) }), ->('matrix', { let __SPL164 = assertIsOfSize(pages(self), 1); let contents = at(__SPL164, 1); draw(asGreyscaleSvg(contents)) })], { draw(asLineDrawing(self)) }) }",
);
...
sl.addMethodToExistingType(
  ...
  function (_self, _fileName) {
    ...
    return _caseOfOtherwise_3(_format_1(_self), [
      _hyphenMinusGreaterThanSign_2("array", function () {
        ...
        let __SPL165 = _assertIsOfSize_2(_pages_1(_self), 1);
        let _contents = _at_2(__SPL165, 1);
        /* Statements */
      ...
      _hyphenMinusGreaterThanSign_2("graph", function () {
        ...
        /* Statements */
        return _writeSvg_2(_dotDrawing_2(_graph, _options_1(_self)), _fileName);
      }),
      _hyphenMinusGreaterThanSign_2("matrix", function () {
        /* ArityCheck */
        if (arguments.length !== 0) {
  ...
  "{ :self :fileName | caseOfOtherwise(format(self), [->('array', { let __SPL165 = assertIsOfSize(pages(self), 1); let contents = at(__SPL165, 1); writeSvg(asColourSvg(contents),fileName) }), ->('graph', { let __SPL166 = assertIsOfSize(pages(self), 1); let graph = at(__SPL166, 1); writeSvg(dotDrawing(graph,options(self)),fileName) }), ->('matrix', { let __SPL167 = assertIsOfSize(pages(self), 1); let contents = at(__SPL167, 1); writeSvg(asGreyscaleSvg(contents),fileName) })], { writeSvg(asLineDrawing(self),fileName) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
    ...
    return _if_3(_isVector_1(_self), function () {
      ...
      return _if_3(_isMatrix_1(_self), function () {
        ...
        _do_2(_self, function (_each) {
          ...
          let __SPL168 = _assertIsOfSize_2(_each, 2);
          let _x2 = _at_2(__SPL168, 1);
          let _y = _at_2(__SPL168, 2);
          /* Statements */
          _add_2(_l, [_x1, _y]);
          _add_2(_l, [_x2, _y]);
          return _x1 = _x2;
  ...
  "{ :self | if(isVector(self), { let l = []; withIndexDo(self, { :each :index | add(l,[index, each]); add(l,[+(index, 1), each]) }); linePlot(l) }, { if(isMatrix(self), { let x1 = at(at(self, 1), 1); let l = []; do(self, { :each | let __SPL168 = assertIsOfSize(each, 2); let x2 = at(__SPL168, 1); let y = at(__SPL168, 2); add(l,[x1, y]); add(l,[x2, y]); x1 := x2 }); linePlot(l) }, { error(self,'stepPlot') }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    return _if_3(_isMatrix_1(_self), function () {
      ...
      let __SPL169 = _assertIsOfSize_2(_shape_1(_self), 2);
      let _m = _at_2(__SPL169, 1);
      let _n = _at_2(__SPL169, 2);
      /* Statements */
      ...
      let __SPL170 = _assertIsOfSize_2(_shape_1(_self), 3);
      let _m = _at_2(__SPL170, 1);
      let _n = _at_2(__SPL170, 2);
      let __genSym171 = _at_2(__SPL170, 3);
  ...
  "{ :self | if(isMatrix(self), { let __SPL169 = assertIsOfSize(shape(self), 2); let m = at(__SPL169, 1); let n = at(__SPL169, 2); surfacePlot(table({ :i :j | [i, j, at(self, i, j)] },to(1, m), to(1, n))) }, { let __SPL170 = assertIsOfSize(shape(self), 3); let m = at(__SPL170, 1); let n = at(__SPL170, 2); let _ = at(__SPL170, 3); let p = collect(to(1, m), { :i | collect(to(1, n), { :j | at(at(self, i), j) }) }); let q = collect(to(1, n), { :j | collect(to(1, m), { :i | at(at(self, i), j) }) }); linePlot((++(p, q))) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "Plot",
  ["self", "format", "options"],
  function (_self, _format, _options) {
    /* ArityCheck */
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :domain :anObject | let blockList = nest(anObject); linePlot(collect(blockList, { :aBlock:/1 | collect(domain, { :x | let y = aBlock(x); if(isSequence(y), { y }, { if(isComplex(y), { asList(y) }, { [x, y] }) }) }) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self_2, _xInterval, _yInterval) {
    /* ArityCheck */
    if (arguments.length !== 3) {
    ...
    let _k = 100;
    let _colourFunction_1 = function (_z) {
      /* ArityCheck */
      if (arguments.length !== 1) {
