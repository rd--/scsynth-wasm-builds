...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isMatrix(self), { let __SPL20 = assertIsOfSize(shape(self), 2); let m = at(__SPL20, 1); let n = at(__SPL20, 2); surfacePlot(table({ :i :j | [i, j, at(self, i, j)] },Span(1, m, 1), Span(1, n, 1))) }, { let __SPL21 = assertIsOfSize(shape(self), 3); let m = at(__SPL21, 1); let n = at(__SPL21, 2); let _ = at(__SPL21, 3); let p = collect(Span(1, m, 1), { :i | collect(Span(1, n, 1), { :j | at(at(self, i), j) }) }); let q = collect(Span(1, n, 1), { :j | collect(Span(1, m, 1), { :i | at(at(self, i), j) }) }); linePlot((++(p, q))) }) }",
);
