...
sl.addMethodToExistingType(
  ...
  "{ :self | if((=(format(self), 'line')), { let p:/1 = asUnaryBlock(AxonometricProjection(pi(Fraction(1L, 6L)), 0, 0, 0.5, 1, 1)); let r = [[-1, -1, 0], [+1, -1, 0], [+1, +1, 0], [-1, +1, 0]]; let t = { :list | collect(list, { :each | let __SPL2 = assertIsOfSize(each, 3); let x = at(__SPL2, 1); let y = at(__SPL2, 2); let z = at(__SPL2, 3); p([negate(x), z, negate(y)]) }) }; let l = collect(pages(self), { :each | Line(t(each)) }); LineDrawing([l]) }, { error(self,'n×3 matrix: format must be line') }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | asLineDrawing(Plot(collect(pages(self), { :each | withIndexCollect(each, { :item :x | let __SPL3 = assertIsOfSize(item, 1); let y = at(__SPL3, 1); [x, y] }) }), format(self))) }",
);
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _caseOf_3(
      ...
      sl.annotateFunction(function () {
        ...
        return _error_2(
          _self,
          _plusSignPlusSign_2("Multiple plots not implemented: ", _columnCount),
        );
      }, []),
    );
  }, ["self"]),
  "{ :self | caseOf(columnCount(self), [->(1, { asLineDrawingY(self) }), ->(2, { asLineDrawingXy(self) }), ->(3, { asLineDrawingXyz(self) })], { error(self,++('Multiple plots not implemented: ', columnCount)) }) }",
);
...
sl.addMethodToExistingType(
    ...
    let _counts = _nub_1(
      _collect_2(
        ...
        sl.annotateFunction(function (_each) {
          ...
          let __SPL4 = _assertIsOfSize_2(_shape_1(_each), 2);
          let _rowCount = _at_2(__SPL4, 1);
          let _columnCount = _at_2(__SPL4, 2);
          /* Statements */
          return _columnCount;
        }, ["each"]),
      ),
  ...
  "{ :self | let counts = nub(collect(pages(self), { :each | let __SPL4 = assertIsOfSize(shape(each), 2); let rowCount = at(__SPL4, 1); let columnCount = at(__SPL4, 2); columnCount })); if((=(size(counts), 1)), { anyOne(counts) }, { error(self,'columnCount: pages have differing column counts') }) }",
);
...
sl.addMethodToExistingType(
      ...
      [
        ...
        _hyphenMinusGreaterThanSign_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL6 = _assertIsOfSize_2(_pages_1(_self), 1);
            let _graph = _at_2(__SPL6, 1);
            /* Statements */
        ...
        _hyphenMinusGreaterThanSign_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL7 = _assertIsOfSize_2(_pages_1(_self), 1);
            let _contents = _at_2(__SPL7, 1);
            /* Statements */
  ...
  "{ :self | caseOf(format(self), [->('array', { let __SPL5 = assertIsOfSize(pages(self), 1); let contents = at(__SPL5, 1); asColourSvg(contents) }), ->('graph', { let __SPL6 = assertIsOfSize(pages(self), 1); let graph = at(__SPL6, 1); dotDrawing(graph,options(self)) }), ->('matrix', { let __SPL7 = assertIsOfSize(pages(self), 1); let contents = at(__SPL7, 1); asGreyscaleSvg(contents) })], { drawing(asLineDrawing(self)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _k, _aBlock_1) {
    ...
    let _min = _at_2(__SPL8, 1);
    let _max = _at_2(__SPL8, 2);
  ...
  "{ :self :k :aBlock:/1 | let __SPL8 = assertIsOfSize(self, 2); let min = at(__SPL8, 1); let max = at(__SPL8, 2); let colourFunction:/1 = asBlock(asColourGradient(at(colourGradients(system), 'Kovesi', 'Cyclic-Mygbm-30-95-C78S25'))); let r = subdivide((--(real(min), real(max))),k); let i = subdivide((--(imaginary(min), imaginary(max))),k); Image(reverse(table({ :x :y | let z = Complex(y, x); colourFunction(+(/(arg(aBlock(z)), pi(2)), 0.5)) },i, r))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_d) {
    ...
    let __SPL9 = _assertIsOfSize_2(_minMax_1(_d), 2);
    let _p = _at_2(__SPL9, 1);
    let _q = _at_2(__SPL9, 2);
    let __SPL10 = _assertIsOfSize_2(_p, 2);
    let _x1 = _at_2(__SPL10, 1);
    let _y1 = _at_2(__SPL10, 2);
    let __SPL11 = _assertIsOfSize_2(_q, 2);
  ...
  "{ :d | let __SPL9 = assertIsOfSize(minMax(d), 2); let p = at(__SPL9, 1); let q = at(__SPL9, 2); let __SPL10 = assertIsOfSize(p, 2); let x1 = at(__SPL10, 1); let y1 = at(__SPL10, 2); let __SPL11 = assertIsOfSize(q, 2); let x2 = at(__SPL11, 1); let y2 = at(__SPL11, 2); let k = 11; let x3 = /((-(x2, x1)), k); let y3 = /((-(y2, y1)), k); let b1 = [x1, +(x2, x3), x3]; let b2 = [y1, +(y2, y3), y3]; densityHistogramPlot(d, b1, b2) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL12 = _assertIsOfSize_2(_self, 2);
    let _b = _at_2(__SPL12, 1);
  ...
  "{ :self | let __SPL12 = assertIsOfSize(self, 2); let b = at(__SPL12, 1); let y = at(__SPL12, 2); let x = adjacentPairsCollect(b, { :i :j | +(i, (/((-(j, i)), 2))) }); discretePlot(transpose([x, y])) }",
);
...
sl.extendTypeOrTraitWithMethod(
      ...
      sl.annotateFunction(function () {
        ...
        let _r = _collect_2(
          ...
          sl.annotateFunction(function (_each) {
            ...
            let __SPL14 = _assertIsOfSize_2(_each, 3);
            let _t = _at_2(__SPL14, 1);
            let _d = _at_2(__SPL14, 2);
            let _p = _at_2(__SPL14, 3);
            /* Statements */
  ...
  "{ :self | let __SPL13 = assertIsOfSize(shape(self), 2); let m = at(__SPL13, 1); let n = at(__SPL13, 2); if((>=(n, 3)), { let tMax = injectInto(self, 0, { :z :each | max(z,+(at(each, 1), at(each, 2))) }); let tScale = /(100, max(tMax,20)); let r = collect(self, { :each | let __SPL14 = assertIsOfSize(each, 3); let t = at(__SPL14, 1); let d = at(__SPL14, 2); let p = at(__SPL14, 3); Rectangle([*(t, tScale), p], [*((+(t, d)), tScale), (+(p, 1))]) }); LineDrawing([PointCloud([[0, 32], [0, 60], [0, 84], [*(tMax, tScale), 60]]), r]) }, { error(self,'pianoRollPlot: not at least three-column matrix') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _options) {
    ...
    let __SPL16 = _assertIsOfSize_2(_deepMinMax_1(_self), 2);
    let _z0 = _at_2(__SPL16, 1);
    let _z1 = _at_2(__SPL16, 2);
  ...
  "{ :self :options | let __SPL15 = assertIsOfSize(shape(self), 2); let m = at(__SPL15, 1); let n = at(__SPL15, 2); let __SPL16 = assertIsOfSize(deepMinMax(self), 2); let z0 = at(__SPL16, 1); let z1 = at(__SPL16, 2); let colourFunction = { :z | hslToRgb([z, 0.75, 0.75]) }; let data = table({ :i :j | colourFunction(rescale(at(at(self, i), j),[z0, z1], [0, 1])) },Span(1, m, 1), Span(1, n, 1)); ifFalse(at(options, 'dataReversed'), { reverse(data) }); Image(data) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_y, _i) {
    ...
    let __SPL17 = _assertIsOfSize_2(_minMax_1(_i), 2);
  ...
  "{ :y :i | let __SPL17 = assertIsOfSize(minMax(i), 2); let a = at(__SPL17, 1); let b = at(__SPL17, 2); let c = -(b, a); if(isVector(y), { linePlot(y) }, { let n = size(y); let z = 1.15; linePlot(withIndexCollect(y, { :each :i | +(each, (*(*((-(n, i)), c), z))) })) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _k, _aBlock_2) {
    ...
    let __SPL20 = _assertIsOfSize_2(_self, 2);
    let _u = _at_2(__SPL20, 1);
    let _v = _at_2(__SPL20, 2);
    let __SPL21 = _assertIsOfSize_2(_k, 2);
  ...
  "{ :self :k :aBlock:/2 | let __SPL20 = assertIsOfSize(self, 2); let u = at(__SPL20, 1); let v = at(__SPL20, 2); let __SPL21 = assertIsOfSize(k, 2); let m = at(__SPL21, 1); let n = at(__SPL21, 2); let i = subdivide((--(min(u), max(u))),m); let j = subdivide((--(min(v), max(v))),n); surfacePlot(table(aBlock:/2, i, j)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL23 = _assertIsOfSize_2(_shape_1(_self), 3);
        let _m = _at_2(__SPL23, 1);
        let _n = _at_2(__SPL23, 2);
        let __genSym34 = _at_2(__SPL23, 3);
        let _p = _collect_2(
          _Span_3(1, _m, 1),
          sl.annotateFunction(function (_i) {
            /* ArityCheck */
  ...
  "{ :self | if(isMatrix(self), { let __SPL22 = assertIsOfSize(shape(self), 2); let m = at(__SPL22, 1); let n = at(__SPL22, 2); surfacePlot(table({ :i :j | [i, j, at(self, i, j)] },Span(1, m, 1), Span(1, n, 1))) }, { let __SPL23 = assertIsOfSize(shape(self), 3); let m = at(__SPL23, 1); let n = at(__SPL23, 2); let _ = at(__SPL23, 3); let p = collect(Span(1, m, 1), { :i | collect(Span(1, n, 1), { :j | at(at(self, i), j) }) }); let q = collect(Span(1, n, 1), { :j | collect(Span(1, m, 1), { :i | at(at(self, i), j) }) }); linePlot((++(p, q))) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :format | self := asFloat(self); if(isVector(self), { typedVectorPlot([self],format) }, { if(isColumnVector(self), { typedVectorPlot([catenate(self)],format) }, { if(isSmallFloatMatrix(self), { let __SPL24 = assertIsOfSize(shape(self), 2); let m = at(__SPL24, 1); let n = at(__SPL24, 2); if((<=(n, 3)), { Plot([self],format) }, { typedVectorPlot(self,format) }) }, { Plot(self,format) }) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _operand) {
    ...
    let _k = 100;
    let __SPL25 = _assertIsOfSize_2(_log_1(_minMax_1(_self)), 2);
  ...
  "{ :self :operand | let k = 100; let __SPL25 = assertIsOfSize(log(minMax(self)), 2); let a = at(__SPL25, 1); let b = at(__SPL25, 2); let h = /((-(b, a)), (-(k, 1))); let x = collect(upOrDownTo(0, -(k, 1)), { :i | exp((+(a, (*(i, h))))) }); linePlot(collect(nest(operand), { :aBlock:/1 | collect(x, { :each | aBlock(each) }) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_f_2, _v, _n) {
    ...
    let __SPL26 = _assertIsOfSize_2(_v, 2);
    let _a = _at_2(__SPL26, 1);
    let _b = _at_2(__SPL26, 2);
    let __SPL27 = _assertIsOfSize_2(_n, 2);
    let _i = _at_2(__SPL27, 1);
    let _j = _at_2(__SPL27, 2);
  ...
  "{ :f:/2 :v :n | let __SPL26 = assertIsOfSize(v, 2); let a = at(__SPL26, 1); let b = at(__SPL26, 2); let __SPL27 = assertIsOfSize(n, 2); let i = at(__SPL27, 1); let j = at(__SPL27, 2); let aR = resample(a,i); let bR = resample(b,j); linePlot(catenate([collect(a, { :p | collect(bR, { :q | f(p, q) }) }), collect(b, { :q | collect(aR, { :p | f(p, q) }) })])) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_f_1, _v, _m, _n) {
    ...
    let __SPL28 = _assertIsOfSize_2(_v, 2);
    let _p = _at_2(__SPL28, 1);
    let _q = _at_2(__SPL28, 2);
    let __SPL29 = _assertIsOfSize_2(_m, 2);
    let _i = _at_2(__SPL29, 1);
    let _j = _at_2(__SPL29, 2);
    /* Statements */
  ...
  "{ :f:/1 :v :m :n | let __SPL28 = assertIsOfSize(v, 2); let p = at(__SPL28, 1); let q = at(__SPL28, 2); let __SPL29 = assertIsOfSize(m, 2); let i = at(__SPL29, 1); let j = at(__SPL29, 2); carpetPlot({ :a :b | realImaginary(f(Complex(a, b))) },[asList(discretize((--(real(p), real(q))),i)), asList(discretize((--(imaginary(p), imaginary(q))),j))], n) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _xInterval, _yInterval) {
    ...
    let __SPL30 = _assertIsOfSize_2(_minMax_1(_xInterval), 2);
    let _x1 = _at_2(__SPL30, 1);
    let _x2 = _at_2(__SPL30, 2);
    let __SPL31 = _assertIsOfSize_2(_minMax_1(_yInterval), 2);
    let _y1 = _at_2(__SPL31, 1);
    let _y2 = _at_2(__SPL31, 2);
    let _n = 16;
  ...
  "{ :self :xInterval :yInterval | let __SPL30 = assertIsOfSize(minMax(xInterval), 2); let x1 = at(__SPL30, 1); let x2 = at(__SPL30, 2); let __SPL31 = assertIsOfSize(minMax(yInterval), 2); let y1 = at(__SPL31, 1); let y2 = at(__SPL31, 2); let n = 16; let xStep = /((-(x2, x1)), n); let yStep = /((-(y2, y1)), n); let minStep = min(xStep,yStep); let coordinateList = flatten(table({ :i :j | let x = +((+(x1, (*(i, xStep)))), (*((%(j, 2)), (/(xStep, 2))))); let y = +(y1, (*(j, yStep))); [x, y] },asList(upOrDownTo(0, -(n, 1))), asList(upOrDownTo(0, -(n, 1)))),1); vectorPlot(coordinateList,self, *(minStep, 0.35)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _aBlock_2, _scalar) {
    ...
    return _LineDrawing_1(_collect_2(
      ...
      sl.annotateFunction(function (_xy) {
        ...
        let __SPL32 = _assertIsOfSize_2(_xy, 2);
  ...
  "{ :self :aBlock:/2 :scalar | LineDrawing(collect(self, { :xy | let __SPL32 = assertIsOfSize(xy, 2); let x = at(__SPL32, 1); let y = at(__SPL32, 2); let uv = *(normalize(aBlock(x, y)), scalar); [Point(+(xy, uv)), Line([-(xy, uv), +(xy, uv)])] })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _timesList) {
    ...
    let _n = _at_2(__SPL33, 1);
    let _m = _at_2(__SPL33, 2);
    /* Statements */
    return _if_3(
      _equalsSign_2(_m, 2),
      sl.annotateFunction(function () {
