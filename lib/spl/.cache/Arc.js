...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _options) {
    ...
    let __SPL76 = _assertIsOfSize_2(_center_1(_self), 2);
    let _cx = _at_2(__SPL76, 1);
    let _cy = _at_2(__SPL76, 2);
    let __SPL77 = _assertIsOfSize_2(_radii_1(_self), 2);
    let _rx = _at_2(__SPL77, 1);
    let _ry = _at_2(__SPL77, 2);
    let __SPL78 = _assertIsOfSize_2(_angles_1(_self), 2);
    let _startAngle = _at_2(__SPL78, 1);
    let _endAngle = _at_2(__SPL78, 2);
    let __SPL79 = _assertIsOfSize_2(
      _plusSign_2([_cx, _cy], [
    ...
    let _x1 = _at_2(__SPL79, 1);
    let _y1 = _at_2(__SPL79, 2);
    let __SPL80 = _assertIsOfSize_2(
      _plusSign_2([_cx, _cy], [
    ...
    let _x2 = _at_2(__SPL80, 1);
    let _y2 = _at_2(__SPL80, 2);
  ...
  "{ :self :options | let precision = at(options, 'precision'); let __SPL76 = assertIsOfSize(center(self), 2); let cx = at(__SPL76, 1); let cy = at(__SPL76, 2); let __SPL77 = assertIsOfSize(radii(self), 2); let rx = at(__SPL77, 1); let ry = at(__SPL77, 2); let __SPL78 = assertIsOfSize(angles(self), 2); let startAngle = at(__SPL78, 1); let endAngle = at(__SPL78, 2); let __SPL79 = assertIsOfSize(+([cx, cy], [*(rx, cos(startAngle)), *(ry, sin(startAngle))]), 2); let x1 = at(__SPL79, 1); let y1 = at(__SPL79, 2); let __SPL80 = assertIsOfSize(+([cx, cy], [*(rx, cos(endAngle)), *(ry, sin(endAngle))]), 2); let x2 = at(__SPL80, 1); let y2 = at(__SPL80, 2); let largeArcFlag = boole((>((%((-(endAngle, startAngle)), pi(2))), pi(1)))); format('<path d=\"M %,% A % % 0 % 1 %,%\" />',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), printStringToFixed(rx,precision), printStringToFixed(ry,precision), largeArcFlag, printStringToFixed(x2,precision), printStringToFixed(y2,precision)]) }",
);
