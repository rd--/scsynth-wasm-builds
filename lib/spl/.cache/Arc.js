...
sl.addMethodToExistingType(
  ...
  "{ :self :options | let precision = at(options, 'precision'); let __SPL2 = assertIsOfSize(center(self), 2); let cx = at(__SPL2, 1); let cy = at(__SPL2, 2); let __SPL3 = assertIsOfSize(radii(self), 2); let rx = at(__SPL3, 1); let ry = at(__SPL3, 2); let __SPL4 = assertIsOfSize(angles(self), 2); let startAngle = at(__SPL4, 1); let endAngle = at(__SPL4, 2); let __SPL5 = assertIsOfSize(+([cx, cy], [*(rx, cos(startAngle)), *(ry, sin(startAngle))]), 2); let x1 = at(__SPL5, 1); let y1 = at(__SPL5, 2); let __SPL6 = assertIsOfSize(+([cx, cy], [*(rx, cos(endAngle)), *(ry, sin(endAngle))]), 2); let x2 = at(__SPL6, 1); let y2 = at(__SPL6, 2); let largeArcFlag = >((%((-(endAngle, startAngle)), pi(2))), pi(1)); format('<path d=\"M %,% %\" />',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), svgArcTo(rx, ry, 0, largeArcFlag, true, x2, y2, precision)]) }",
);
