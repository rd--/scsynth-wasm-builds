...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(radii(self), 2); let rx = at(__SplVar1, 1); let ry = at(__SplVar1, 2); if((=(rx, ry)), { rx }, { error(self,'Arc>>radius: unequal radii') }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :options | let precision = at(options, 'precision'); let __SplVar2 = assertIsOfSize(center(self), 2); let cx = at(__SplVar2, 1); let cy = at(__SplVar2, 2); let __SplVar3 = assertIsOfSize(radii(self), 2); let rx = at(__SplVar3, 1); let ry = at(__SplVar3, 2); let __SplVar4 = assertIsOfSize(angles(self), 2); let startAngle = at(__SplVar4, 1); let endAngle = at(__SplVar4, 2); let __SplVar5 = assertIsOfSize(+([cx, cy], [*(rx, cos(startAngle)), *(ry, sin(startAngle))]), 2); let x1 = at(__SplVar5, 1); let y1 = at(__SplVar5, 2); let __SplVar6 = assertIsOfSize(+([cx, cy], [*(rx, cos(endAngle)), *(ry, sin(endAngle))]), 2); let x2 = at(__SplVar6, 1); let y2 = at(__SplVar6, 2); let largeArcFlag = >((%((-(endAngle, startAngle)), pi(2))), pi(1)); format('<path d=\"M %,% %\" />',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), svgArcTo(rx, ry, 0, largeArcFlag, true, x2, y2, precision)]) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar7 = assertIsOfSize(angles(self), 2); let a = at(__SplVar7, 1); let b = at(__SplVar7, 2); abs((-(b, a))) }",
);
