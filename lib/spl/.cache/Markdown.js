...
sl.addType(
  ...
  "Markdown",
  "Markdown",
  ["Object", "Equatable", "Cache", "Iterable"],
  ["source", "cache"],
);
      ...
      sl.annotateFunction(function (_each) {
        ...
        return _ifTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let _annotation = _if_3(
              _includesKey_2(_each, "info"),
              sl.annotateFunction(function () {
                /* ArityCheck */
                if (arguments.length !== 0) {
                  const errorMessage = "Arity: expected 0, ";
  ...
  sl.annotateFunction(function (_self, _anObject, _aBlock_2) {
    ...
    return _ampersand_2(
      ...
      sl.annotateFunction(function () {
        ...
        return _aBlock_2(_source_1(_self), _source_1(_anObject));
      }, []),
    );
  }, ["self", "anObject", "aBlock:/2"]),
  "{ :self :anObject :aBlock:/2 | &(isMarkdown(anObject), { aBlock(source(self), source(anObject)) }) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _do_2(
      _lines_1(_self),
      sl.annotateFunction(function (_current) {
        /* ArityCheck */
        if (arguments.length !== 1) {
  ...
  "{ :self | let answer = []; let previous = ''; let inBlock = false; let block = []; do(lines(self), { :current | ifTrue((&(isNotEmpty(current), { &(isTab(first(current)), { &(isEmpty(previous), { not(inBlock) }) }) })), { inBlock := true }); ifTrue((&(isEmpty(current), { &(isNotEmpty(previous), { &(isTab(first(previous)), { inBlock }) }) })), { add(answer,copy(block)); removeAll(block); inBlock := false }); ifTrue(inBlock, { add(block,current) }); previous := current }); answer }",
);
