sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :first :middle :last :destination :sortBlock:/2 | let i1 = first; let i2 = +(middle, 1); let val1 = at(self, i1); let val2 = at(self, i2); let out = -(first, 1); whileTrue({ &(<=(i1, middle), { <=(i2, last) }) }, { if(sortBlock(val1, val2), { out := +(out, 1); i1 := +(i1, 1); atPut(destination, out, val1); val1 := at(self, i1) }, { out := +(out, 1); i2 := +(i2, 1); atPut(destination, out, val2); val2 := atWrap(self,i2) }) }); if((<=(i1, middle)), { replaceFromToWithStartingAt(destination,+(out, 1), last, self, i1) }, { replaceFromToWithStartingAt(destination,+(out, 1), last, self, i2) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :startIndex :stopIndex :sortBlock:/2 | let size = size(self); if((|(<=(size, 1), { =(startIndex, stopIndex) })), { self }, { ifFalse((<=(1, startIndex)), { errorSubscriptBounds(self,startIndex) }); ifFalse((<=(stopIndex, size)), { errorSubscriptBounds(self,stopIndex) }); ifFalse((<(startIndex, stopIndex)), { errorSubscriptBounds(self,startIndex) }); mergeSortFromToIntoBy(shallowCopy(self),startIndex, stopIndex, self, sortBlock:/2); self }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :firstIndex :lastIndex :destination :sortBlock:/2 | let n = -(lastIndex, firstIndex); if((<=(n, 1)), { if((=(n, 0)), { self }, { let firstObject = at(self, firstIndex); let lastObject = at(self, lastIndex); if(sortBlock(firstObject, lastObject), { atPut(destination, lastIndex, lastObject); atPut(destination, firstIndex, firstObject) }, { atPut(destination, lastIndex, firstObject); atPut(destination, firstIndex, lastObject) }); self }) }, { n := //(+(firstIndex, lastIndex), 2); mergeSortFromToIntoBy(destination,firstIndex, n, self, sortBlock:/2); mergeSortFromToIntoBy(destination,+(n, 1), lastIndex, self, sortBlock:/2); mergeFirstMiddleLastIntoBy(self,firstIndex, n, lastIndex, destination, sortBlock:/2) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :sortBlock:/2 | mergeSortFromToBy(self,1, size(self), sortBlock:/2) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :sortBlock:/2 | mergeSortBy(self,sortBlock:/2) }",
);
