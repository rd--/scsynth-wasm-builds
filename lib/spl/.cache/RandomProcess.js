sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _t, _n) {
    ...
    let __SPL541 = _assertIsOfSize_2(_t, 2);
    let _tMin = _at_2(__SPL541, 1);
    let _tMax = _at_2(__SPL541, 2);
    /* Statements */
    return _TemporalData_1(_exclamationMark_2(
      sl.annotateFunction(function () {
        ...
        _whileTrue_1(sl.annotateFunction(function () {
          ...
          let __SPL542 = _assertIsOfSize_2(_next_1(_self), 2);
          let _t = _at_2(__SPL542, 1);
          let _x = _at_2(__SPL542, 2);
          /* Statements */
          _t = _min_2(_t, _tMax);
  ...
  "{ :self :t :n | let __SPL541 = assertIsOfSize(t, 2); let tMin = at(__SPL541, 1); let tMax = at(__SPL541, 2); TemporalData(!({ let answer = []; whileTrue({ let __SPL542 = assertIsOfSize(next(self), 2); let t = at(__SPL542, 1); let x = at(__SPL542, 2); t := min(t,tMax); add(answer,[t, x]); <(t, tMax) }); reset(self); answer }, n)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _a_2, _b_2, _t, _y0) {
    ...
    let __SPL543 = _assertIsOfSize_2(_t, 3);
    let _tMin = _at_2(__SPL543, 1);
    let _tMax = _at_2(__SPL543, 2);
    let _dt = _at_2(__SPL543, 3);
  ...
  "{ :self :a:/2 :b:/2 :t :y0 | let __SPL543 = assertIsOfSize(t, 3); let tMin = at(__SPL543, 1); let tMax = at(__SPL543, 2); let dt = at(__SPL543, 3); let ts = asList(thenTo(tMin, +(tMin, dt), tMax)); let n = size(ts); let ys = List(n, 0); let dW = { :dt | normalDistribution(self,0, sqrt(dt)) }; atPut(ys, 1, y0); toDo(2, n, { :i | let t = at(ts, -(i, 1)); let y = at(ys, -(i, 1)); let dw = dW(dt); atPut(ys, i, +(+(y, (*(a(y, t), dt))), (*(b(y, t), dw)))) }); [ts, ys] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :a:/1 :b:/1 :c:/1 :t :y0 | let __SPL544 = assertIsOfSize(t, 3); let tMin = at(__SPL544, 1); let tMax = at(__SPL544, 2); let dt = at(__SPL544, 3); let ts = asList(thenTo(tMin, +(tMin, dt), tMax)); let n = size(ts); let ys = List(n, 0); let dW = { :dt | normalDistribution(self,0, sqrt(dt)) }; atPut(ys, 1, y0); toDo(2, n, { :i | let y = at(ys, -(i, 1)); let dw = dW(dt); atPut(ys, i, +(+(+(y, (*(a(y), dt))), (*(b(y), dw))), (*(*(0.5, c(y)), (-(square(dw), dt)))))) }); [ts, ys] }",
);
