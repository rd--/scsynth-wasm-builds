sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _t, _n) {
    ...
    let __SPL314 = _assertIsOfSize_2(_t, 2);
    let _tMin = _at_2(__SPL314, 1);
    let _tMax = _at_2(__SPL314, 2);
    /* Statements */
    return _TemporalData_1(_exclamationMark_2(
      sl.annotateFunction(function () {
        ...
        _whileTrue_1(sl.annotateFunction(function () {
          ...
          let __SPL315 = _assertIsOfSize_2(_next_1(_self), 2);
          let _t = _at_2(__SPL315, 1);
          let _x = _at_2(__SPL315, 2);
          /* Statements */
          _t = _min_2(_t, _tMax);
  ...
  "{ :self :t :n | let __SPL314 = assertIsOfSize(t, 2); let tMin = at(__SPL314, 1); let tMax = at(__SPL314, 2); TemporalData(!({ let answer = []; whileTrue({ let __SPL315 = assertIsOfSize(next(self), 2); let t = at(__SPL315, 1); let x = at(__SPL315, 2); t := min(t,tMax); add(answer,[t, x]); <(t, tMax) }); reset(self); answer }, n)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _a_2, _b_2, _t, _y0) {
    ...
    let __SPL316 = _assertIsOfSize_2(_t, 3);
    let _tMin = _at_2(__SPL316, 1);
    let _tMax = _at_2(__SPL316, 2);
    let _dt = _at_2(__SPL316, 3);
  ...
  "{ :self :a:/2 :b:/2 :t :y0 | let __SPL316 = assertIsOfSize(t, 3); let tMin = at(__SPL316, 1); let tMax = at(__SPL316, 2); let dt = at(__SPL316, 3); let ts = asList(thenTo(tMin, +(tMin, dt), tMax)); let n = size(ts); let ys = List(n, 0); let dW = { :dt | normalDistribution(self,0, sqrt(dt)) }; atPut(ys, 1, y0); toDo(2, n, { :i | let t = at(ts, -(i, 1)); let y = at(ys, -(i, 1)); let dw = dW(dt); atPut(ys, i, +(+(y, (*(a(y, t), dt))), (*(b(y, t), dw)))) }); [ts, ys] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _a_1, _b_1, _c_1, _t, _y0) {
    ...
    let __SPL317 = _assertIsOfSize_2(_t, 3);
    let _tMin = _at_2(__SPL317, 1);
    let _tMax = _at_2(__SPL317, 2);
    let _dt = _at_2(__SPL317, 3);
    let _ts = _asList_1(_thenTo_3(_tMin, _plusSign_2(_tMin, _dt), _tMax));
    let _n = _size_1(_ts);
    let _ys = _List_2(_n, 0);
    let _dW_1 = sl.annotateFunction(function (_dt) {
  ...
  "{ :self :a:/1 :b:/1 :c:/1 :t :y0 | let __SPL317 = assertIsOfSize(t, 3); let tMin = at(__SPL317, 1); let tMax = at(__SPL317, 2); let dt = at(__SPL317, 3); let ts = asList(thenTo(tMin, +(tMin, dt), tMax)); let n = size(ts); let ys = List(n, 0); let dW = { :dt | normalDistribution(self,0, sqrt(dt)) }; atPut(ys, 1, y0); toDo(2, n, { :i | let y = at(ys, -(i, 1)); let dw = dW(dt); atPut(ys, i, +(+(+(y, (*(a(y), dt))), (*(b(y), dw))), (*(*(0.5, c(y)), (-(squared(dw), dt)))))) }); [ts, ys] }",
);
