...
sl.addMethodToExistingType(
  ...
  "{ :self | |(isNil(positionListOrNil(self)), { let p = positionList(self); isArithmeticProgression(p,-(at(p, 2), at(p, 1)), ~) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(shape(colourList(self)), 2); let m = at(__SplVar1, 1); let n = at(__SplVar1, 2); &((=(n, 3)), { ifNil(positionListOrNil(self), { true }, { :p | =(size(p), m) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(self, 2); let c = at(__SplVar2, 1); let p = at(__SplVar2, 2); ColourGradient(c, p) }",
);
