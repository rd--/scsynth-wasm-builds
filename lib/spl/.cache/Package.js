...
sl.extendTypeOrTraitWithMethod(
    ...
    return _asRecord_1(
      _collectCatenate_2(
        ...
        sl.annotateFunction(function (_each) {
          ...
          let __SPL554 = _assertIsOfSize_2(
            _splitBy_2(_withBlanksTrimmed_1(_each), ": "),
            2,
          );
          let _key = _at_2(__SPL554, 1);
          let _value = _at_2(__SPL554, 2);
          /* Statements */
          return _caseOf_3(
            _key,
            [
              _hyphenMinusGreaterThanSign_2(
                ...
                sl.annotateFunction(function () {
                  ...
                  let __SPL555 = _assertIsOfSize_2(
                    _parseQualifiedPackageName_1(_withBlanksTrimmed_1(_value)),
                    2,
                  );
                  let _category = _at_2(__SPL555, 1);
                  let _name = _at_2(__SPL555, 2);
                  /* Statements */
                  return [
  ...
  "{ :self | let fields = splitBy(firstPliComment(self),','); asRecord(collectCatenate(fields, { :each | let __SPL554 = assertIsOfSize(splitBy(withBlanksTrimmed(each),': '), 2); let key = at(__SPL554, 1); let value = at(__SPL554, 2); caseOf(key, [->('Package', { let __SPL555 = assertIsOfSize(parseQualifiedPackageName(withBlanksTrimmed(value)), 2); let category = at(__SPL555, 1); let name = at(__SPL555, 2); [->('Category', category), ->('Name', name)] }), ->('Requires', { [->(key, words(withBlanksTrimmed(value)))] })], { error(self,++('parsePackageHeader: unknown field: ', key)) }) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL556 = _assertIsOfSize_2(_splitBy_2(_self, "-"), 2);
    let _category = _at_2(__SPL556, 1);
    let _name = _at_2(__SPL556, 2);
    /* Statements */
    return [_category, _name];
  }, ["self"]),
  "{ :self | let __SPL556 = assertIsOfSize(splitBy(self,'-'), 2); let category = at(__SPL556, 1); let name = at(__SPL556, 2); [category, name] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _name) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL557 = _assertIsOfSize_2(
          _parseQualifiedPackageName_1(_name),
          2,
        );
        let _categoryName = _at_2(__SPL557, 1);
        let _packageName = _at_2(__SPL557, 2);
  ...
  "{ :self :name | if(isQualifiedPackageName(name), { let __SPL557 = assertIsOfSize(parseQualifiedPackageName(name), 2); let categoryName = at(__SPL557, 1); let packageName = at(__SPL557, 2); let package = at(packageDictionary(self), packageName); if((&(=(categoryName, category(package)), { includesPackage(self,packageName) })), { package }, { error(self,++('package: category name mismatch or package not registered: ', name)) }) }, { if(includesPackage(self,name), { at(packageDictionary(self), name) }, { error(self,++('package: package not registered: ', name)) }) }) }",
);
