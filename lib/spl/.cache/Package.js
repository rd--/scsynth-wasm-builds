...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject :aBlock:/2 | ==(self, anObject) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let fields = splitBy(firstPliComment(self),','); asRecord(collectCatenate(fields, { :each | let __SplVar1 = assertIsOfSize(splitBy(withBlanksTrimmed(each),': '), 2); let key = at(__SplVar1, 1); let value = at(__SplVar1, 2); caseOf(key, [->('Package', { let __SplVar2 = assertIsOfSize(parseQualifiedPackageName(withBlanksTrimmed(value)), 2); let category = at(__SplVar2, 1); let name = at(__SplVar2, 2); [->('Category', category), ->('Name', name)] }), ->('Requires', { [->(key, words(withBlanksTrimmed(value)))] })], { error(self,++('parsePackageHeader: unknown field: ', key)) }) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar3 = assertIsOfSize(splitBy(self,'-'), 2); let category = at(__SplVar3, 1); let name = at(__SplVar3, 2); [category, name] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :name | if(isQualifiedPackageName(name), { let __SplVar4 = assertIsOfSize(parseQualifiedPackageName(name), 2); let categoryName = at(__SplVar4, 1); let packageName = at(__SplVar4, 2); let package = at(packageDictionary(self), packageName); if((&(=(categoryName, category(package)), { includesPackage(self,packageName) })), { package }, { error(self,++('package: category name mismatch or package not registered: ', name)) }) }, { if(includesPackage(self,name), { at(packageDictionary(self), name) }, { error(self,++('package: package not registered: ', name)) }) }) }",
);
