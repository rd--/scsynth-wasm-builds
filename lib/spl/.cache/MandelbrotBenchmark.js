sl.extendTypeOrTraitWithMethod(
  ...
  "{ Benchmark('Mandelbrot', Map([[1, 128], [500, 191], [750, 50]]), { :size | let sum = 0; let byteAcc = 0; let bitNum = 0; let y = 0; whileTrue({ <(y, size) }, { let ci = -((/(*(2, y), size)), 1); let x = 0; whileTrue({ <(x, size) }, { let zrzr = 0; let zr = 0; let zizi = 0; let zi = 0; let cr = -((/(*(2, x), size)), 1.5); let z = 0; let notDone = true; let escape = 0; whileTrue({ &(notDone, { <(z, 50) }) }, { zr := +(-(zrzr, zizi), cr); zi := +(*(*(2, zr), zi), ci); zrzr := *(zr, zr); zizi := *(zi, zi); ifTrue((>(+(zrzr, zizi), 4)), { notDone := false; escape := 1 }); z := +(z, 1) }); byteAcc := +((<<(byteAcc, 1)), escape); bitNum := +(bitNum, 1); if((=(bitNum, 8)), { sum := bitXor(sum,byteAcc); byteAcc := 0; bitNum := 0 }, { ifTrue((=(x, (-(size, 1)))), { byteAcc := <<(byteAcc, (-(8, bitNum))); sum := bitXor(sum,byteAcc); byteAcc := 0; bitNum := 0 }) }); x := +(x, 1) }); y := +(y, 1) }); sum }) }",
);
