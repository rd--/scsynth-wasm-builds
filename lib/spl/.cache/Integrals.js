  ...
  sl.annotateFunction(function (_f_1, _i, _epsilon) {
    ...
    let _rec_8 = sl.annotateFunction(
      function (_a, _fa, _b, _fb, _epsilon, _whole, _m, _fm) {
        ...
        let __SPL1 = _assertIsOfSize_2(_mem_4(_a, _fa, _m, _fm), 3);
        let _lm = _at_2(__SPL1, 1);
        let _flm = _at_2(__SPL1, 2);
        let _left = _at_2(__SPL1, 3);
        let __SPL2 = _assertIsOfSize_2(_mem_4(_m, _fm, _b, _fb), 3);
        let _rm = _at_2(__SPL2, 1);
        let _frm = _at_2(__SPL2, 2);
        let _right = _at_2(__SPL2, 3);
    ...
    let _fa = _f_1(_a);
    let _fb = _f_1(_b);
    let __SPL3 = _assertIsOfSize_2(_mem_4(_a, _fa, _b, _fb), 3);
    let _m = _at_2(__SPL3, 1);
    let _fm = _at_2(__SPL3, 2);
    let _whole = _at_2(__SPL3, 3);
    /* Statements */
    return _rec_8(_a, _fa, _b, _fb, _epsilon, _whole, _m, _fm);
  }, ["f:/1", "i", "epsilon"]),
  "{ :f:/1 :i :epsilon | let mem = { :a :fa :b :fb | let m = /((+(a, b)), 2); let fm = f(m); let s = *(/(abs((-(b, a))), 6), (+(+(fa, (*(4, fm))), fb))); [m, fm, s] }; let rec = { :a :fa :b :fb :epsilon :whole :m :fm | let __SPL1 = assertIsOfSize(mem(a, fa, m, fm), 3); let lm = at(__SPL1, 1); let flm = at(__SPL1, 2); let left = at(__SPL1, 3); let __SPL2 = assertIsOfSize(mem(m, fm, b, fb), 3); let rm = at(__SPL2, 1); let frm = at(__SPL2, 2); let right = at(__SPL2, 3); let delta = -(+(left, right), whole); if((<=(abs(delta), (*(epsilon, 15)))), { +(+(left, right), (/(delta, 15))) }, { +(rec(a, fa, m, fm, /(epsilon, 2), left, lm, flm), rec(m, fm, b, fb, /(epsilon, 2), right, rm, frm)) }) }; let a = at(i, 1); let b = at(i, 2); let fa = f(a); let fb = f(b); let __SPL3 = assertIsOfSize(mem(a, fa, b, fb), 3); let m = at(__SPL3, 1); let fm = at(__SPL3, 2); let whole = at(__SPL3, 3); rec(a, fa, b, fb, epsilon, whole, m, fm) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_f_1, _interval, _n) {
    ...
    let __SPL4 = _assertIsOfSize_2(_minMax_1(_interval), 2);
  ...
  "{ :f:/1 :interval :n | let __SPL4 = assertIsOfSize(minMax(interval), 2); let a = at(__SPL4, 1); let b = at(__SPL4, 2); let stepSize = /((-(b, a)), n); let integral = /((+(f(a), f(b))), 2); let x = +(a, stepSize); whileTrue({ <(x, b) }, { integral := +(integral, f(x)); x := +(x, stepSize) }); *(integral, stepSize) }",
);
