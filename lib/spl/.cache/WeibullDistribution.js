sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :alpha :beta :mu | let r = 1; whileTrue({ =(r, 1) }, { r := nextRandomFloat(self) }); +(*(beta, (^(negate(log(r)), (/(1, alpha))))), mu) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let alpha = alpha(self); let beta = beta(self); let mu = mu(self); { :x | if((>=(x, 0)), { -(1, exp(negate((^((/((-(x, mu)), beta)), alpha))))) }, { 0 }) } }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let alpha = alpha(self); let beta = beta(self); let mu = mu(self); { :x | if((>(x, mu)), { /((*(alpha, (^((/((+(-(mu), x)), beta)), (+(-1, alpha)))))), beta) }, { 0 }) } }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | { :p | if((&(>(p, 0), { <(p, 1) })), { +(mu(self), (*(beta(self), (^(-(log((-(1, p)))), (/(1, alpha(self)))))))) }, { if((<=(p, 0)), { mu(self) }, { Infinity }) }) } }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let alpha = alpha(self); let beta = beta(self); let mu = mu(self); { :x | if((>=(x, mu)), { *(*((/(alpha, beta)), (^((/((-(x, mu)), beta)), (-(alpha, 1))))), exp(negate((^((/((-(x, mu)), beta)), alpha))))) }, { 0 }) } }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :rng :shape | let alpha = alpha(self); let beta = beta(self); let mu = mu(self); !({ weibullDistribution(rng,alpha, beta, mu) }, shape) }",
);
