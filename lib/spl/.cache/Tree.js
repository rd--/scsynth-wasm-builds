...
sl.addMethodToExistingType(
  ...
  "{ :self :index :ifAbsent:/0 | atIfAbsent(subTrees(self),index, ifAbsent:/0) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject :aBlock:/2 | &(isTree(anObject), { &(aBlock(value(self), value(anObject)), { equalBy(subTrees(self), subTrees(anObject), aBlock:/2) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :n :r | unfoldTree(n, r, { :each | let __SPL22 = assertIsOfSize([numerator(each), denominator(each)], 2); let a = at(__SPL22, 1); let b = at(__SPL22, 2); [Fraction(a, +(a, b)), Fraction(+(a, b), b)] }) }",
);
