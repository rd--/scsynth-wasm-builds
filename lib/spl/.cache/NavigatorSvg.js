...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _scale, _projection_1) {
    ...
    let _dots = _collect_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL284 = _assertIsOfSize_2(_each, 2);
        let _x = _at_2(__SPL284, 1);
        let _y = _at_2(__SPL284, 2);
        /* Statements */
    ...
    let _lines = _collect_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let _i = _at_2(__SPL285, 1);
        let _j = _at_2(__SPL285, 2);
        let __SPL286 = _assertIsOfSize_2(_at_2(_points, _i), 2);
        let _x1 = _at_2(__SPL286, 1);
        let _y1 = _at_2(__SPL286, 2);
        let __SPL287 = _assertIsOfSize_2(_at_2(_points, _j), 2);
        let _x2 = _at_2(__SPL287, 1);
        let _y2 = _at_2(__SPL287, 2);
        /* Statements */
        return _createSvgElement_2(
          "line",
  ...
  "{ :self :scale :projection:/1 | let lineWidth = 0.25; let points = collect((*(vertexCoordinates(self), scale)),projection:/1); let bbox = computeBoundingBoxRectangle(points); let dots = collect(points, { :each | let __SPL284 = assertIsOfSize(each, 2); let x = at(__SPL284, 1); let y = at(__SPL284, 2); createSvgElement('circle', Record([['cx', x], ['cy', y], ['r', *(lineWidth, 2)], ['fill', 'black']])) }); let lines = collect(edgeList(self), { :each | let __SPL285 = assertIsOfSize(each, 2); let i = at(__SPL285, 1); let j = at(__SPL285, 2); let __SPL286 = assertIsOfSize(at(points, i), 2); let x1 = at(__SPL286, 1); let y1 = at(__SPL286, 2); let __SPL287 = assertIsOfSize(at(points, j), 2); let x2 = at(__SPL287, 1); let y2 = at(__SPL287, 2); createSvgElement('line', Record([['x1', x1], ['y1', y1], ['x2', x2], ['y2', y2], ['stroke', 'black'], ['stroke-width', lineWidth]])) }); let svg = createSvgElement('svg', Record([['width', width(bbox)], ['height', height(bbox)], ['viewBox', asSvgViewBox(bbox, Record([['margin', 5], ['precision', 1]]))], ['preserveAspectRatio', 'xMidYMid meet']])); let group = createSvgElement('g', Record([['transform', unwords([format('translate(0, %)',[+(height(bbox), (*(2, at(lowerLeft(bbox), 2))))]), 'scale(1, -1)'])]])); appendChildren(group,dots); appendChildren(group,lines); appendChild(svg,group); svg }",
);
