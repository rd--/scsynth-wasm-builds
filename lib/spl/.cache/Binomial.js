...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :m :n | sum(collect(upOrDownTo(0, min(m,n)), { :k | *(integerBinomial((-(+(m, n), k)),m), integerBinomial(m,k)) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isOdd(self), { 0 }, { let f:/1 = memoize({ :m | let n = /(m, 2); if((=(n, 0)), { 1 }, { negate(sum(collect(Span(1, n, 1), { :k | *(integerBinomial(*(2, n), *(2, k)), f(*(2, (-(n, k))))) }))) }) },false); f(self) }) }",
);
