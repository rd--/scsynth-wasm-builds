...
sl.addMethodToExistingTrait(
  ...
  "{ :self | let v = vertexList(self); let e = Set(); comparator(e, matchesEdge:/2); do(edgeList(self), { :each | ifTrue((!=(at(each, 1), at(each, 2))), { include(e,each) }) }); Graph(v, contents(e)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_m, _n) {
    ...
    _tuplesDo_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL5 = _assertIsOfSize_2(_each, 2);
        let _i = _at_2(__SPL5, 1);
        let _j = _at_2(__SPL5, 2);
        let __SPL6 = _assertIsOfSize_2(_atAll_2(_s, [_i, _j]), 2);
  ...
  "{ :m :n | let o = -(n, 1); let k = ^(m, n); let s = collect(upOrDownTo(0, -(k, 1)), { :i | integerDigits(i,m, n) }); let v = asList(upOrDownTo(1, k)); let e = []; let g = nil; let withLabels = false; tuplesDo([v, v], { :each | let __SPL5 = assertIsOfSize(each, 2); let i = at(__SPL5, 1); let j = at(__SPL5, 2); let __SPL6 = assertIsOfSize(atAll(s,[i, j]), 2); let p = at(__SPL6, 1); let q = at(__SPL6, 2); ifTrue((=(last(p,o), first(q,o))), { add(e,[i, j]) }) }); g := Graph(v, e); ifTrue(withLabels, { vertexLabels(g,collect(s, { :each | stringCatenate(collect(each,asString:/1)) })) }); g }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let m = deepCollect(self, { :each | if((=(each, Infinity)), { 0 }, { 1 }) }); let g = adjacencyGraph(m); let w = collect(edgeList(g), { :each | let __SPL7 = assertIsOfSize(each, 2); let i = at(__SPL7, 1); let j = at(__SPL7, 2); at(self, i, j) }); edgeWeights(g, w); g }",
);
