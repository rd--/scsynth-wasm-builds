...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g, _s, _t) {
    ...
    let _answer = [];
    let __SPL13 = _assertIsOfSize_2(_dijkstrasAlgorithm_3(_g, _s, _t), 2);
    let _d = _at_2(__SPL13, 1);
    let _p = _at_2(__SPL13, 2);
    let _u = _t;
    /* Statements */
    _ifTrue_2(
      _verticalLine_2(
  ...
  "{ :g :s :t | let answer = []; let __SPL13 = assertIsOfSize(dijkstrasAlgorithm(g,s, t), 2); let d = at(__SPL13, 1); let p = at(__SPL13, 2); let u = t; ifTrue((|(~=(at(p, u), nil), { =(u, s) })), { whileTrue({ ~=(u, nil) }, { addFirst(answer,u); u := at(p, u) }) }); answer }",
);
...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g, _s, _t) {
    ...
    let __SPL14 = _assertIsOfSize_2(_dijkstrasAlgorithm_3(_g, _s, _t), 2);
    let _d = _at_2(__SPL14, 1);
    let _p = _at_2(__SPL14, 2);
    /* Statements */
    return _at_2(_d, _t);
  }, ["g", "s", "t"]),
  "{ :g :s :t | let __SPL14 = assertIsOfSize(dijkstrasAlgorithm(g,s, t), 2); let d = at(__SPL14, 1); let p = at(__SPL14, 2); at(d, t) }",
);
...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g) {
    ...
    _toDo_3(
      ...
      sl.annotateFunction(function (_i) {
        ...
        let __SPL15 = _assertIsOfSize_2(_dijkstrasAlgorithm_2(_g, _i), 2);
        let _d = _at_2(__SPL15, 1);
        let _p = _at_2(__SPL15, 2);
        /* Statements */
        return _add_2(_m, _d);
      }, ["i"]),
    );
  ...
  "{ :g | let n = vertexCount(g); let m = []; toDo(1, n, { :i | let __SPL15 = assertIsOfSize(dijkstrasAlgorithm(g,i), 2); let d = at(__SPL15, 1); let p = at(__SPL15, 2); add(m,d) }); m }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let m = deepCollect(self, { :each | if((=(each, Infinity)), { 0 }, { 1 }) }); let g = adjacencyGraph(m); let w = collect(edgeList(g), { :each | let __SPL16 = assertIsOfSize(each, 2); let i = at(__SPL16, 1); let j = at(__SPL16, 2); at(self, i, j) }); edgeWeights(g, w); g }",
);
