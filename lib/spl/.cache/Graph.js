...
sl.addMethodToExistingTrait(
  ...
  "{ :self | let vertexCoordinates = vertexCoordinates(self); let contents = [PointCloud(vertexCoordinates)]; do(edgeList(self), { :each | let __SPL1 = assertIsOfSize(each, 2); let i = at(__SPL1, 1); let j = at(__SPL1, 2); add(contents,LineSegment(at(vertexCoordinates, i), at(vertexCoordinates, j))) }); GeometryCollection(contents) }",
);
...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g, _s, _t) {
    ...
    let _answer = [];
    let __SPL2 = _assertIsOfSize_2(_dijkstrasAlgorithm_3(_g, _s, _t), 2);
    let _d = _at_2(__SPL2, 1);
    let _p = _at_2(__SPL2, 2);
    let _u = _t;
    /* Statements */
    _ifTrue_2(
      _verticalLine_2(
  ...
  "{ :g :s :t | let answer = []; let __SPL2 = assertIsOfSize(dijkstrasAlgorithm(g,s, t), 2); let d = at(__SPL2, 1); let p = at(__SPL2, 2); let u = t; ifTrue((|(!=(at(p, u), nil), { =(u, s) })), { whileTrue({ !=(u, nil) }, { addFirst(answer,u); u := at(p, u) }) }); answer }",
);
...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g, _s, _t) {
    ...
    let __SPL3 = _assertIsOfSize_2(_dijkstrasAlgorithm_3(_g, _s, _t), 2);
    let _d = _at_2(__SPL3, 1);
    let _p = _at_2(__SPL3, 2);
    /* Statements */
    return _at_2(_d, _t);
  }, ["g", "s", "t"]),
  "{ :g :s :t | let __SPL3 = assertIsOfSize(dijkstrasAlgorithm(g,s, t), 2); let d = at(__SPL3, 1); let p = at(__SPL3, 2); at(d, t) }",
);
...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_g) {
    ...
    _toDo_3(
      ...
      sl.annotateFunction(function (_i) {
        ...
        let __SPL4 = _assertIsOfSize_2(_dijkstrasAlgorithm_2(_g, _i), 2);
        let _d = _at_2(__SPL4, 1);
        let _p = _at_2(__SPL4, 2);
        /* Statements */
        return _add_2(_m, _d);
      }, ["i"]),
    );
  ...
  "{ :g | let n = vertexCount(g); let m = []; toDo(1, n, { :i | let __SPL4 = assertIsOfSize(dijkstrasAlgorithm(g,i), 2); let d = at(__SPL4, 1); let p = at(__SPL4, 2); add(m,d) }); m }",
);
  ...
  sl.annotateFunction(function (_g1, _g2, _kind) {
    ...
    _toDo_3(
      ...
      sl.annotateFunction(function (_i) {
        ...
        let __SPL5 = _assertIsOfSize_2(_at_2(_v, _i), 2);
        let _u1 = _at_2(__SPL5, 1);
        let _u2 = _at_2(__SPL5, 2);
        /* Statements */
        return _toDo_3(
          _plusSign_2(_i, 1),
          _k,
          sl.annotateFunction(function (_j) {
            ...
            let __SPL6 = _assertIsOfSize_2(_at_2(_v, _j), 2);
            let _v1 = _at_2(__SPL6, 1);
            let _v2 = _at_2(__SPL6, 2);
            /* Statements */
            return _ifTrue_2(
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_m, _n) {
    ...
    _tuplesDo_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL8 = _assertIsOfSize_2(_atAll_2(_s, [_i, _j]), 2);
  ...
  "{ :m :n | let o = -(n, 1); let k = ^(m, n); let s = collect(upOrDownTo(0, -(k, 1)), { :i | integerDigits(i,m, n) }); let v = asList(upOrDownTo(1, k)); let e = []; let g = nil; let withLabels = false; tuplesDo([v, v], { :each | let __SPL7 = assertIsOfSize(each, 2); let i = at(__SPL7, 1); let j = at(__SPL7, 2); let __SPL8 = assertIsOfSize(atAll(s,[i, j]), 2); let p = at(__SPL8, 1); let q = at(__SPL8, 2); ifTrue((=(last(p,o), first(q,o))), { add(e,[i, j]) }) }); g := Graph(v, e); ifTrue(withLabels, { vertexLabels(g,collect(s, { :each | stringCatenate(collect(each,asString:/1)) })) }); g }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let m = deepCollect(self, { :each | if((=(each, Infinity)), { 0 }, { 1 }) }); let g = adjacencyGraph(m); let w = collect(edgeList(g), { :each | let __SPL9 = assertIsOfSize(each, 2); let i = at(__SPL9, 1); let j = at(__SPL9, 2); at(self, i, j) }); edgeWeights(g, w); g }",
);
