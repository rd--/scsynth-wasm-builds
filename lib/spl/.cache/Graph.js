...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let m = deepCollect(self, { :each | if((=(each, Infinity)), { 0 }, { 1 }) }); let g = adjacencyGraph(m); let w = collect(edgeList(g), { :each | let __SPL5 = assertIsOfSize(each, 2); let i = at(__SPL5, 1); let j = at(__SPL5, 2); at(self, i, j) }); edgeWeights(g, w); g }",
);
