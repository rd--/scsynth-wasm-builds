...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _projectionName) {
    ...
    let _f_1 = sl.annotateFunction(function (_each) {
      ...
      let __SPL522 = _assertIsOfSize_2(_degreesToRadians_1(_each), 2);
      let _phi = _at_2(__SPL522, 1);
      let _lambda = _at_2(__SPL522, 2);
      /* Statements */
  ...
  "{ :self :projectionName | let projectionFunction:/1 = namedCartographicProjection(projectionName); let f = { :each | let __SPL522 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SPL522, 1); let lambda = at(__SPL522, 2); projectionFunction([lambda, phi]) }; collect(geometryValues(self), { :each | project(each,f:/1) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | collect(simplyConnectedPolygons(self), { :each | let __SPL523 = assertIsOfSize(coordinates(each), 1); let c = at(__SPL523, 1); c }) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _projectionFunction_1) {
    ...
    return _collect_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL524 = _assertIsOfSize_2(_coordinates_1(_each), 1);
        let _c = _at_2(__SPL524, 1);
        /* Statements */
        return _collect_2(
          _c,
          sl.annotateFunction(function (_each) {
            ...
            let __SPL525 = _assertIsOfSize_2(_degreesToRadians_1(_each), 2);
            let _phi = _at_2(__SPL525, 1);
            let _lambda = _at_2(__SPL525, 2);
            /* Statements */
            return _projectionFunction_1([_lambda, _phi]);
          }, ["each"]),
        );
  ...
  "{ :self :projectionFunction:/1 | collect(simplyConnectedPolygons(self), { :each | let __SPL524 = assertIsOfSize(coordinates(each), 1); let c = at(__SPL524, 1); collect(c, { :each | let __SPL525 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SPL525, 1); let lambda = at(__SPL525, 2); projectionFunction([lambda, phi]) }) }) }",
);
