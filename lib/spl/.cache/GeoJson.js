...
sl.addMethodToExistingType(
  ...
  "{ :self :projectionName | let projectionFunction:/1 = namedCartographicProjection(projectionName); let f = { :each | let __SplVar1 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SplVar1, 1); let lambda = at(__SplVar1, 2); projectionFunction([lambda, phi]) }; collect(geometryValues(self), { :each | project(each,f:/1) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | collect(simplyConnectedPolygons(self), { :each | let __SplVar2 = assertIsOfSize(coordinates(each), 1); let c = at(__SplVar2, 1); c }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :projectionFunction:/1 | collect(simplyConnectedPolygons(self), { :each | let __SplVar3 = assertIsOfSize(coordinates(each), 1); let c = at(__SplVar3, 1); collect(c, { :each | let __SplVar4 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SplVar4, 1); let lambda = at(__SplVar4, 2); projectionFunction([lambda, phi]) }) }) }",
);
