...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _projectionName) {
    ...
    let _f_1 = sl.annotateFunction(function (_each) {
      ...
      let __SPL1 = _assertIsOfSize_2(_degreesToRadians_1(_each), 2);
      let _phi = _at_2(__SPL1, 1);
      let _lambda = _at_2(__SPL1, 2);
      /* Statements */
  ...
  "{ :self :projectionName | let projectionFunction:/1 = namedCartographicProjection(projectionName); let f = { :each | let __SPL1 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SPL1, 1); let lambda = at(__SPL1, 2); projectionFunction([lambda, phi]) }; collect(geometryValues(self), { :each | project(each,f:/1) }) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _collect_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL2 = _assertIsOfSize_2(_coordinates_1(_each), 1);
  ...
  "{ :self | collect(simplyConnectedPolygons(self), { :each | let __SPL2 = assertIsOfSize(coordinates(each), 1); let c = at(__SPL2, 1); c }) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _projectionFunction_1) {
    ...
    return _collect_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL3 = _assertIsOfSize_2(_coordinates_1(_each), 1);
        ...
        return _collect_2(
          ...
          sl.annotateFunction(function (_each) {
            ...
            let __SPL4 = _assertIsOfSize_2(_degreesToRadians_1(_each), 2);
            let _phi = _at_2(__SPL4, 1);
            let _lambda = _at_2(__SPL4, 2);
            /* Statements */
            return _projectionFunction_1([_lambda, _phi]);
          }, ["each"]),
        );
  ...
  "{ :self :projectionFunction:/1 | collect(simplyConnectedPolygons(self), { :each | let __SPL3 = assertIsOfSize(coordinates(each), 1); let c = at(__SPL3, 1); collect(c, { :each | let __SPL4 = assertIsOfSize(degreesToRadians(each), 2); let phi = at(__SPL4, 1); let lambda = at(__SPL4, 2); projectionFunction([lambda, phi]) }) }) }",
);
