...
sl.addMethodToExistingType(
  ...
  "{ :self | let codeTable = reverse(sortOn(associations(symbolTable(self)), { :x | size(value(x)) })); let codes = encoding(self); let symbols = []; whileFalse({ isEmpty(codes) }, { let code = detect(codeTable, { :each | beginsWith(codes,value(each)) }); add(symbols,key(code)); removeFirst(codes,size(value(code))) }); symbols }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _symbolTable = _huffmanSymbolTable_1(_asBag_1(_self));
    /* Statements */
