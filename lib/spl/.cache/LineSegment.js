...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _eq_2) {
    ...
    _whileTrue_2(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL31 = _assertIsOfSize_2(_l, 2);
        let _p = _at_2(__SPL31, 1);
        let _q = _at_2(__SPL31, 2);
        let _i = 1;
        /* Statements */
        _removeAt_2(_self, 1);
        _whileTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL32 = _assertIsOfSize_2(_at_2(_self, _i), 2);
  ...
  "{ :self :eq:/2 | let answer = []; whileTrue({ not(isEmpty(self)) }, { let l = at(self, 1); let __SPL31 = assertIsOfSize(l, 2); let p = at(__SPL31, 1); let q = at(__SPL31, 2); let i = 1; removeAt(self,1); whileTrue({ <=(i, size(self)) }, { let __SPL32 = assertIsOfSize(at(self, i), 2); let a = at(__SPL32, 1); let b = at(__SPL32, 2); let matched = true; if(eq(p, a), { addFirst(l,b); p := b }, { if(eq(p, b), { addFirst(l,a); p := a }, { if(eq(q, a), { addLast(l,b); q := b }, { if(eq(q, b), { addLast(l,a); q := a }, { matched := false }) }) }) }); if(matched, { removeAt(self,i); i := 1 }, { i := +(i, 1) }) }); add(answer,l) }); answer }",
);
