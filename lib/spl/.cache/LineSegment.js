...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _eq_2) {
    ...
    _whileTrue_2(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL164 = _assertIsOfSize_2(_l, 2);
        let _p = _at_2(__SPL164, 1);
        let _q = _at_2(__SPL164, 2);
        let _i = 1;
        /* Statements */
        _removeAt_2(_self, 1);
        _whileTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL165 = _assertIsOfSize_2(_at_2(_self, _i), 2);
            let _a = _at_2(__SPL165, 1);
            let _b = _at_2(__SPL165, 2);
            let _matched = true;
            /* Statements */
            _if_3(
              _eq_2(_p, _a),
  ...
  "{ :self :eq:/2 | let answer = []; whileTrue({ not(isEmpty(self)) }, { let l = at(self, 1); let __SPL164 = assertIsOfSize(l, 2); let p = at(__SPL164, 1); let q = at(__SPL164, 2); let i = 1; removeAt(self,1); whileTrue({ <=(i, size(self)) }, { let __SPL165 = assertIsOfSize(at(self, i), 2); let a = at(__SPL165, 1); let b = at(__SPL165, 2); let matched = true; if(eq(p, a), { addFirst(l,b); p := b }, { if(eq(p, b), { addFirst(l,a); p := a }, { if(eq(q, a), { addLast(l,b); q := b }, { if(eq(q, b), { addLast(l,a); q := a }, { matched := false }) }) }) }); if(matched, { removeAt(self,i); i := 1 }, { i := +(i, 1) }) }); add(answer,l) }); answer }",
);
