...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :eq:/2 | let answer = []; whileTrue({ not(isEmpty(self)) }, { let l = at(self, 1); let __SplVar1 = assertIsOfSize(l, 2); let p = at(__SplVar1, 1); let q = at(__SplVar1, 2); let i = 1; removeAt(self,1); whileTrue({ <=(i, size(self)) }, { let __SplVar2 = assertIsOfSize(at(self, i), 2); let a = at(__SplVar2, 1); let b = at(__SplVar2, 2); let matched = true; if(eq(p, a), { addFirst(l,b); p := b }, { if(eq(p, b), { addFirst(l,a); p := a }, { if(eq(q, a), { addLast(l,b); q := b }, { if(eq(q, b), { addLast(l,a); q := a }, { matched := false }) }) }) }); if(matched, { removeAt(self,i); i := 1 }, { i := +(i, 1) }) }); add(answer,l) }); answer }",
);
