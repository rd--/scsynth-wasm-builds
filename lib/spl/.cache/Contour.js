sl.extendTypeOrTraitWithMethod(
  ...
  ["d", "x", "y", "z", "drawContour:/6"],
  sl.annotateFunction(function (_d, _x, _y, _z, _drawContour_6) {
    /* ArityCheck */
    if (arguments.length !== 5) {
      const errorMessage = "Arity: expected 5, _d, _x, _y, _z, _drawContour_6";
      throw new Error(errorMessage);
    } /* Temporaries */
    let __SPL109 = _assertIsOfSize_2(_shape_1(_d), 2);
    let _m = _at_2(__SPL109, 1);
    let _n = _at_2(__SPL109, 2);
    /* Statements */
    ...
    _assert_1(sl.annotateFunction(function () {
      ...
      return _equalsSign_2(_numArgs_1(_drawContour_6), 6);
    }, []));
  ...
  "{ :d :x :y :z :drawContour:/6 | let __SPL109 = assertIsOfSize(shape(d), 2); let m = at(__SPL109, 1); let n = at(__SPL109, 2); assert({ =(size(x), m) }); assert({ =(size(y), n) }); assert({ >(size(z), 0) }); assert({ isMonotonicallyIncreasing(z) }); assert({ =(numArgs(drawContour:/6), 6) }); uncheckedBourkeContourAlgorithm(d, 0, -(m, 1), 0, -(n, 1), asList(x), asList(y), size(z), asList(z), drawContour:/6) }",
);
