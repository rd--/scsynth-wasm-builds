...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :z :o | if(isVector(self), { let __SPL3 = assertIsOfSize(z, 2); let m = at(__SPL3, 1); let n = at(__SPL3, 2); let k = size(self); collect(upOrDownTo(1, +(k, sum(z))), { :i | let d = -(i, m); if(betweenAnd(d,1, k), { at(self, d) }, { o }) }) }, { if(isMatrix(self), { let __SPL4 = assertIsOfSize(z, 2); let m = at(__SPL4, 1); let n = at(__SPL4, 2); let __SPL5 = assertIsOfSize(shape(self), 2); let a = at(__SPL5, 1); let b = at(__SPL5, 2); let p = +(sum(m), a); let q = +(sum(n), b); table({ :i :j | let d = -(i, at(m, 1)); let e = -(j, at(n, 1)); if((&(betweenAnd(d,1, a), { betweenAnd(e,1, b) })), { at(at(self, d), e) }, { o }) },to(1, p), to(1, q)) }, { error(self,'arrayPad') }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :rangeList :stepList :offset | tuples(withCollect(((each(+) . (rangeList, offset))), stepList, { :range :step | let __SPL6 = assertIsOfSize(range, 2); let l = at(__SPL6, 1); let r = at(__SPL6, 2); asList(thenTo(l, +(l, step), r)) })) }",
);
