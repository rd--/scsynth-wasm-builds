...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_a, _b, _c) {
    ...
    return sl.annotateFunction(function (_p) {
      ...
      let __SPL94 = _assertIsOfSize_2(
        _dot_2(_inverse_1(_t), _hyphenMinus_2(_p, _c)),
        2,
      );
      let _u = _at_2(__SPL94, 1);
      let _v = _at_2(__SPL94, 2);
      /* Statements */
      return [_u, _v, _hyphenMinus_2(_hyphenMinus_2(1, _u), _v)];
    }, ["p"]);
  }, ["a", "b", "c"]),
  "{ :a :b :c | let t = [[-(at(a, 1), at(c, 1)), -(at(b, 1), at(c, 1))], [-(at(a, 2), at(c, 2)), -(at(b, 2), at(c, 2))]]; { :p | let __SPL94 = assertIsOfSize(dot(inverse(t),-(p, c)), 2); let u = at(__SPL94, 1); let v = at(__SPL94, 2); [u, v, -(-(1, u), v)] } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_a, _b, _c, _d) {
    ...
    let _f_4 = sl.annotateFunction(function (_u, _v, _w, _x) {
      ...
      return _determinant_1(
        _transposed_1(_arrayPad_3([_u, _v, _w, _x], [[0, 0], [0, 1]], 1)),
      );
    }, ["u", "v", "w", "x"]);
  ...
  "{ :a :b :c :d | let f = { :u :v :w :x | determinant(transposed(arrayPad([u, v, w, x],[[0, 0], [0, 1]], 1))) }; let z = f(a, b, c, d); { :p | let u = /(f(p, b, c, d), z); let v = /(f(a, p, c, d), z); let w = /(f(a, b, p, d), z); [u, v, w, -(-(-(1, u), v), w)] } }",
);
