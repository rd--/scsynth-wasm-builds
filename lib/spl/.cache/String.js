...
sl.addMethodToExistingType(
  ...
  "{ :self :index :ifAbsent:/0 | if(includesIndex(self,index), { let codePoint = codePointAt(self,index); ifNil(codePoint, { ifAbsent() }, { if(isUtf16SurrogateCodePoint(codePoint), { error(self,'String>>at: code point is lone surrogate') }, { asCharacter(codePoint) }) }) }, { ifAbsent() }) }",
);
