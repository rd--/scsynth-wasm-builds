...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _base16Encoded_1(_asciiByteArray_1(_self));
  }, ["self"]),
  "{ :self | base16Encoded(asciiByteArray(self)) }",
);
    ...
    return _lexicographicSort_1(
      _collect_2(
        ...
        sl.annotateFunction(function (_each) {
          ...
          return _rotatedLeft_2(_self, _each);
        }, ["each"]),
      ),
    );
  }, ["self"]),
  "{ :self | lexicographicSort(collect(upOrDownTo(1, size(self)), { :each | rotatedLeft(self,each) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _delimiters) {
    ...
    let __SPL466 = _assertIsOfSize_2(_delimiters, 2);
    let _p = _at_2(__SPL466, 1);
    let _q = _at_2(__SPL466, 2);
  ...
  "{ :self :delimiters | let __SPL466 = assertIsOfSize(delimiters, 2); let p = at(__SPL466, 1); let q = at(__SPL466, 2); let c = ++(++([p], characters(self)), [q]); stringJoin(burrowsWheelerTransform(c)) }",
);
