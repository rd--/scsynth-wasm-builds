...
sl.addMethodToExistingType(
  ...
  "{ :self | let height = at(metadata(self), 'height'); let actualBoundingBox = asRectangle(boundingBox(self)); let boundingBox = if(isZero(height(actualBoundingBox)), { Rectangle(lowerLeft(actualBoundingBox), [right(actualBoundingBox), +(lower(actualBoundingBox), 1)]) }, { actualBoundingBox }); let yRange = height(boundingBox); let precision = max((-(3, round(log10(yRange)))),0); let scaleFactor = (/(height, height(boundingBox))); let scaledBoundingBox = Rectangle(*(lowerLeft(boundingBox), scaleFactor), *(upperRight(boundingBox), scaleFactor)); let options = Record([['precision', precision], ['scaleFactor', scaleFactor]]); let fragments = collect(contents(self), { :each | svgFragment(each,options) }); let strokeWith = (/(0.5, scaleFactor)); let yTranslation = +(height(scaledBoundingBox), (*(2, at(lowerLeft(scaledBoundingBox), 2)))); Svg(unlines(flatten([format('<svg xmlns=\"%\" width=\"%\" height=\"%\" viewBox=\"%\">',['http://www.w3.org/2000/svg', printStringToFixed(width(scaledBoundingBox),1), printStringToFixed(height(scaledBoundingBox),1), asSvgViewBox(scaledBoundingBox, Record([['margin', 5], ['precision', precision]]))]), format('<g fill=\"none\" stroke=\"black\" stroke-width=\"%%\" transform=\"translate(0, %) scale(%, %)\">',[printStringToFixed(strokeWith,4), '%', printStringToFixed(yTranslation,4), printStringToFixed(scaleFactor,4), printStringToFixed(negate(scaleFactor),4)]), fragments, '</g>', '</svg>']))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL347 = _assertIsOfSize_2(_shape_1(_self), 2);
        let _m = _at_2(__SPL347, 1);
        let _n = _at_2(__SPL347, 2);
        /* Statements */
        return _if_3(
          _equalsSign_2(_n, 2),
          sl.annotateFunction(function () {
  ...
  "{ :self | if(isMatrix(self), { let __SPL347 = assertIsOfSize(shape(self), 2); let m = at(__SPL347, 1); let n = at(__SPL347, 2); if((=(n, 2)), { asLineDrawing(Line(self)) }, { error(self,'asLineDrawing') }) }, { LineDrawing(self) }) }",
);
