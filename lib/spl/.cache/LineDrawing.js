...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    return _if_3(_isMatrix_1(_self), function () {
      ...
      let __SPL154 = _assertIsOfSize_2(_shape_1(_self), 2);
      let _m = _at_2(__SPL154, 1);
      let _n = _at_2(__SPL154, 2);
      /* Statements */
  ...
  "{ :self | if(isMatrix(self), { let __SPL154 = assertIsOfSize(shape(self), 2); let m = at(__SPL154, 1); let n = at(__SPL154, 2); if((=(n, 2)), { asLineDrawing(Line(self)) }, { error(self,'asLineDrawing') }) }, { LineDrawing(self) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let radius = 1; let circleCount = +(size(self), 1); let period = sum(anyOne(self)); let innerCircle = [period]; let places = collect(concatenation([[innerCircle], self]),prefixSum:/1); let angles = collect(places, { :p | collect(p, { :q | %((+(*(negated((/(q, period))), pi(2)), pi(0.5))), pi(2)) }) }); let circles = collect(upOrDownTo(1, circleCount), { :each | Circle([0, 0], *(each, radius)) }); let lines = collect(upOrDownTo(1, circleCount), { :each | let innerRadius = *(-(each, 1), radius); let outerRadius = *(each, radius); collect(at(angles, each), { :theta | Line(fromPolarCoordinates([[innerRadius, theta], [outerRadius, theta]])) }) }); LineDrawing([circles, lines]) }",
);
