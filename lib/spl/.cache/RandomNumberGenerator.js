...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_self, _range, _shape) {
    ...
    let __SPL448 = _assertIsOfSize_2(_range, 2);
    let _min = _at_2(__SPL448, 1);
    let _max = _at_2(__SPL448, 2);
    /* Statements */
    return _Complex_2(
  ...
  "{ :self :range :shape | let __SPL448 = assertIsOfSize(range, 2); let min = at(__SPL448, 1); let max = at(__SPL448, 2); Complex(randomReal(self,[real(min), real(max)], shape), randomReal(self,[imaginary(min), imaginary(max)], shape)) }",
);
