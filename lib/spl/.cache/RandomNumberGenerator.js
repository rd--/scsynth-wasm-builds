...
sl.addMethodToExistingTrait(
  ...
  sl.annotateFunction(function (_self, _range, _shape) {
    ...
    let __SPL541 = _assertIsOfSize_2(_range, 2);
    let _min = _at_2(__SPL541, 1);
    let _max = _at_2(__SPL541, 2);
    /* Statements */
    return _Complex_2(
  ...
  "{ :self :range :shape | let __SPL541 = assertIsOfSize(range, 2); let min = at(__SPL541, 1); let max = at(__SPL541, 2); Complex(randomReal(self,[real(min), real(max)], shape), randomReal(self,[imaginary(min), imaginary(max)], shape)) }",
);
...
sl.addMethodToExistingTrait(
  ...
  "{ :self :s :k | let n = size(s); let r = copyFromTo(s,1, k); let w = exp((/(log(nextRandomFloat(self)), k))); let i = +(k, 1); whileTrue({ <=(i, n) }, { i := +(+(i, floor((/(log(nextRandomFloat(self)), log((-(1, w))))))), 1); ifTrue((<=(i, n)), { atPut(r, nextRandomInteger(self,1, k), at(s, i)); w := *(w, exp((/(log(nextRandomFloat(self)), k)))) }) }); r }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :max | randomInteger(system,[negate(max), max], []) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :max | randomReal(system,[negate(max), max], []) }",
);
