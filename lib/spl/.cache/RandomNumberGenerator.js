...
sl.addMethodToExistingTrait(
  ...
  "{ :self :range :shape | let __SplVar1 = assertIsOfSize(range, 2); let min = at(__SplVar1, 1); let max = at(__SplVar1, 2); Complex(randomReal(self,[real(min), real(max)], shape), randomReal(self,[imaginary(min), imaginary(max)], shape)) }",
);
