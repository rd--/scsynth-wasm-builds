...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _options) {
    ...
    let __SPL297 = _assertIsOfSize_2(_lowerLeft_1(_self), 2);
    let _x = _at_2(__SPL297, 1);
    let _y = _at_2(__SPL297, 2);
    /* Statements */
    return [
  ...
  '{ :self :options | let precision = at(options, \'precision\'); let __SPL297 = assertIsOfSize(lowerLeft(self), 2); let x = at(__SPL297, 1); let y = at(__SPL297, 2); [format(\'<g x="%" y="%" transform="translate(%, %) scale(1, -1)">\',[printStringToFixed(x,precision), printStringToFixed(y,precision), printStringToFixed(x,precision), printStringToFixed(y,precision)]), format(\'<text fill="black" stroke="none">%</text>\',[contents(self)]), \'</g>\'] }',
);
