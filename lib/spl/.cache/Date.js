...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject :aBlock:/2 | &(isDate(anObject), { aBlock(absoluteTime(self), absoluteTime(anObject)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL1 = assertIsOfSize(self, 3); let year = at(__SPL1, 1); let month = at(__SPL1, 2); let dayOfMonth = at(__SPL1, 3); Date(year, month, dayOfMonth) }",
);
