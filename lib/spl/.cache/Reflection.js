...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _signature) {
    ...
    let __SPL558 = _assertIsOfSize_2(_parseMethodSignature_1(_signature), 2);
    let _qualifiedOriginName = _at_2(__SPL558, 1);
    let _qualifiedMethodName = _at_2(__SPL558, 2);
    /* Statements */
    return _if_3(
  ...
  "{ :self :signature | let __SPL558 = assertIsOfSize(parseMethodSignature(signature), 2); let qualifiedOriginName = at(__SPL558, 1); let qualifiedMethodName = at(__SPL558, 2); if(isQualifiedTraitName(qualifiedOriginName), { at(methodDictionary(at(traitDictionary(self), parseQualifiedTraitName(qualifiedOriginName))), qualifiedMethodName) }, { at(methodDictionary(at(typeDictionary(self), qualifiedOriginName)), qualifiedMethodName) }) }",
);
