...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _h, _k, _t, _n) {
    ...
    let __SPL443 = _assertIsOfSize_2(_t, 3);
    let _tMin = _at_2(__SPL443, 1);
    let _tMax = _at_2(__SPL443, 2);
    let _dt = _at_2(__SPL443, 3);
  ...
  "{ :self :h :k :t :n | let __SPL443 = assertIsOfSize(t, 3); let tMin = at(__SPL443, 1); let tMax = at(__SPL443, 2); let dt = at(__SPL443, 3); let timeList = asList(thenTo(tMin, +(tMin, dt), tMax)); let l = size(timeList); let m = log(nextPowerOfTwo(l),2); TemporalData(!({ transposed([timeList, take(hoskingMethod(self,-(tMax, tMin), m, h, k),l)]) }, n)) }",
);
