...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _vector) {
    ...
    let __SPL214 = _assertIsOfSize_2(_elements_1(_self), 9);
    let _a = _at_2(__SPL214, 1);
    let _b = _at_2(__SPL214, 2);
    let _c = _at_2(__SPL214, 3);
    let _d = _at_2(__SPL214, 4);
    let _e = _at_2(__SPL214, 5);
    let _f = _at_2(__SPL214, 6);
    let _g = _at_2(__SPL214, 7);
    let _h = _at_2(__SPL214, 8);
    let _i = _at_2(__SPL214, 9);
    let __SPL215 = _assertIsOfSize_2(_asList_1(_vector), 3);
    let _x = _at_2(__SPL215, 1);
    let _y = _at_2(__SPL215, 2);
    let _z = _at_2(__SPL215, 3);
    /* Statements */
  ...
  "{ :self :vector | let __SPL214 = assertIsOfSize(elements(self), 9); let a = at(__SPL214, 1); let b = at(__SPL214, 2); let c = at(__SPL214, 3); let d = at(__SPL214, 4); let e = at(__SPL214, 5); let f = at(__SPL214, 6); let g = at(__SPL214, 7); let h = at(__SPL214, 8); let i = at(__SPL214, 9); let __SPL215 = assertIsOfSize(asList(vector), 3); let x = at(__SPL215, 1); let y = at(__SPL215, 2); let z = at(__SPL215, 3); CartesianCoordinates(+(+((*(a, x)), (*(b, y))), (*(c, z))), +(+((*(d, x)), (*(e, y))), (*(f, z))), +(+((*(g, x)), (*(h, y))), (*(i, z)))) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL216 = _assertIsOfSize_2(_elements_1(_self), 9);
    let _a = _at_2(__SPL216, 1);
    let _b = _at_2(__SPL216, 2);
    let _c = _at_2(__SPL216, 3);
    let _d = _at_2(__SPL216, 4);
    let _e = _at_2(__SPL216, 5);
    let _f = _at_2(__SPL216, 6);
    let _g = _at_2(__SPL216, 7);
    let _h = _at_2(__SPL216, 8);
    let _i = _at_2(__SPL216, 9);
    /* Statements */
  ...
  "{ :self | let __SPL216 = assertIsOfSize(elements(self), 9); let a = at(__SPL216, 1); let b = at(__SPL216, 2); let c = at(__SPL216, 3); let d = at(__SPL216, 4); let e = at(__SPL216, 5); let f = at(__SPL216, 6); let g = at(__SPL216, 7); let h = at(__SPL216, 8); let i = at(__SPL216, 9); -(-(-(+(+((*(*(a, e), i)), (*(*(b, f), g))), (*(*(c, d), h))), (*(*(c, e), g))), (*(*(b, d), i))), (*(*(a, f), h))) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SPL217 = assertIsOfSize(elements(self), 9); let a = at(__SPL217, 1); let b = at(__SPL217, 2); let c = at(__SPL217, 3); let d = at(__SPL217, 4); let e = at(__SPL217, 5); let f = at(__SPL217, 6); let g = at(__SPL217, 7); let h = at(__SPL217, 8); let i = at(__SPL217, 9); elements(self, *(determinant(self), [-((*(e, i)), (*(f, h))), negate((-((*(b, i)), (*(c, h))))), -((*(b, f)), (*(c, e))), negate((-((*(d, i)), (*(f, g))))), -((*(a, i)), (*(c, g))), negate((-((*(a, f)), (*(c, d))))), -((*(d, h)), (*(e, g))), (-(negate((*(a, h))), (*(b, g)))), -((*(a, e)), (*(b, d)))])); self }",
);
