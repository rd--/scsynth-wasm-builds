_primitiveReadLocalPackages_1([
  "Behaviour-Adverb",
  "Behaviour-Apl",
  "Behaviour-Block",
  "Behaviour-HashFunction",
  "Behaviour-Piecewise",
  "Behaviour-Prototype",
  "Behaviour-Thunk",
  "Benchmark-Benchmark",
  "Benchmark-BounceBenchmark",
  "Benchmark-HavlakBenchmark",
  "Benchmark-JsonBenchmark",
  "Benchmark-ListBenchmark",
  "Benchmark-MandelbrotBenchmark",
  "Benchmark-NBodyBenchmark",
  "Benchmark-PermuteBenchmark",
  "Benchmark-QueensBenchmark",
  "Benchmark-SieveBenchmark",
  "Benchmark-SomRandom",
  "Benchmark-SqueakBenchmark",
  "Benchmark-StorageBenchmark",
  "Benchmark-TowersBenchmark",
  "Chemistry-CrystalStructure",
  "Collection-ArithmeticProgression",
  "Collection-Array",
  "Collection-ArrayBuffer",
  "Collection-Association",
  "Collection-AssociationList",
  "Collection-BitSet",
  "Collection-Box",
  "Collection-ByteArray",
  "Collection-Cache",
  "Collection-CategoryDictionary",
  "Collection-Collection",
  "Collection-Cons",
  "Collection-CopyableSequence",
  "Collection-Dictionary",
  "Collection-DirectedEdge",
  "Collection-Extensible",
  "Collection-FiniteWord",
  "Collection-Float32Array",
  "Collection-Float64Array",
  "Collection-Graph",
  "Collection-Heap",
  "Collection-Histogram",
  "Collection-HuffmanCoding",
  "Collection-IdentityMultiset",
  "Collection-IdentitySet",
  "Collection-Indexable",
  "Collection-Iterable",
  "Collection-LinkedList",
  "Collection-List",
  "Collection-ListView",
  "Collection-Map",
  "Collection-Matrix",
  "Collection-MemoizationTable",
  "Collection-MergeSort",
  "Collection-Multiset",
  "Collection-NumericArray",
  "Collection-Ordered",
  "Collection-PatienceSort",
  "Collection-PrimitiveArray",
  "Collection-PrimitiveMap",
  "Collection-PriorityQueue",
  "Collection-QuickSort",
  "Collection-Range",
  "Collection-Record",
  "Collection-RelativeRange",
  "Collection-Removable",
  "Collection-ResidueSet",
  "Collection-RunArray",
  "Collection-SequenceAlignment",
  "Collection-Sequenceable",
  "Collection-Set",
  "Collection-Sort",
  "Collection-SortedList",
  "Collection-SortedSet",
  "Collection-SoundFile",
  "Collection-SparseArray",
  "Collection-Stack",
  "Collection-Table",
  "Collection-TemporalData",
  "Collection-TimeSeries",
  "Collection-Tree",
  "Collection-Tuple",
  "Collection-TypedDictionary",
  "Collection-UndirectedEdge",
  "Collection-Unordered",
  "Collection-UnsortedSet",
  "Collection-WeakMap",
  "Collection-WeightedData",
  "Exception-Error",
  "Exception-Missing",
  "Geometry-Annulus",
  "Geometry-Arc",
  "Geometry-AxonometricProjection",
  "Geometry-Ball",
  "Geometry-BarycentricCoordinates",
  "Geometry-BezierCurve",
  "Geometry-BipolarCoordinates",
  "Geometry-CartesianCoordinates",
  "Geometry-Cartography",
  "Geometry-Circle",
  "Geometry-Cone",
  "Geometry-Contour",
  "Geometry-ConvexHull",
  "Geometry-CylindricalCoordinates",
  "Geometry-DelaunayTriangulation",
  "Geometry-Disk",
  "Geometry-DoyleSpiral",
  "Geometry-Ellipse",
  "Geometry-FractalCurve",
  "Geometry-GeneralisedCircle",
  "Geometry-GeodeticCoordinates",
  "Geometry-Geometry",
  "Geometry-GeometryCollection",
  "Geometry-HalfPlane",
  "Geometry-Heightmap",
  "Geometry-HexagonalCoordinates",
  "Geometry-HexagonalEfficientCoordinates",
  "Geometry-InfiniteLine",
  "Geometry-InfinitePlane",
  "Geometry-Line",
  "Geometry-LineSegment",
  "Geometry-Matrix22",
  "Geometry-Matrix33",
  "Geometry-Parallelepiped",
  "Geometry-Parallelogram",
  "Geometry-Perspective",
  "Geometry-PlanarCoordinates",
  "Geometry-Plane",
  "Geometry-Point",
  "Geometry-PointCloud",
  "Geometry-PolarCoordinates",
  "Geometry-Polygon",
  "Geometry-PolygonMesh",
  "Geometry-PolygonWithHoles",
  "Geometry-Polyhedron",
  "Geometry-Rectangle",
  "Geometry-Simplex",
  "Geometry-Sphere",
  "Geometry-SphericalCoordinates",
  "Geometry-Spiral",
  "Geometry-StadiumShape",
  "Geometry-Superellipse",
  "Geometry-Tetrahedron",
  "Geometry-Triangle",
  "Geometry-TurtleGeometry",
  "Geometry-WeavingInformationFile",
  "Graphics-Bitmap",
  "Graphics-Colour",
  "Graphics-ColourGradient",
  "Graphics-ColourGrid",
  "Graphics-ColourPalette",
  "Graphics-Greymap",
  "Graphics-Image",
  "Graphics-LineDrawing",
  "Graphics-NaturalColourSystem",
  "Graphics-PerspectiveDrawing",
  "Graphics-Plot",
  "Graphics-PlotSet",
  "Graphics-Svg",
  "Graphics-Writing",
  "Help-DocumentationTest",
  "Help-HelpFile",
  "Help-HelpIndex",
  "Help-HelpProgram",
  "Help-TerseGuide",
  "Help-TerseReference",
  "Kernel-Comparable",
  "Kernel-Copyable",
  "Kernel-Equatable",
  "Kernel-Nil",
  "Kernel-Object",
  "Kernel-Storeable",
  "Logic-Boolean",
  "Logic-SymbolicExpression",
  "Math-ApproximateMathFunctions",
  "Math-Bessel",
  "Math-Binomial",
  "Math-BivariatePolynomial",
  "Math-ChordDiagram",
  "Math-Combinatorics",
  "Math-DifferentialEquations",
  "Math-DistanceFunctions",
  "Math-DyckWord",
  "Math-EasingFunctions",
  "Math-Elementwise",
  "Math-FeatureDetection",
  "Math-FibonacciNumbers",
  "Math-FiniteAutomata",
  "Math-Fourier",
  "Math-Gamma",
  "Math-GrayCode",
  "Math-GroupTheory",
  "Math-HyperbolicFunctions",
  "Math-IntegerPartition",
  "Math-IntegerSequence",
  "Math-Integrals",
  "Math-Interpolation",
  "Math-IterativeMaps",
  "Math-LowDiscrepancySequence",
  "Math-MathematicalConstant",
  "Math-MatrixFunctions",
  "Math-MixedRadix",
  "Math-ModularArithmetic",
  "Math-Nim",
  "Math-NumericalFunctions",
  "Math-NumericalFunctions",
  "Math-OnlineEncyclopediaOfIntegerSequences",
  "Math-OptimizationFunctions",
  "Math-ParametricEquations",
  "Math-Permutations",
  "Math-Primes",
  "Math-Shuffle",
  "Math-SignalProcessing",
  "Math-Siteswap",
  "Math-SpecialFunctions",
  "Math-Statistics",
  "Math-TrigonometricFunctions",
  "Math-UnivariatePolynomial",
  "Math-VectorFunctions",
  "Math-WindowFunction",
  "Meta-BenchmarkLibrary",
  "Meta-Categories",
  "Meta-Kernel",
  "Meta-SmallKansasLibrary",
  "Meta-StandardLibrary",
  "Meta-SuperColliderLibrary",
  "Music-AtonalTheory",
  "Music-CentsTuning",
  "Music-Meter",
  "Music-Pitch",
  "Music-RatioTuning",
  "Music-ScalaTuning",
  "Music-Scale",
  "Music-Tuning",
  "Music-TuningLattice",
  "Music-Xenharmonic",
  "Navigator-DocumentObjectModel",
  "Navigator-NavigatorMidi",
  "Navigator-NavigatorSvg",
  "Number-Binary",
  "Number-Complex",
  "Number-Decimal",
  "Number-EisensteinInteger",
  "Number-Fraction",
  "Number-Integer",
  "Number-Interval",
  "Number-LargeInteger",
  "Number-Magnitude",
  "Number-Number",
  "Number-Quaternion",
  "Number-Residue",
  "Number-SmallFloat",
  "ProbabilityDistribution-ArcSinDistribution",
  "ProbabilityDistribution-BernoulliDistribution",
  "ProbabilityDistribution-BetaBinomialDistribution",
  "ProbabilityDistribution-BetaDistribution",
  "ProbabilityDistribution-BinomialDistribution",
  "ProbabilityDistribution-CauchyDistribution",
  "ProbabilityDistribution-DirichletDistribution",
  "ProbabilityDistribution-DiscreteUniformDistribution",
  "ProbabilityDistribution-EmpiricalDistribution",
  "ProbabilityDistribution-EpanechnikovDistribution",
  "ProbabilityDistribution-ExponentialDistribution",
  "ProbabilityDistribution-ExtremeValueDistribution",
  "ProbabilityDistribution-FRatioDistribution",
  "ProbabilityDistribution-GammaDistribution",
  "ProbabilityDistribution-GeometricDistribution",
  "ProbabilityDistribution-HypergeometricDistribution",
  "ProbabilityDistribution-HyperbolicDistribution",
  "ProbabilityDistribution-InverseGaussianDistribution",
  "ProbabilityDistribution-LaplaceDistribution",
  "ProbabilityDistribution-LevyDistribution",
  "ProbabilityDistribution-LogNormalDistribution",
  "ProbabilityDistribution-LogisticDistribution",
  "ProbabilityDistribution-MeixnerDistribution",
  "ProbabilityDistribution-MultinormalDistribution",
  "ProbabilityDistribution-MultivariateTDistribution",
  "ProbabilityDistribution-NoncentralBetaDistribution",
  "ProbabilityDistribution-NoncentralChiSquareDistribution",
  "ProbabilityDistribution-NormalDistribution",
  "ProbabilityDistribution-ParetoDistribution",
  "ProbabilityDistribution-PoissonDistribution",
  "ProbabilityDistribution-ProbabilityDistribution",
  "ProbabilityDistribution-ReciprocalDistribution",
  "ProbabilityDistribution-SechDistribution",
  "ProbabilityDistribution-SkewNormalDistribution",
  "ProbabilityDistribution-StableDistribution",
  "ProbabilityDistribution-StudentTDistribution",
  "ProbabilityDistribution-TriangularDistribution",
  "ProbabilityDistribution-UniformDistribution",
  "ProbabilityDistribution-WeibullDistribution",
  "Protocol-GeoJson",
  "Protocol-Json",
  "Protocol-OpenSoundControl",
  "Protocol-Schema",
  "Quantity-Frequency",
  "Quantity-Length",
  "Quantity-Mass",
  "Quantity-PlaneAngle",
  "Quantity-Quantity",
  "Quantity-QuantityArray",
  "Quantity-SiPrefix",
  "Quantity-SiUnit",
  "Quantity-Time",
  "Random-AliasMethod",
  "Random-LaggedFibonacci",
  "Random-LinearCongruential",
  "Random-MersenneTwister",
  "Random-PoissonDiskSampling",
  "Random-RandomFixedSum",
  "Random-RandomNumberGenerator",
  "Random-Sfc32",
  "Random-SplitMix",
  "RandomProcess-BernoulliProcess",
  "RandomProcess-BinomialProcess",
  "RandomProcess-BrownianBridgeProcess",
  "RandomProcess-ContinuousMarkovProcess",
  "RandomProcess-CoxIngersollRossProcess",
  "RandomProcess-DiscreteMarkovProcess",
  "RandomProcess-FractionalBrownianMotionProcess",
  "RandomProcess-FractionalGaussianNoiseProcess",
  "RandomProcess-GeometricBrownianMotionProcess",
  "RandomProcess-HiddenMarkovProcess",
  "RandomProcess-OrnsteinUhlenbeckProcess",
  "RandomProcess-PoissonProcess",
  "RandomProcess-RandomProcess",
  "RandomProcess-RandomWalkProcess",
  "RandomProcess-TelegraphProcess",
  "RandomProcess-WienerProcess",
  "Scheduling-Promise",
  "SmallKansas-AnalogueClock",
  "SmallKansas-CategoryBrowser",
  "SmallKansas-ColourChooser",
  "SmallKansas-ColumnBrowser",
  "SmallKansas-CrystalStructureBrowser",
  "SmallKansas-CrystalStructureOracle",
  "SmallKansas-DigitalClock",
  "SmallKansas-Frame",
  "SmallKansas-HelpBrowser",
  "SmallKansas-HelpFileViewer",
  "SmallKansas-HelpTextViewer",
  "SmallKansas-ImageViewer",
  "SmallKansas-Inspector",
  "SmallKansas-ListChooser",
  "SmallKansas-Menu",
  "SmallKansas-MethodBrowser",
  "SmallKansas-MethodSignatureBrowser",
  "SmallKansas-MidiMonitorMenu",
  "SmallKansas-MidiPortBrowser",
  "SmallKansas-Notebook",
  "SmallKansas-PackageBrowser",
  "SmallKansas-PackageIndexBrowser",
  "SmallKansas-PngViewer",
  "SmallKansas-ScalaJiMetaBrowser",
  "SmallKansas-ScalaJiTuningBrowser",
  "SmallKansas-ScalaScaleBrowser",
  "SmallKansas-SmallKansas",
  "SmallKansas-SmallProgram",
  "SmallKansas-SuperColliderProgramBrowser",
  "SmallKansas-SuperColliderProgramOracle",
  "SmallKansas-SuperColliderSynthesiserStatusMonitor",
  "SmallKansas-SvgViewer",
  "SmallKansas-SystemBrowser",
  "SmallKansas-TableViewer",
  "SmallKansas-TextEditor",
  "SmallKansas-TextElement",
  "SmallKansas-TraitBrowser",
  "SmallKansas-TranscriptViewer",
  "SmallKansas-TypeBrowser",
  "SmallKansas-UserEventTarget",
  "SmallKansas-View",
  "SmallKansas-WindowMenu",
  "SmallKansas-Workspace",
  "SmallKansas-WorldMenu",
  "Sound-Wavetable",
  "Stream-AsciiStream",
  "Stream-BlockStream",
  "Stream-CollectionStream",
  "Stream-Iterator",
  "Stream-MutableCollectionStream",
  "Stream-PositionableStream",
  "Stream-Stream",
  "Stream-Utf8Stream",
  "Stream-WriteStream",
  "SuperCollider-ContinuousEvent",
  "SuperCollider-Env",
  "SuperCollider-J",
  "SuperCollider-KeywordUgens",
  "SuperCollider-LocalControl",
  "SuperCollider-Ls",
  "SuperCollider-Mix",
  "SuperCollider-PseudoUgens",
  "SuperCollider-ScLang",
  "SuperCollider-ScProgram",
  "SuperCollider-ScSynth",
  "SuperCollider-ScSynthOptions",
  "SuperCollider-ScSynthStatus",
  "SuperCollider-ScUgen",
  "SuperCollider-Texture",
  "SuperCollider-Ugen",
  "SuperCollider-UgenBindings",
  "SuperCollider-UgenGraph",
  "System-BinaryLargeObject",
  "System-CacheStorage",
  "System-Event",
  "System-EventTarget",
  "System-File",
  "System-FilePath",
  "System-FileSystem",
  "System-Headers",
  "System-LibraryItem",
  "System-Location",
  "System-MessageEvent",
  "System-Meta",
  "System-Method",
  "System-Navigator",
  "System-Package",
  "System-ReadableStream",
  "System-Reflection",
  "System-Request",
  "System-Response",
  "System-Scheduling",
  "System-Storage",
  "System-SubProcess",
  "System-System",
  "System-Trait",
  "System-Transcript",
  "System-Type",
  "System-Url",
  "System-UrlQueryParameters",
  "System-WebSocket",
  "System-Window",
  "Text-AsciiString",
  "Text-Character",
  "Text-Html",
  "Text-Markdown",
  "Text-QuotedString",
  "Text-RegularExpression",
  "Text-String",
  "Text-Unicode",
  "Time-CalendarDuration",
  "Time-Clock",
  "Time-Date",
  "Time-DateAndTime",
  "Time-Duration",
  "Time-TimeInterval",
  "Time-TimeStamp",
]);
