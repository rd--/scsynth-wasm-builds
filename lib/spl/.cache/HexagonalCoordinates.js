...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _operand) {
    ...
    let _q1 = _at_2(__SPL4, 1);
    let _r1 = _at_2(__SPL4, 2);
    let __genSym10 = _at_2(__SPL4, 3);
    let __SPL5 = _assertIsOfSize_2(_coordinates_1(_operand), 3);
    let _q2 = _at_2(__SPL5, 1);
    let _r2 = _at_2(__SPL5, 2);
    let __genSym11 = _at_2(__SPL5, 3);
    /* Statements */
    return _sqrt_1(
      _plusSign_2(
        _plusSign_2(
  ...
  "{ :self :operand | let __SPL4 = assertIsOfSize(coordinates(self), 3); let q1 = at(__SPL4, 1); let r1 = at(__SPL4, 2); let _ = at(__SPL4, 3); let __SPL5 = assertIsOfSize(coordinates(operand), 3); let q2 = at(__SPL5, 1); let r2 = at(__SPL5, 2); let _ = at(__SPL5, 3); sqrt((+(+(square((-(q2, q1))), square((-(r2, r1)))), (*((-(q2, q1)), (-(r2, r1))))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _answer = [];
    let __SPL6 = _assertIsOfSize_2(_at_2(_b, 1), 2);
    let _x1 = _at_2(__SPL6, 1);
    let _y1 = _at_2(__SPL6, 2);
    let __SPL7 = _assertIsOfSize_2(_at_2(_b, 2), 2);
  ...
  "{ :self | let p = Polygon(self); let b = coordinateBoundingBox(self); let answer = []; let __SPL6 = assertIsOfSize(at(b, 1), 2); let x1 = at(__SPL6, 1); let y1 = at(__SPL6, 2); let __SPL7 = assertIsOfSize(at(b, 2), 2); let x2 = at(__SPL7, 1); let y2 = at(__SPL7, 2); let m = /(1, sqrt(3)); let k = 3; toDo((-(y1, k)), +(y2, k), { :r | let rOffset = floor((/(r, 2))); toDo((-(-(x1, rOffset), k)), +(-(x2, rOffset), k), { :q | let s = -(-(0, q), r); let c = *(fromHexagonalCoordinates([q, r, s]), 0.5); let h = regularPolygon(6,c, 0.5, pi(0.5)); ifTrue(anySatisfy(vertexCoordinates(h), { :each | containsPoint(p,each) }), { add(answer,h) }) }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let _q = _at_2(__SPL8, 1);
        let _r = _at_2(__SPL8, 2);
        let _s = _at_2(__SPL8, 3);
        let _qR = _round_1(_q);
        let _rR = _round_1(_r);
        let _sR = _round_1(_s);
  ...
  "{ :self | if(isVector(self), { let __SPL8 = assertIsOfSize(self, 3); let q = at(__SPL8, 1); let r = at(__SPL8, 2); let s = at(__SPL8, 3); let qR = round(q); let rR = round(r); let sR = round(s); let qD = abs((-(qR, q))); let rD = abs((-(rR, r))); let sD = abs((-(sR, s))); if((&(>(qD, rD), { >(qD, sD) })), { qD := -(-(0, rD), sD) }, { if((>(rD, sD)), { rD := -(-(0, qD), sD) }, { sD := -(-(0, qD), rD) }) }); [qR, rR, sR] }, { collect(self,roundedHexagonalCoordinates:/1) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _atVectorOrElementwise_2(
      ...
      sl.annotateFunction(function (_v) {
        ...
        let __SPL9 = _assertIsOfSize_2(_v, 2);
  ...
  "{ :self | atVectorOrElementwise(self, { :v | let __SPL9 = assertIsOfSize(v, 2); let x = at(__SPL9, 1); let y = at(__SPL9, 2); let m = sqrt(3); let q = -((*(/(m, 3), x)), (*(/(1, 3), y))); let r = *(/(2, 3), y); let s = -(-(0, q), r); [q, r, s] }) }",
);
