...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _operand) {
    ...
    let __SPL131 = _assertIsOfSize_2(_coordinates_1(_self), 3);
    let _q1 = _at_2(__SPL131, 1);
    let _r1 = _at_2(__SPL131, 2);
    let __genSym137 = _at_2(__SPL131, 3);
    let __SPL132 = _assertIsOfSize_2(_coordinates_1(_operand), 3);
    let _q2 = _at_2(__SPL132, 1);
    let _r2 = _at_2(__SPL132, 2);
    let __genSym138 = _at_2(__SPL132, 3);
    /* Statements */
    return _sqrt_1(
      _plusSign_2(
        _plusSign_2(
  ...
  "{ :self :operand | let __SPL131 = assertIsOfSize(coordinates(self), 3); let q1 = at(__SPL131, 1); let r1 = at(__SPL131, 2); let _ = at(__SPL131, 3); let __SPL132 = assertIsOfSize(coordinates(operand), 3); let q2 = at(__SPL132, 1); let r2 = at(__SPL132, 2); let _ = at(__SPL132, 3); sqrt((+(+(squared((-(q2, q1))), squared((-(r2, r1)))), (*((-(q2, q1)), (-(r2, r1))))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _answer = [];
    let __SPL133 = _assertIsOfSize_2(_at_2(_b, 1), 2);
    let _x1 = _at_2(__SPL133, 1);
    let _y1 = _at_2(__SPL133, 2);
    let __SPL134 = _assertIsOfSize_2(_at_2(_b, 2), 2);
    let _x2 = _at_2(__SPL134, 1);
    let _y2 = _at_2(__SPL134, 2);
  ...
  "{ :self | let p = Polygon(self); let b = coordinateBoundingBox(self); let answer = []; let __SPL133 = assertIsOfSize(at(b, 1), 2); let x1 = at(__SPL133, 1); let y1 = at(__SPL133, 2); let __SPL134 = assertIsOfSize(at(b, 2), 2); let x2 = at(__SPL134, 1); let y2 = at(__SPL134, 2); let m = /(1, sqrt(3)); let k = 3; toDo((-(y1, k)), +(y2, k), { :r | let rOffset = floor((/(r, 2))); toDo((-(-(x1, rOffset), k)), +(-(x2, rOffset), k), { :q | let s = -(-(0, q), r); let c = *(fromHexagonalCoordinates([q, r, s]), 0.5); let h = regularPolygon(6,c, 0.5, pi(0.5)); ifTrue(anySatisfy(vertexCoordinates(h), { :each | containsPoint(p,each) }), { add(answer,h) }) }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL135 = _assertIsOfSize_2(_self, 3);
        let _q = _at_2(__SPL135, 1);
        let _r = _at_2(__SPL135, 2);
        let _s = _at_2(__SPL135, 3);
  ...
  "{ :self | if(isVector(self), { let __SPL135 = assertIsOfSize(self, 3); let q = at(__SPL135, 1); let r = at(__SPL135, 2); let s = at(__SPL135, 3); let qR = rounded(q); let rR = rounded(r); let sR = rounded(s); let qD = abs((-(qR, q))); let rD = abs((-(rR, r))); let sD = abs((-(sR, s))); if((&(>(qD, rD), { >(qD, sD) })), { qD := -(-(0, rD), sD) }, { if((>(rD, sD)), { rD := -(-(0, qD), sD) }, { sD := -(-(0, qD), rD) }) }); [qR, rR, sR] }, { collect(self,roundedHexagonalCoordinates:/1) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _atVectorOrElementwise_2(
      ...
      sl.annotateFunction(function (_v) {
        ...
        let __SPL136 = _assertIsOfSize_2(_v, 2);
        let _x = _at_2(__SPL136, 1);
        let _y = _at_2(__SPL136, 2);
        let _m = _sqrt_1(3);
  ...
  "{ :self | atVectorOrElementwise(self, { :v | let __SPL136 = assertIsOfSize(v, 2); let x = at(__SPL136, 1); let y = at(__SPL136, 2); let m = sqrt(3); let q = -((*(/(m, 3), x)), (*(/(1, 3), y))); let r = *(/(2, 3), y); let s = -(-(0, q), r); [q, r, s] }) }",
);
