...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _operand) {
    ...
    let _q1 = _at_2(__SPL165, 1);
    let _r1 = _at_2(__SPL165, 2);
    let __genSym171 = _at_2(__SPL165, 3);
    let __SPL166 = _assertIsOfSize_2(_coordinates_1(_operand), 3);
    let _q2 = _at_2(__SPL166, 1);
    let _r2 = _at_2(__SPL166, 2);
    let __genSym172 = _at_2(__SPL166, 3);
    /* Statements */
    return _sqrt_1(
      _plusSign_2(
        _plusSign_2(
  ...
  "{ :self :operand | let __SPL165 = assertIsOfSize(coordinates(self), 3); let q1 = at(__SPL165, 1); let r1 = at(__SPL165, 2); let _ = at(__SPL165, 3); let __SPL166 = assertIsOfSize(coordinates(operand), 3); let q2 = at(__SPL166, 1); let r2 = at(__SPL166, 2); let _ = at(__SPL166, 3); sqrt((+(+(square((-(q2, q1))), square((-(r2, r1)))), (*((-(q2, q1)), (-(r2, r1))))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _answer = [];
    let __SPL167 = _assertIsOfSize_2(_at_2(_b, 1), 2);
    let _x1 = _at_2(__SPL167, 1);
    let _y1 = _at_2(__SPL167, 2);
    let __SPL168 = _assertIsOfSize_2(_at_2(_b, 2), 2);
    let _x2 = _at_2(__SPL168, 1);
    let _y2 = _at_2(__SPL168, 2);
  ...
  "{ :self | let p = Polygon(self); let b = coordinateBoundingBox(self); let answer = []; let __SPL167 = assertIsOfSize(at(b, 1), 2); let x1 = at(__SPL167, 1); let y1 = at(__SPL167, 2); let __SPL168 = assertIsOfSize(at(b, 2), 2); let x2 = at(__SPL168, 1); let y2 = at(__SPL168, 2); let m = /(1, sqrt(3)); let k = 3; toDo((-(y1, k)), +(y2, k), { :r | let rOffset = floor((/(r, 2))); toDo((-(-(x1, rOffset), k)), +(-(x2, rOffset), k), { :q | let s = -(-(0, q), r); let c = *(fromHexagonalCoordinates([q, r, s]), 0.5); let h = regularPolygon(6,c, 0.5, pi(0.5)); ifTrue(anySatisfy(vertexCoordinates(h), { :each | containsPoint(p,each) }), { add(answer,h) }) }) }); answer }",
);
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let _q = _at_2(__SPL169, 1);
        let _r = _at_2(__SPL169, 2);
        let _s = _at_2(__SPL169, 3);
        let _qR = _round_1(_q);
        let _rR = _round_1(_r);
        let _sR = _round_1(_s);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _atVectorOrElementwise_2(
      ...
      sl.annotateFunction(function (_v) {
        ...
        let __SPL170 = _assertIsOfSize_2(_v, 2);
        let _x = _at_2(__SPL170, 1);
        let _y = _at_2(__SPL170, 2);
        let _m = _sqrt_1(3);
  ...
  "{ :self | atVectorOrElementwise(self, { :v | let __SPL170 = assertIsOfSize(v, 2); let x = at(__SPL170, 1); let y = at(__SPL170, 2); let m = sqrt(3); let q = -((*(/(m, 3), x)), (*(/(1, 3), y))); let r = *(/(2, 3), y); let s = -(-(0, q), r); [q, r, s] }) }",
);
