...
sl.addMethodToExistingType(
  ...
  "{ :self | addEventListener(editorText(self), 'contextmenu', { :event | preventDefault(event); textEditorMenu(self,event) }); addEventListener(editorText(self), 'keydown', { :event | ifTrue(ctrlKey(event), { let where = caretBoundingBox(window(system)); let bindings = collect(menuItems(self), { :each | keyBinding(each,event, where) }); where(smallKansas(self), where); caseOfOtherwise(key(event),bindings, { :key | nil }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :aString | caseOfOtherwise(mimeType(self), [->({ 'text/html' }, { innerHtml(editorText(self), aString) }), ->({ 'text/markdown' }, { innerHtml(editorText(self), markdownToHtml(aString)) }), ->({ 'text/plain' }, { textContent(editorText(self), aString) })], { warning(self,++('setEditorText: unkown mimeType: ', mimeType(self))) }) }",
);
