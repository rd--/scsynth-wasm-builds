...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(coordinates(self), 3); let rho = at(__SplVar1, 1); let phi = at(__SplVar1, 2); let z = at(__SplVar1, 3); Record([['rho', rho], ['phi', phi], ['z', z]]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(self, 3); let rho = at(__SplVar2, 1); let phi = at(__SplVar2, 2); let z = at(__SplVar2, 3); initializeSlots(newCylindricalCoordinates(),self) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { let __SplVar3 = assertIsOfSize(self, 3); let rho = at(__SplVar3, 1); let phi = at(__SplVar3, 2); let z = at(__SplVar3, 3); let x = *(rho, cos(phi)); let y = *(rho, sin(phi)); [x, y, z] }, { collect(self,fromCylindricalCoordinates:/1) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isVector(self), { let __SplVar4 = assertIsOfSize(self, 3); let x = at(__SplVar4, 1); let y = at(__SplVar4, 2); let z = at(__SplVar4, 3); let rho = sqrt((+(square(x), square(y)))); let phi = atan2(y,x); [rho, phi, z] }, { collect(self,toCylindricalCoordinates:/1) }) }",
);
