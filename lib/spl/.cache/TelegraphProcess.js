...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _r, _t, _n) {
    ...
    let _mu = _mu_1(_self);
    let __SPL1 = _assertIsOfSize_2(_t, 3);
    let _tMin = _at_2(__SPL1, 1);
    let _tMax = _at_2(__SPL1, 2);
    let _dt = _at_2(__SPL1, 3);
  ...
  "{ :self :r :t :n | let mu = mu(self); let __SPL1 = assertIsOfSize(t, 3); let tMin = at(__SPL1, 1); let tMax = at(__SPL1, 2); let dt = at(__SPL1, 3); let ts = asList(thenTo(tMin, +(tMin, dt), tMax)); let k = size(ts); TemporalData(!({ let a = 1; let e = exponentialDistribution(r,mu); transpose([ts, collect(ts, { :t | ifTrue((>=(t, e)), { a := negate(a); e := +(e, exponentialDistribution(r,mu)) }); a })]) }, n)) }",
);
