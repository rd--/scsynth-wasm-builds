...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _k) {
    ...
    let __SPL21 = _assertIsOfSize_2(_max_1(_i), 2);
    let _w = _at_2(__SPL21, 1);
    let _h = _at_2(__SPL21, 2);
    let _a = _collect_2(
      _i,
  ...
  "{ :self :k | let p = vertexCoordinates(self); let q = rescale((-(p, [min(p)]))); let i = rounded((*(q, k))); let __SPL21 = assertIsOfSize(max(i), 2); let w = at(__SPL21, 1); let h = at(__SPL21, 2); let a = collect(i, { :each | let __SPL22 = assertIsOfSize(each, 2); let x = at(__SPL22, 1); let y = at(__SPL22, 2); ->([-(h, y), x], 1) }); Bitmap(normal(SparseArray(a, [h, w], 0))) }",
);
