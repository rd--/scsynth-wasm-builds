...
sl.addMethodToExistingType(
  ...
  "{ :self :k | let p = vertexCoordinates(self); let q = rescale((-(p, [min(p)]))); let i = round((*(q, k))); let __SplVar1 = assertIsOfSize(max(i), 2); let w = at(__SplVar1, 1); let h = at(__SplVar1, 2); let a = collect(i, { :each | let __SplVar2 = assertIsOfSize(each, 2); let x = at(__SplVar2, 1); let y = at(__SplVar2, 2); ->([-(h, y), x], 1) }); Bitmap(normal(SparseArray(a, [h, w], 0))) }",
);
