...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _k) {
    ...
    let __SPL253 = _assertIsOfSize_2(_max_1(_i), 2);
    let _w = _at_2(__SPL253, 1);
    let _h = _at_2(__SPL253, 2);
    let _a = _collect_2(
      _i,
      sl.annotateFunction(function (_each) {
        ...
        let __SPL254 = _assertIsOfSize_2(_each, 2);
  ...
  "{ :self :k | let p = vertexCoordinates(self); let q = rescale((-(p, [min(p)]))); let i = round((*(q, k))); let __SPL253 = assertIsOfSize(max(i), 2); let w = at(__SPL253, 1); let h = at(__SPL253, 2); let a = collect(i, { :each | let __SPL254 = assertIsOfSize(each, 2); let x = at(__SPL254, 1); let y = at(__SPL254, 2); ->([-(h, y), x], 1) }); Bitmap(normal(SparseArray(a, [h, w], 0))) }",
);
