...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self, _options) {
    ...
    return _join_2(
      _collect_2(_self, function (_each) {
        ...
        let __SPL172 = _assertIsOfSize_2(_each, 2);
  ...
  "{ :self :options | let precision = at(options, 'precision'); join(collect(self, { :each | let __SPL172 = assertIsOfSize(each, 2); let x = at(__SPL172, 1); let y = at(__SPL172, 2); format('%,%',[printStringToFixed(x,precision), printStringToFixed(y,precision)]) }),' ') }",
);
