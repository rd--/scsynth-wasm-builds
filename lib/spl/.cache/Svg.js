...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :options | let precision = at(options, 'precision'); unwords(collect(self, { :each | let __SPL9 = assertIsOfSize(each, 2); let x = at(__SPL9, 1); let y = at(__SPL9, 2); format('%,%',[printStringToFixed(x,precision), printStringToFixed(y,precision)]) })) }",
);
