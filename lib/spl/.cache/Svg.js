...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :options | let precision = at(options, 'precision'); unwords(collect(self, { :each | let __SPL1 = assertIsOfSize(each, 2); let x = at(__SPL1, 1); let y = at(__SPL1, 2); format('%,%',[printStringToFixed(x,precision), printStringToFixed(y,precision)]) })) }",
);
