...
sl.addMethodToExistingType(
  ...
  "{ :self :aBlock:/2 | let all = UnsortedSet(); let common = UnsortedSet(); comparator(all, aBlock:/2); comparator(common, aBlock:/2); do(self, { :each | ifTrue(isSymbolicExpression(each), { if(includes(all,each), { basicInclude(common,each) }, { basicInclude(all,each) }) }) }); asList(common) }",
);
