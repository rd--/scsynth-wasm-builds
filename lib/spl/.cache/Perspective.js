...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :left :right :bottom :top :nearVal :farVal | [[/(2, (-(right, left))), 0, 0, 0], [0, /(2, (-(top, bottom))), 0, 0], [0, 0, /(-2, (-(farVal, nearVal))), 0], [/(negate((+(right, left))), (-(right, left))), /(negate((+(top, bottom))), (-(top, bottom))), /((+(farVal, negate(nearVal))), (-(farVal, nearVal))), 1]] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :fovy :aspect :zNear :zFar | let ymax = *(zNear, tan((/(pi(fovy), 360)))); let ymin = negate(ymax); let xmin = *(ymin, aspect); let xmax = *(ymax, aspect); glFrustum(xmin, xmax, ymin, ymax, zNear, zFar) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_eye, _center, _up) {
    ...
    let __SPL222 = _assertIsOfSize_2(_normalize_1(_x), 3);
    let _x1 = _at_2(__SPL222, 1);
    let _x2 = _at_2(__SPL222, 2);
    let _x3 = _at_2(__SPL222, 3);
    let __SPL223 = _assertIsOfSize_2(_normalize_1(_y), 3);
    let _y1 = _at_2(__SPL223, 1);
    let _y2 = _at_2(__SPL223, 2);
    let _y3 = _at_2(__SPL223, 3);
    let __SPL224 = _assertIsOfSize_2(_z, 3);
    let _z1 = _at_2(__SPL224, 1);
    let _z2 = _at_2(__SPL224, 2);
    let _z3 = _at_2(__SPL224, 3);
    let __SPL225 = _assertIsOfSize_2(_negate_1(_eye), 3);
    let _w1 = _at_2(__SPL225, 1);
    let _w2 = _at_2(__SPL225, 2);
    let _w3 = _at_2(__SPL225, 3);
    /* Statements */
  ...
  "{ :eye :center :up | let z = normalize((-(eye, center))); let x = cross(up,z); let y = cross(z,x); let __SPL222 = assertIsOfSize(normalize(x), 3); let x1 = at(__SPL222, 1); let x2 = at(__SPL222, 2); let x3 = at(__SPL222, 3); let __SPL223 = assertIsOfSize(normalize(y), 3); let y1 = at(__SPL223, 1); let y2 = at(__SPL223, 2); let y3 = at(__SPL223, 3); let __SPL224 = assertIsOfSize(z, 3); let z1 = at(__SPL224, 1); let z2 = at(__SPL224, 2); let z3 = at(__SPL224, 3); let __SPL225 = assertIsOfSize(negate(eye), 3); let w1 = at(__SPL225, 1); let w2 = at(__SPL225, 2); let w3 = at(__SPL225, 3); [[x1, y1, z1, w1], [x2, y2, z2, w2], [x3, y3, z3, w3], [0, 0, 0, 1]] }",
);
