...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _r, _t, _n) {
    ...
    let __SPL533 = _assertIsOfSize_2(_t, 2);
    let _tMin = _at_2(__SPL533, 1);
    let _tMax = _at_2(__SPL533, 2);
    /* Statements */
    return _timeSeriesRandomFunction_3(_asStream_3(_self, _r, _tMin), _t, _n);
  }, ["self", "r", "t", "n"]),
  "{ :self :r :t :n | let __SPL533 = assertIsOfSize(t, 2); let tMin = at(__SPL533, 1); let tMax = at(__SPL533, 2); timeSeriesRandomFunction(asStream(self,r, tMin),t, n) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _q = _q_1(_self);
    let __SPL534 = _assertIsOfSize_2(_shape_1(_q), 2);
    let _m = _at_2(__SPL534, 1);
    let _n = _at_2(__SPL534, 2);
  ...
  "{ :self | let q = q(self); let __SPL534 = assertIsOfSize(shape(q), 2); let m = at(__SPL534, 1); let n = at(__SPL534, 2); let t = zeroMatrix([m, n]); do(Span(1, m, 1), { :i | let h = abs(at(at(q, i), i)); let r = if((=(h, 0)), { 0 }, { /(1, h) }); do(Span(1, n, 1), { :j | ifTrue((!=(i, j)), { atPut(at(t, i), j, *(at(at(q, i), j), r)) }) }); atPut(at(t, i), i, abs((-(1, sum(at(t, i)))))) }); t }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :p0 :m :mu | let n = size(m); let q = table({ :i :j | if((=(i, j)), { -(at(mu, i)) }, { *(at(at(m, i), j), at(mu, i)) }) },Span(1, n, 1), Span(1, n, 1)); ContinuousMarkovProcess(p0, q) }",
);
