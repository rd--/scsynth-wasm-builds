...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _r, _t, _n) {
    ...
    let __SPL42 = _assertIsOfSize_2(_t, 2);
    let _tMin = _at_2(__SPL42, 1);
    let _tMax = _at_2(__SPL42, 2);
    /* Statements */
    return _timeSeriesRandomFunction_3(_asStream_3(_self, _r, _tMin), _t, _n);
  }, ["self", "r", "t", "n"]),
  "{ :self :r :t :n | let __SPL42 = assertIsOfSize(t, 2); let tMin = at(__SPL42, 1); let tMax = at(__SPL42, 2); timeSeriesRandomFunction(asStream(self,r, tMin),t, n) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _q = _q_1(_self);
    let __SPL43 = _assertIsOfSize_2(_shape_1(_q), 2);
  ...
  "{ :self | let q = q(self); let __SPL43 = assertIsOfSize(shape(q), 2); let m = at(__SPL43, 1); let n = at(__SPL43, 2); let t = zeroMatrix([m, n]); do(Span(1, m, 1), { :i | let h = abs(at(at(q, i), i)); let r = if((=(h, 0)), { 0 }, { /(1, h) }); do(Span(1, n, 1), { :j | ifTrue((~=(i, j)), { atPut(at(t, i), j, *(at(at(q, i), j), r)) }) }); atPut(at(t, i), i, abs((-(1, sum(at(t, i)))))) }); t }",
);
