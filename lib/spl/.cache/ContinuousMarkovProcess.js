...
sl.addMethodToExistingType(
  ...
  "{ :self | let q = q(self); let __SplVar2 = assertIsOfSize(shape(q), 2); let m = at(__SplVar2, 1); let n = at(__SplVar2, 2); let t = zeroMatrix([m, n]); do(Span(1, m, 1), { :i | let h = abs(at(at(q, i), i)); let r = if((=(h, 0)), { 0 }, { /(1, h) }); do(Span(1, n, 1), { :j | ifTrue((!=(i, j)), { atPut(at(t, i), j, *(at(at(q, i), j), r)) }) }); atPut(at(t, i), i, abs((-(1, sum(at(t, i)))))) }); t }",
);
