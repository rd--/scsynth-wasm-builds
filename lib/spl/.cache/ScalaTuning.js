...
sl.addMethodToExistingType(
  ...
  "{ :self | asRatioTuning(asRatios(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let octave = at(contents(self), 'octave'); if(isNumber(octave), { centsToRatio(octave) }, { if(isList(octave), { let __SplVar1 = assertIsOfSize(octave, 2); let numerator = at(__SplVar1, 1); let denominator = at(__SplVar1, 2); Fraction(numerator, denominator) }, { error(self,'octave: invalid octave') }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | collect(at(contents(self), 'pitches'), { :each | if(isNumber(each), { each }, { if(isList(each), { let __SplVar2 = assertIsOfSize(each, 2); let numerator = at(__SplVar2, 1); let denominator = at(__SplVar2, 2); Fraction(numerator, denominator) }, { error(self,'pitches: invalid pitch') }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | if(isRational(self), { tuningLatticeGraph(asRatioTuning(self)) }, { error(self,'tuningLatticeGraph: irrational') }) }",
);
