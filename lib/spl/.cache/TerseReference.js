...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :directoryName :options :testName :aBlock:/2 | let totalTestCount = 0; let totalPassCount = 0; let totalErrorCount = 0; let fileNameList = readDirectoryFileNames(directoryName); let helpFileNameList = select(sortBy(fileNameList,<|), { :each | &(endsWith(each,'.help.sl'), { matchesRegularExpression(pathBasename(each),at(options, 'pattern')) }) }); let textList = readTextFileList(helpFileNameList); withIndexDo(textList, { :text :index | let fileName = at(helpFileNameList, index); let helpFile = HelpFile(asFileUrl(fileName), text); let __SplVar1 = assertIsOfSize(value({ ifTrue(at(options, 'verbose'), { postLine(Tuple([originName(helpFile), name(helpFile)])) }); aBlock(helpFile, options) }), 3); let testCount = at(__SplVar1, 1); let passCount = at(__SplVar1, 2); let errorCount = at(__SplVar1, 3); ifTrue(at(options, 'verbose'), { postLine(format('\tPass % of %',[passCount, testCount])) }); totalTestCount := +(totalTestCount, testCount); totalPassCount := +(totalPassCount, passCount); totalErrorCount := +(totalErrorCount, errorCount) }); postLine(format('Terse Reference Summary: %: Pass % of % (% Error)',[testName, totalPassCount, totalTestCount, totalErrorCount])) }",
);
