...
sl.extendTypeOrTraitWithMethod(
    ...
    function (_directoryName, _options, _testName, _aBlock_2) {
      ...
      _withIndexDo_2(
        ...
        sl.annotateFunction(function (_text, _index) {
          ...
          let _testCount = _at_2(__SPL2, 1);
          let _passCount = _at_2(__SPL2, 2);
          let _errorCount = _at_2(__SPL2, 3);
          /* Statements */
          _ifTrue_2(
            _at_2(_options, "verbose"),
            sl.annotateFunction(function () {
  ...
  "{ :directoryName :options :testName :aBlock:/2 | let totalTestCount = 0; let totalPassCount = 0; let totalErrorCount = 0; let fileNameList = readDirectoryFileNames(directoryName); let helpFileNameList = select(sort(fileNameList), { :each | &(endsWith(each,'.help.sl'), { matchesRegularExpression(pathBasename(each),at(options, 'pattern')) }) }); let textList = readTextFileList(helpFileNameList); withIndexDo(textList, { :text :index | let fileName = at(helpFileNameList, index); let helpFile = HelpFile(asFileUrl(fileName), text); let __SPL2 = assertIsOfSize(value({ ifTrue(at(options, 'verbose'), { postLine(Tuple([originName(helpFile), name(helpFile)])) }); aBlock(helpFile, options) }), 3); let testCount = at(__SPL2, 1); let passCount = at(__SPL2, 2); let errorCount = at(__SPL2, 3); ifTrue(at(options, 'verbose'), { postLine(format('\tPass % of %',[passCount, testCount])) }); totalTestCount := +(totalTestCount, testCount); totalPassCount := +(totalPassCount, passCount); totalErrorCount := +(totalErrorCount, errorCount) }); postLine(format('Terse Reference Summary: %: Pass % of % (% Error)',[testName, totalPassCount, totalTestCount, totalErrorCount])) }",
);
