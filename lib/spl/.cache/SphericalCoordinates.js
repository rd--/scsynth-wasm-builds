...
sl.addMethodToExistingTrait(
  ...
  "{ :self | join(['SphericalCoordinates(', storeString(r(self)), ', ', storeString(theta(self)), ', ', storeString(phi(self)), ')'],'') }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    let _r = _at_2(__SPL7, 1);
    let _theta = _at_2(__SPL7, 2);
    let _phi = _at_2(__SPL7, 3);
    /* Statements */
    return _SphericalCoordinates_3(_r, _theta, _phi);
  },
  "{ :self | let __SPL7 = assertIsOfSize(self, 3); let r = at(__SPL7, 1); let theta = at(__SPL7, 2); let phi = at(__SPL7, 3); SphericalCoordinates(r, theta, phi) }",
);
