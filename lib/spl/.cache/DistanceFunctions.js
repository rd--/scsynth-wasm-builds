...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(self, 2); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); let n = size(a); let y = /(n, goldenRatio(1)); let p = collect(upOrDownTo(1, n), { :i | [at(b, i), 0] }); let q = collect(upOrDownTo(1, n), { :i | [at(a, i), y] }); let r = collect(upOrDownTo(1, n), { :i | Line([at(p, i), at(q, i)]) }); LineDrawing([PointCloud(p), PointCloud(q), r]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :y :w :f:/2 | let derivePath = { :tracebackMatrix | let __SplVar2 = assertIsOfSize(shape(tracebackMatrix), 2); let n = at(__SplVar2, 1); let m = at(__SplVar2, 2); let i = n; let j = m; let path = [[i, j]]; whileTrue({ |(>(i, 1), { >(j, 1) }) }, { caseOf(at(at(tracebackMatrix, i), j),[->(1, { i := -(i, 1); j := -(j, 1) }), ->(2, { i := (-(i, 1)) }), ->(3, { j := -(j, 1) })]); addLast(path,[i, j]) }); path }; let __SplVar3 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let tracebackMatrix = at(__SplVar3, 1); let _ = at(__SplVar3, 2); collect(transpose(derivePath(tracebackMatrix)),reverse:/1) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :y :w :f:/2 | let i = size(x); let j = size(y); let __SplVar4 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let _ = at(__SplVar4, 1); let costMatrix = at(__SplVar4, 2); at(at(costMatrix, i), j) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :y :c | let __SplVar5 = assertIsOfSize(c, 2); let a = at(__SplVar5, 1); let b = at(__SplVar5, 2); let o = *((-(range(y), min(x))), 1.5); let p = transpose([asList(upOrDownTo(1, size(x))), +(x, o)]); let q = transpose([asList(upOrDownTo(1, size(y))), y]); let r = collect(upOrDownTo(1, size(a)), { :i | Line([at(p, at(a, i)), at(q, at(b, i))]) }); LineDrawing([PointCloud((++(p, q))), Line(p), Line(q), r]) }",
);
