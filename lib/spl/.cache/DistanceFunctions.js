...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL54 = _assertIsOfSize_2(_self, 2);
    let _a = _at_2(__SPL54, 1);
    let _b = _at_2(__SPL54, 2);
    let _n = _size_1(_a);
  ...
  "{ :self | let __SPL54 = assertIsOfSize(self, 2); let a = at(__SPL54, 1); let b = at(__SPL54, 2); let n = size(a); let y = /(n, goldenRatio(1)); let p = collect(upOrDownTo(1, n), { :i | [at(b, i), 0] }); let q = collect(upOrDownTo(1, n), { :i | [at(a, i), y] }); let r = collect(upOrDownTo(1, n), { :i | Line([at(p, i), at(q, i)]) }); LineDrawing([PointCloud(p), PointCloud(q), r]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _w, _f_2) {
    ...
    let _derivePath_1 = sl.annotateFunction(function (_tracebackMatrix) {
      ...
      let __SPL55 = _assertIsOfSize_2(_shape_1(_tracebackMatrix), 2);
    ...
    let __SPL56 = _assertIsOfSize_2(_warpingMatrices_4(_x, _y, _w, _f_2), 2);
    let _tracebackMatrix = _at_2(__SPL56, 1);
    let __genSym59 = _at_2(__SPL56, 2);
    /* Statements */
    return _collect_2(
  ...
  "{ :x :y :w :f:/2 | let derivePath = { :tracebackMatrix | let __SPL55 = assertIsOfSize(shape(tracebackMatrix), 2); let n = at(__SPL55, 1); let m = at(__SPL55, 2); let i = n; let j = m; let path = [[i, j]]; whileTrue({ |(>(i, 1), { >(j, 1) }) }, { caseOf(at(at(tracebackMatrix, i), j),[->(1, { i := -(i, 1); j := -(j, 1) }), ->(2, { i := (-(i, 1)) }), ->(3, { j := -(j, 1) })]); addLast(path,[i, j]) }); path }; let __SPL56 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let tracebackMatrix = at(__SPL56, 1); let _ = at(__SPL56, 2); collect(transpose(derivePath(tracebackMatrix)),reverse:/1) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _w, _f_2) {
    ...
    let _i = _size_1(_x);
    let _j = _size_1(_y);
    let __SPL57 = _assertIsOfSize_2(_warpingMatrices_4(_x, _y, _w, _f_2), 2);
    let __genSym60 = _at_2(__SPL57, 1);
    let _costMatrix = _at_2(__SPL57, 2);
    /* Statements */
    return _at_2(_at_2(_costMatrix, _i), _j);
  }, ["x", "y", "w", "f:/2"]),
  "{ :x :y :w :f:/2 | let i = size(x); let j = size(y); let __SPL57 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let _ = at(__SPL57, 1); let costMatrix = at(__SPL57, 2); at(at(costMatrix, i), j) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _c) {
    ...
    let __SPL58 = _assertIsOfSize_2(_c, 2);
  ...
  "{ :x :y :c | let __SPL58 = assertIsOfSize(c, 2); let a = at(__SPL58, 1); let b = at(__SPL58, 2); let o = *((-(range(y), min(x))), 1.5); let p = transpose([asList(upOrDownTo(1, size(x))), +(x, o)]); let q = transpose([asList(upOrDownTo(1, size(y))), y]); let r = collect(upOrDownTo(1, size(a)), { :i | Line([at(p, at(a, i)), at(q, at(b, i))]) }); LineDrawing([PointCloud((++(p, q))), Line(p), Line(q), r]) }",
);
