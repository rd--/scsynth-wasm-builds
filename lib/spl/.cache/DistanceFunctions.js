sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_u, _v) {
    ...
    return _boole_1(_tildeEqualsSign_2(_u, _v));
  }, ["u", "v"]),
  "{ :u :v | boole((~=(u, v))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _operand) {
    ...
    return _boole_1(_tildeEqualsSign_2(_self, _operand));
  }, ["self", "operand"]),
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL318 = _assertIsOfSize_2(_self, 2);
    let _a = _at_2(__SPL318, 1);
    let _b = _at_2(__SPL318, 2);
    let _n = _size_1(_a);
  ...
  "{ :self | let __SPL318 = assertIsOfSize(self, 2); let a = at(__SPL318, 1); let b = at(__SPL318, 2); let n = size(a); let y = /(n, goldenRatio(1)); let p = collect(upOrDownTo(1, n), { :i | [at(b, i), 0] }); let q = collect(upOrDownTo(1, n), { :i | [at(a, i), y] }); let r = collect(upOrDownTo(1, n), { :i | Line([at(p, i), at(q, i)]) }); LineDrawing([PointCloud(p), PointCloud(q), r]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _w, _f_2) {
    ...
    let _derivePath_1 = sl.annotateFunction(function (_tracebackMatrix) {
      ...
      let __SPL319 = _assertIsOfSize_2(_shape_1(_tracebackMatrix), 2);
      let _n = _at_2(__SPL319, 1);
      let _m = _at_2(__SPL319, 2);
      let _i = _n;
      let _j = _m;
      let _path = [[_i, _j]];
      /* Statements */
    ...
    let __SPL320 = _assertIsOfSize_2(_warpingMatrices_4(_x, _y, _w, _f_2), 2);
    let _tracebackMatrix = _at_2(__SPL320, 1);
    let __genSym323 = _at_2(__SPL320, 2);
    /* Statements */
    return _collect_2(
      _transposed_1(_derivePath_1(_tracebackMatrix)),
      _reverse_1,
    );
  }, ["x", "y", "w", "f:/2"]),
  "{ :x :y :w :f:/2 | let derivePath = { :tracebackMatrix | let __SPL319 = assertIsOfSize(shape(tracebackMatrix), 2); let n = at(__SPL319, 1); let m = at(__SPL319, 2); let i = n; let j = m; let path = [[i, j]]; whileTrue({ |(>(i, 1), { >(j, 1) }) }, { caseOf(at(at(tracebackMatrix, i), j),[->(1, { i := -(i, 1); j := -(j, 1) }), ->(2, { i := (-(i, 1)) }), ->(3, { j := -(j, 1) })]); addLast(path,[i, j]) }); path }; let __SPL320 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let tracebackMatrix = at(__SPL320, 1); let _ = at(__SPL320, 2); collect(transposed(derivePath(tracebackMatrix)),reverse:/1) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _w, _f_2) {
    ...
    let _i = _size_1(_x);
    let _j = _size_1(_y);
    let __SPL321 = _assertIsOfSize_2(_warpingMatrices_4(_x, _y, _w, _f_2), 2);
    let __genSym324 = _at_2(__SPL321, 1);
    let _costMatrix = _at_2(__SPL321, 2);
    /* Statements */
    return _at_2(_at_2(_costMatrix, _i), _j);
  }, ["x", "y", "w", "f:/2"]),
  "{ :x :y :w :f:/2 | let i = size(x); let j = size(y); let __SPL321 = assertIsOfSize(warpingMatrices(x, y, w, f:/2), 2); let _ = at(__SPL321, 1); let costMatrix = at(__SPL321, 2); at(at(costMatrix, i), j) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_x, _y, _c) {
    ...
    let __SPL322 = _assertIsOfSize_2(_c, 2);
    let _a = _at_2(__SPL322, 1);
    let _b = _at_2(__SPL322, 2);
    let _o = _asterisk_2(_hyphenMinus_2(_range_1(_y), _min_1(_x)), 1.5);
    let _p = _transposed_1([
      _asList_1(_upOrDownTo_2(1, _size_1(_x))),
      _plusSign_2(_x, _o),
    ]);
    let _q = _transposed_1([_asList_1(_upOrDownTo_2(1, _size_1(_y))), _y]);
    let _r = _collect_2(
  ...
  "{ :x :y :c | let __SPL322 = assertIsOfSize(c, 2); let a = at(__SPL322, 1); let b = at(__SPL322, 2); let o = *((-(range(y), min(x))), 1.5); let p = transposed([asList(upOrDownTo(1, size(x))), +(x, o)]); let q = transposed([asList(upOrDownTo(1, size(y))), y]); let r = collect(upOrDownTo(1, size(a)), { :i | Line([at(p, at(a, i)), at(q, at(b, i))]) }); LineDrawing([PointCloud((++(p, q))), Line(p), Line(q), r]) }",
);
