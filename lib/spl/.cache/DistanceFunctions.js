...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :y :c | let __SPL5 = assertIsOfSize(c, 2); let a = at(__SPL5, 1); let b = at(__SPL5, 2); let o = *((-(range(y), min(x))), 1.5); let p = transposed([asList(upOrDownTo(1, size(x))), +(x, o)]); let q = transposed([asList(upOrDownTo(1, size(y))), y]); let r = collect(upOrDownTo(1, size(a)), { :i | Line([at(p, at(a, i)), at(q, at(b, i))]) }); LineDrawing([PointCloud((++(p, q))), Line(p), Line(q), r]) }",
);
