...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(splitBy(self,';'), 2); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); let __SplVar2 = assertIsOfSize(collect([a, b], { :each | collect(splitBy(each,','),parseDecimalInteger:/1) }), 2); let p = at(__SplVar2, 1); let q = at(__SplVar2, 2); let __SplVar3 = assertIsOfSize(collect([p, q],size:/1), 2); let m = at(__SplVar3, 1); let n = at(__SplVar3, 2); +(sum((*(p, (^(60, upOrDownTo(-(m, 1), 0)))))), sum((/(q, (^(60, Span(1, n, 1))))))) }",
);
