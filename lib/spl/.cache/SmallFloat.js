...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject | if(isSmallFloat(anObject), { if(isInteger(anObject), { raisedToInteger(self,anObject) }, { if(isNegative(self), { ^(Complex(self, 0), anObject) }, { raisedToSmallFloat(self,anObject) }) }) }, { adaptToNumberAndApply(anObject,self, ^) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :anInteger :aBlock:/2 | if(isLargeInteger(anInteger), { aBlock(asSmallFloat(anInteger), self) }, { error(self,'adaptToIntegerAndApply: ?') }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :a :b | wrapExclusive((+(a, b)),-2147483648, +(2147483647, 1)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | if(isSmallInteger(self), { self }, { error(self,'SmallFloat>>asSmallInteger: not integer') }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :anInteger | <primitive: return (_self << _anInteger) | (_self >>> (32 - _anInteger));>\n }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :epsilon | if((<(abs(self), epsilon)), { 0 }, { self }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :n :x | let a = sqrt((+(4, square(x)))); let b = +(x, a); let c = ^(2, (-(0, n))); let d = ^(2, n); let e = ^(b, n); let f = ^(b, (-(0, n))); let g = cos(pi(n)); let r = /((-((*(c, e)), (*(*(d, f), g)))), a); if((&(isInteger(n), { isInteger(x) })), { round(r) }, { r }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :n | let phi = goldenRatio(1); let r = /((-((^(phi, n)), (*(cos(pi(n)), (^(phi, -(n))))))), sqrt(5)); if(isInteger(n), { round(r) }, { r }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :aNumber :epsilon | if(not(isNumber(aNumber)), { false }, { if((=(self, 0)), { <(abs(aNumber), epsilon) }, { if((=(aNumber, 0)), { <(abs(self), epsilon) }, { let other = asSmallFloat(aNumber); |(=(self, other), { let z = abs(self); if((<(z, epsilon)), { <(abs(other), epsilon) }, { <(/(abs((-(self, other))), (max(z,abs(other)))), epsilon) }) }) }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :epsilon | <(abs((-(self, round(self)))), epsilon) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let phi = goldenRatio(1); +((^(phi, self)), (*(cos(pi(self)), (^(phi, negate(self)))))) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :m :n | <primitive: return Math.imul(_m, _n);>\n }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :aPositiveInteger | ifTrue((|(not(isInteger(aPositiveInteger)), { isNegative(aPositiveInteger) })), { error('nthRoot: only defined for positive integers') }); if(isNegative(self), { if(isOdd(aPositiveInteger), { negate((^(negate(self), (/(1, aPositiveInteger))))) }, { error('nthRoot: negative numbers do not have even roots.') }) }, { ^(self, (/(1, aPositiveInteger))) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :base | let i = truncate(abs(self)); let f = fractionalPart(abs(self)); let d = integerDigits(i,base); let k = size(d); let m = collect(upOrDownTo(k, 1), { :each | ^(base, (-(each, 1))) }); let l = (*(d, m)); ifFalse((=(f, 0)), { atPut(l, k, +(at(l, k), f)) }); *(l, sign(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :base :size | if((!=(base, 10)), { error(self,'SmallFloat>>realDigits: not implemented unless base=10') }, { let exponent = round((+(log10(self), 1))); [integerDigits(round((*(self, (^(10, (-(size, exponent))))))),10, size), exponent] }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | if(isCloseTo(self,round(self)), { round(self) }, { self }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :n | if((=(fractionalPart(n), 0.5)), { *(2, round((/(n, 2)))) }, { round(n) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL38 = _assertIsOfSize_2(_splitBy_2(_self, ";"), 2);
    let _a = _at_2(__SPL38, 1);
    let _b = _at_2(__SPL38, 2);
    let __SPL39 = _assertIsOfSize_2(
      _collect_2(
        [_a, _b],
        sl.annotateFunction(function (_each) {
          /* ArityCheck */
    ...
    let _p = _at_2(__SPL39, 1);
    let _q = _at_2(__SPL39, 2);
    let __SPL40 = _assertIsOfSize_2(_collect_2([_p, _q], _size_1), 2);
  ...
  "{ :self | let __SPL38 = assertIsOfSize(splitBy(self,';'), 2); let a = at(__SPL38, 1); let b = at(__SPL38, 2); let __SPL39 = assertIsOfSize(collect([a, b], { :each | collect(splitBy(each,','),parseDecimalInteger:/1) }), 2); let p = at(__SPL39, 1); let q = at(__SPL39, 2); let __SPL40 = assertIsOfSize(collect([p, q],size:/1), 2); let m = at(__SPL40, 1); let n = at(__SPL40, 2); +(sum((*(p, (^(60, upOrDownTo(-(m, 1), 0)))))), sum((/(q, (^(60, Span(1, n, 1))))))) }",
);
