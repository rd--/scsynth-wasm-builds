...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :c :x :y0 | let __SplVar1 = assertIsOfSize(c, 2); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); let __SplVar2 = assertIsOfSize([size(a), size(b)], 2); let n = at(__SplVar2, 1); let m = at(__SplVar2, 2); let y = List(size(x), 0); toDo(1, size(x), { :i | let p = sum(collect(upOrDownTo(0, -(m, 1)), { :j | *(at(b,+(j, 1)), atIfAbsent(x, -(i, j), { 0 })) })); let q = sum(collect(upOrDownTo(0, -(n, 1)), { :j | *(at(a,+(j, 1)), atIfAbsent(y, -(i, j), { at(y0,-(1, (-(i, j)))) })) })); atPut(y, i, -(p, q)) }); y }",
);
