...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_c, _x, _y0) {
    ...
    let __SPL468 = _assertIsOfSize_2(_c, 2);
    let _a = _at_2(__SPL468, 1);
    let _b = _at_2(__SPL468, 2);
    let __SPL469 = _assertIsOfSize_2([_size_1(_a), _size_1(_b)], 2);
    let _n = _at_2(__SPL469, 1);
    let _m = _at_2(__SPL469, 2);
  ...
  "{ :c :x :y0 | let __SPL468 = assertIsOfSize(c, 2); let a = at(__SPL468, 1); let b = at(__SPL468, 2); let __SPL469 = assertIsOfSize([size(a), size(b)], 2); let n = at(__SPL469, 1); let m = at(__SPL469, 2); let y = List(size(x), 0); toDo(1, size(x), { :i | let p = sum(collect(upOrDownTo(0, -(m, 1)), { :j | *(at(b,+(j, 1)), atIfAbsent(x, -(i, j), { 0 })) })); let q = sum(collect(upOrDownTo(0, -(n, 1)), { :j | *(at(a,+(j, 1)), atIfAbsent(y, -(i, j), { at(y0,-(1, (-(i, j)))) })) })); atPut(y, i, -(p, q)) }); y }",
);
