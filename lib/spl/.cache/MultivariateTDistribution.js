...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _r, _shape) {
    ...
    let __SPL445 = _assertIsOfSize_2(_shape, 1);
    let _n = _at_2(__SPL445, 1);
    let _x = _solidus_2(
  ...
  "{ :self :r :shape | let k = size(sigma(self)); let __SPL445 = assertIsOfSize(shape, 1); let n = at(__SPL445, 1); let x = /(randomVariate(ChiSquareDistribution(nu(self)),r, shape), nu(self)); let z = randomVariate(MultinormalDistribution(#(0, k), sigma(self)),r, shape); +([mu(self)], (/(z, sqrt(x)))) }",
);
