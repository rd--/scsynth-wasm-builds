...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_c) {
    ...
    let _n = _size_1(_c);
    let __SPL1 = _assertIsOfSize_2(_transposed_1(_c), 2);
  ...
  "{ :c | let n = size(c); let __SPL1 = assertIsOfSize(transposed(c), 2); let x = at(__SPL1, 1); let y = at(__SPL1, 2); let p = List(n, UnivariatePolynomial([1])); toDo(1, n, { :i | toDo(1, n, { :j | ifFalse((=(i, j)), { let q = UnivariatePolynomial([-(0, at(x, j)), 1]); atPut(p, i, *(at(p, i), q)) }) }); atPut(p, i, *(at(p, i), (/(1, at(at(p, i),at(x, i)))))) }); sum((*(p, y))) }",
);
