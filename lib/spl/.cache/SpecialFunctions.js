sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x | if((>=(x, 0)), { let p = 0.47047; let t = /(1, (+(1, (*(p, x))))); let a1 = *(0.3480242, t); let a2 = *(*(-0.0958798, t), t); let a3 = *(*(*(0.7478556, t), t), t); -(1, (*((+(+(a1, a2), a3)), exp(negate(square(x)))))) }, { negate(erf(negate(x))) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x | if((>=(x, 0)), { let p = 0.3275911; let t = /(1, (+(1, (*(p, x))))); let a1 = *(0.254829592, t); let a2 = *(*(-0.284496736, t), t); let a3 = *(*(*(1.421413741, t), t), t); let a4 = *(*(*(*(-1.453152027, t), t), t), t); let a5 = *(*(*(*(*(1.061405429, t), t), t), t), t); -(1, (*((+(+(+(+(a1, a2), a3), a4), a5)), exp(negate(square(x)))))) }, { negate(erf(negate(x))) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :s | let p = 8; let n = round((+((*(1.3, p)), (*(0.9, abs(imaginary(s))))))); let e = { :k | sum(upOrDownTo(k, n), { :j | binomial(n, j) }) }; let a = /(1, (-(1, (^(2, (-(1, s))))))); let b = sum(upOrDownTo(1, n), { :k | /((^(-1, (-(k, 1)))), (^(k, s))) }); let c = /(1, (^(2, n))); let d = sum(upOrDownTo(+(n, 1), *(2, n)), { :k | /((*((^(-1, (-(k, 1)))), e(-(k, n)))), (^(k, s))) }); *(a, (+(b, (*(c, d))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :a :b :c :z | let tolerance = 1E-10; let iterationLimit = 1000; let term = /(*(*(a, b), z), c); let value = +(1, term); let n = 1; whileTrue({ &(>(abs(term), tolerance), { <(n, iterationLimit) }) }, { a := +(a, 1); b := +(b, 1); c := +(c, 1); n := +(n, 1); term := *(term, (/(/(*(*(a, b), z), c), n))); value := +(value, term) }); value }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :n | let dyadicRationals = { :x :n | let m = ^(2, n); collect(asList(upOrDownTo(0, *(x, m))), { :k | \\(k, m) }) }; *(/((^(2, (/(*(n, (+(n, 1))), 2)))), factorial(n)), sum(dyadicRationals(x, n), { :k | *((^(-1, digitSum(asLargeInteger((*(k, (^(2, n))))), 2))), (^((-(x, k)), n))) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :n | fabiusFunction(asFraction(x),n) }",
);
