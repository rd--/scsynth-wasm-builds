...
sl.extendTypeOrTraitWithMethod(
          ...
          return _caseOf_2(_size_1(_path), [
            ...
            _hyphenMinusGreaterThanSign_2(
              ...
              sl.annotateFunction(function () {
                ...
                let __SPL550 = _assertIsOfSize_2(
                  _commercialAtAsterisk_2(_path, [1, 2, 3]),
                  3,
                );
                let _category = _at_2(__SPL550, 1);
                let _author = _at_2(__SPL550, 2);
                let _name = _at_2(__SPL550, 3);
                let _url = _format_2(
  ...
  "{ :self :index :path | setPath(ColumnBrowser(self,'SuperCollider Program Browser', 'text/plain', false, false, [1, 1, 3], nil, nil, { :browser :path | caseOf(size(path),[->(0, { categories(index) }), ->(1, { authors(index,at(path, 1)) }), ->(2, { names(index,at(path, 1), at(path, 2)) }), ->(3, { let __SPL550 = assertIsOfSize(@*(path, [1, 2, 3]), 3); let category = at(__SPL550, 1); let author = at(__SPL550, 2); let name = at(__SPL550, 3); let url = format('./lib/spl/Program/SuperCollider/%/% - %.sp',[category, author, name]); fetchTextWithDefault(url,'*Fetch failed*') })]) }),path) }",
);
