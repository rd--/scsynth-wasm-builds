...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :index :path | setPath(ColumnBrowser(self,'SuperCollider Program Browser', 'text/plain', false, false, [1, 1, 3], nil, nil, { :browser :path | caseOf(size(path),[->(0, { categories(index) }), ->(1, { authors(index,at(path, 1)) }), ->(2, { names(index,at(path, 1), at(path, 2)) }), ->(3, { let __SplVar1 = assertIsOfSize(@*(path, [1, 2, 3]), 3); let category = at(__SplVar1, 1); let author = at(__SplVar1, 2); let name = at(__SplVar1, 3); let url = format('./lib/spl/Program/SuperCollider/%/% - %.sp',[category, author, name]); fetchTextWithDefault(url,'*Fetch failed*') })]) }),path) }",
);
