...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :a :b | let __SPL3 = assertIsOfSize(shape(a), 2); let n = at(__SPL3, 1); let p = at(__SPL3, 2); let __SPL4 = assertIsOfSize(shape(b), 2); let m = at(__SPL4, 1); let q = at(__SPL4, 2); let l = List(n, 1); assert({ =(n, m) }); *((/(1, (-(n, 1)))), dot(transpose((-(a, (outer(*,l, mean(a)))))),conjugate((-(a, (outer(*,l, mean(a)))))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL8 = assertIsOfSize(self, 2); let m = at(__SPL8, 1); let n = at(__SPL8, 2); table({ :i :j | /(1, (-(+(i, j), 1))) },Span(1, m, 1), Span(1, n, 1)) }",
);
...
sl.extendTypeOrTraitWithMethod(
      ...
      sl.annotateFunction(function () {
        ...
        return _if_3(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL9 = _assertIsOfSize_2(_catenate_1(_contents_1(_self)), 4);
            let _a = _at_2(__SPL9, 1);
            let _b = _at_2(__SPL9, 2);
            let _c = _at_2(__SPL9, 3);
            let _d = _at_2(__SPL9, 4);
            let _r = _solidus_2(
              1,
  ...
  "{ :self | if(isSquareMatrix(self), { let n = numberOfRows(self); if((=(n, 2)), { let __SPL9 = assertIsOfSize(catenate(contents(self)), 4); let a = at(__SPL9, 1); let b = at(__SPL9, 2); let c = at(__SPL9, 3); let d = at(__SPL9, 4); let r = /(1, (-((*(a, d)), (*(b, c))))); let m = [[d, -(b)], [-(c), a]]; *(r, m) }, { let m = contents(self); let i = identityMatrix(n); let e = rowReduce(((each(++) . (m, i)))); collect(e, { :each | drop(each,n) }) }) }, { error(self,'Sequence>>inverse: matrix not square') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :aBlock:/2 | let __SPL10 = assertIsOfSize(shape(self), 2); let m = at(__SPL10, 1); let n = at(__SPL10, 2); allSatisfy(Span(1, m, 1), { :i | let x = abs(at(at(self, i), i)); let z = sum(collect(Span(1, n, 1), { :j | if((=(j, i)), { 0 }, { abs(at(at(self, i), j)) }) })); aBlock(x, z) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _k) {
    ...
    return _ampersand_2(
      ...
      sl.annotateFunction(function () {
        ...
        let _r = _at_2(__SPL11, 1);
        let _c = _at_2(__SPL11, 2);
        /* Statements */
        return _allSatisfy_2(
  ...
  "{ :self :k | &(isMatrix(self), { let __SPL11 = assertIsOfSize(shape(self), 2); let r = at(__SPL11, 1); let c = at(__SPL11, 2); allSatisfy(to(1,-(r, k)), { :i | allSatisfy(to((+(+(i, 1), k)),c), { :j | isVeryCloseTo(at(at(self, i), j),0) }) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let a = self; let b = conjugateTranspose(a); =(dot(a,b), dot(b,a)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL12 = assertIsOfSize(shape(self), 2); let p = at(__SPL12, 1); let q = at(__SPL12, 2); if((>=(p, q)), { isVeryCloseTo(dot(transpose(self),self),identityMatrix(q)) }, { isVeryCloseTo(dot(self,transpose(self)),identityMatrix(p)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL13 = assertIsOfSize(shape(self), 2); let p = at(__SPL13, 1); let q = at(__SPL13, 2); if((>=(p, q)), { isVeryCloseTo(dot(conjugateTranspose(self),self),identityMatrix(q)) }, { isVeryCloseTo(dot(self,conjugateTranspose(self)),identityMatrix(p)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :k | &(isMatrix(self), { let __SPL14 = assertIsOfSize(shape(self), 2); let r = at(__SPL14, 1); let c = at(__SPL14, 2); allSatisfy(to((-(2, k)),r), { :i | allSatisfy(to(1,+(-(i, 1), k)), { :j | isVeryCloseTo(at(at(self, i), j),0) }) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _k) {
    ...
    let __SPL15 = _assertIsOfSize_2(_shape_1(_m), 2);
    let _r = _at_2(__SPL15, 1);
    let _c = _at_2(__SPL15, 2);
    /* Statements */
    _do_2(
      _to_2(1, _hyphenMinus_2(_r, _k)),
      sl.annotateFunction(function (_i) {
  ...
  "{ :self :k | let m = assertIsMatrix(self,'List>>lowerTriangularize'); let __SPL15 = assertIsOfSize(shape(m), 2); let r = at(__SPL15, 1); let c = at(__SPL15, 2); do(to(1,-(r, k)), { :i | do(to((+(+(i, 1), k)),c), { :j | atPut(at(m, i), j, 0) }) }); m }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL16 = _assertIsOfSize_2(_kuhnMunkresAlgorithm_1(_self), 2);
    let __genSym31 = _at_2(__SPL16, 1);
    let _i = _at_2(__SPL16, 2);
    let _k = _size_1(_i);
    /* Statements */
    return _select_2(
  ...
  "{ :self | let __SPL16 = assertIsOfSize(kuhnMunkresAlgorithm(self), 2); let _ = at(__SPL16, 1); let i = at(__SPL16, 2); let k = size(i); select(transpose([i, Span(1, k, 1)]), { :each | !=(at(each, 1), 0) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SPL18 = assertIsOfSize(shape(self), 2); let m = at(__SPL18, 1); let n = at(__SPL18, 2); determinant(submatrix(self,without(asList(upOrDownTo(1, m)),i), without(asList(upOrDownTo(1, n)),j))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL19 = assertIsOfSize(shape(self), 2); let m = at(__SPL19, 1); let n = at(__SPL19, 2); table({ :i :j | let r = +(-(m, i), 1); let c = +(-(n, j), 1); determinant(submatrix(self,without(asList(upOrDownTo(1, m)),r), without(asList(upOrDownTo(1, n)),c))) },Span(1, m, 1), Span(1, n, 1)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL20 = _assertIsOfSize_2(_singularValueDecomposition_1(_self), 3);
    let _u = _at_2(__SPL20, 1);
    let _s = _at_2(__SPL20, 2);
    let _v = _at_2(__SPL20, 3);
  ...
  "{ :self | let __SPL20 = assertIsOfSize(singularValueDecomposition(self), 3); let u = at(__SPL20, 1); let s = at(__SPL20, 2); let v = at(__SPL20, 3); let i = deepCollect(s, { :x | if(isZero(x), { 0 }, { /(1, x) }) }); dot(dot(v,i),transpose(u)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_data, _dimension, _delay, _metric) {
    ...
    let _f = _at_2(__SPL22, 1);
    let _g = _at_2(__SPL22, 2);
    /* Statements */
    _toDo_3(
      1,
      _n,
  ...
  "{ :data :dimension :delay :metric | let n = -(size(data), (*((-(dimension, 1)), delay))); let m = reshape([0],[n, n]); let __SPL22 = assertIsOfSize(caseOf(metric,[->('Manhattan', { [abs:/1, sum:/1] }), ->('Euclidean', { [square:/1, { :x | sqrt(sum(x)) }] }), ->('Supremum', { [abs:/1, max:/1] })]), 2); let f = at(__SPL22, 1); let g = at(__SPL22, 2); toDo(1, n, { :i | toDo(i, n, { :j | let x = collect(upOrDownTo(0, -(dimension, 1)), { :k | value(f,-(at(data, +(i, (*(k, delay)))), at(data, +(j, (*(k, delay)))))) }); atPut(at(m, i), j, atPut(at(m, j), i, value(g,x))) }) }); m }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _tolerance) {
    ...
    let _a = _self;
    let __SPL24 = _assertIsOfSize_2(_shape_1(_a), 2);
    ...
    _whileTrue_2(
      ...
      sl.annotateFunction(function () {
        ...
        sl.annotateFunction(function () {
          ...
          let __SPL26 = _qrDecomposition_1(_conjugateTranspose_1(_s));
          /* Statements */
          _q = _at_2(__SPL26, 1);
          return _s = _at_2(__SPL26, 2);
        }, [])();
        _v = _dot_2(_v, _q);
  ...
  "{ :self :tolerance | let a = self; let __SPL24 = assertIsOfSize(shape(a), 2); let m = at(__SPL24, 1); let n = at(__SPL24, 2); let loopMax = *(100, max(m,n)); let loopCount = 0; let u = identityMatrix(m); let s = conjugateTranspose(a); let v = identityMatrix(n); let err = Infinity; let ss = nil; whileTrue({ &(>(err, tolerance), { <(loopCount, loopMax) }) }, { var q, e, f;({ let __SPL25 = qrDecomposition(conjugateTranspose(s)); q := at(__SPL25, 1); s := at(__SPL25, 2) } . ()); u := dot(u,q); ({ let __SPL26 = qrDecomposition(conjugateTranspose(s)); q := at(__SPL26, 1); s := at(__SPL26, 2) } . ()); v := dot(v,q); e := norm(catenate(upperTriangularize(deepCopy(s),1))); f := norm(diagonal(s)); err := if((||(=(f, 0), (=(f, 1)))), { 0 }, { /(e, f) }); loopCount := +(loopCount, 1) }); ss := diagonal(s); s := reshape([0],[m, n]); toDo(1, size(ss), { :n | let ssn = at(ss, n); atPut(at(s, n), n, abs(ssn)); ifTrue((<(ssn, 0)), { toDo(1, m, { :i | atPut(at(u, i), n, negate(at(at(u, i), n))) }) }) }); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL27 = _assertIsOfSize_2(
      _singularValueDecompositionGolubReinsch_1(_self),
      3,
    );
    let _u = _at_2(__SPL27, 1);
    let _q = _at_2(__SPL27, 2);
    let _v = _at_2(__SPL27, 3);
    let _s = _diagonalMatrix_3(_q, 0, _shape_1(_self));
    /* Statements */
    return [_u, _s, _v];
  }, ["self"]),
  "{ :self | let __SPL27 = assertIsOfSize(singularValueDecompositionGolubReinsch(self), 3); let u = at(__SPL27, 1); let q = at(__SPL27, 2); let v = at(__SPL27, 3); let s = diagonalMatrix(q,0, shape(self)); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SPL28 = assertIsOfSize(shape(self), 2); let m = at(__SPL28, 1); let n = at(__SPL28, 2); toDo(1, m, { :k | let x = at(at(self, k), i); atPut(at(self, k), i, at(at(self, k), j)); atPut(at(self, k), j, x) }); self }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SPL29 = assertIsOfSize(shape(self), 2); let m = at(__SPL29, 1); let n = at(__SPL29, 2); swapWith(self,i, j) }",
);
