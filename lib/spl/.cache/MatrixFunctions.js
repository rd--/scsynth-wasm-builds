...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(shape(self), 2); let m = at(__SplVar1, 1); let n = at(__SplVar1, 2); let answer = []; downToDo(n, 1, { :i | let row = []; downToDo(m, 1, { :j | add(row,at(at(self, j), i)) }); add(answer,row) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :a | let __SplVar2 = assertIsOfSize(shape(a), 2); let m = at(__SplVar2, 1); let n = at(__SplVar2, 2); let l = zeroMatrix([m, n]); toDo(1, m, { :i | toDo(1, i, { :k | let sum = 0; toDo(1, k, { :j | sum := +(sum, (*(at(at(l, i), j), at(at(l, k), j)))) }); atPut(at(l, i), k, if((=(i, k)), { sqrt((-(at(at(a, i), i), sum))) }, { *(/(1, at(at(l, k), k)), (-(at(at(a, i), k), sum))) })) }) }); l }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :a :b | let __SplVar3 = assertIsOfSize(shape(a), 2); let n = at(__SplVar3, 1); let p = at(__SplVar3, 2); let __SplVar4 = assertIsOfSize(shape(b), 2); let m = at(__SplVar4, 1); let q = at(__SplVar4, 2); let l = List(n, 1); assert({ =(n, m) }); *((/(1, (-(n, 1)))), dot(transpose((-(a, (outer(*,l, mean(a)))))),conjugate((-(a, (outer(*,l, mean(a)))))))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :m :epsilon :n | let x = deepCopy(m); let i = 0; whileTrue({ &(not(isUpperTriangularMatrix(x)), { <(i, n) }) }, { let __SplVar5 = assertIsOfSize(qrDecomposition(x), 2); let q = at(__SplVar5, 1); let r = at(__SplVar5, 2); x := dot(r,q); i := +(i, 1) }); diagonal(x) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let a = deepCopy(self); let __SplVar7 = assertIsOfSize(shape(a), 2); let n = at(__SplVar7, 1); let m = at(__SplVar7, 2); toDo(1, n, { :k | atPut(a, k, normalize(at(a, k))); toDo((+(k, 1)), n, { :j | atPut(a, j, -(at(a, j), (*(dot(at(a, j),at(a, k)), at(a, k))))) }) }); a }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar8 = assertIsOfSize(self, 2); let m = at(__SplVar8, 1); let n = at(__SplVar8, 2); table({ :i :j | /(1, (-(+(i, j), 1))) },Span(1, m, 1), Span(1, n, 1)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(isSquareMatrix(self), { let n = numberOfRows(self); if((=(n, 2)), { let __SplVar9 = assertIsOfSize(catenate(contents(self)), 4); let a = at(__SplVar9, 1); let b = at(__SplVar9, 2); let c = at(__SplVar9, 3); let d = at(__SplVar9, 4); let r = /(1, (-((*(a, d)), (*(b, c))))); let m = [[d, -(b)], [-(c), a]]; *(r, m) }, { let m = contents(self); let i = identityMatrix(n); let e = rowReduce(((each(++) . (m, i)))); collect(e, { :each | drop(each,n) }) }) }, { error(self,'Sequence>>inverse: matrix not square') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :aBlock:/2 | let __SplVar10 = assertIsOfSize(shape(self), 2); let m = at(__SplVar10, 1); let n = at(__SplVar10, 2); allSatisfy(Span(1, m, 1), { :i | let x = abs(at(at(self, i), i)); let z = sum(collect(Span(1, n, 1), { :j | if((=(j, i)), { 0 }, { abs(at(at(self, i), j)) }) })); aBlock(x, z) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :k | &(isMatrix(self), { let __SplVar11 = assertIsOfSize(shape(self), 2); let r = at(__SplVar11, 1); let c = at(__SplVar11, 2); allSatisfy(to(1,-(r, k)), { :i | allSatisfy(to((+(+(i, 1), k)),c), { :j | isVeryCloseTo(at(at(self, i), j),0) }) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar12 = assertIsOfSize(shape(self), 2); let p = at(__SplVar12, 1); let q = at(__SplVar12, 2); if((>=(p, q)), { isVeryCloseTo(dot(transpose(self),self),identityMatrix(q)) }, { isVeryCloseTo(dot(self,transpose(self)),identityMatrix(p)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar13 = assertIsOfSize(shape(self), 2); let p = at(__SplVar13, 1); let q = at(__SplVar13, 2); if((>=(p, q)), { isVeryCloseTo(dot(conjugateTranspose(self),self),identityMatrix(q)) }, { isVeryCloseTo(dot(self,conjugateTranspose(self)),identityMatrix(p)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :k | &(isMatrix(self), { let __SplVar14 = assertIsOfSize(shape(self), 2); let r = at(__SplVar14, 1); let c = at(__SplVar14, 2); allSatisfy(to((-(2, k)),r), { :i | allSatisfy(to(1,+(-(i, 1), k)), { :j | isVeryCloseTo(at(at(self, i), j),0) }) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :k | let m = assertIsMatrix(self,'List>>lowerTriangularize'); let __SplVar15 = assertIsOfSize(shape(m), 2); let r = at(__SplVar15, 1); let c = at(__SplVar15, 2); do(to(1,-(r, k)), { :i | do(to((+(+(i, 1), k)),c), { :j | atPut(at(m, i), j, 0) }) }); m }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar16 = assertIsOfSize(kuhnMunkresAlgorithm(self), 2); let _ = at(__SplVar16, 1); let i = at(__SplVar16, 2); let k = size(i); select(transpose([i, Span(1, k, 1)]), { :each | !=(at(each, 1), 0) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SplVar18 = assertIsOfSize(shape(self), 2); let m = at(__SplVar18, 1); let n = at(__SplVar18, 2); determinant(submatrix(self,without(asList(upOrDownTo(1, m)),i), without(asList(upOrDownTo(1, n)),j))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar19 = assertIsOfSize(shape(self), 2); let m = at(__SplVar19, 1); let n = at(__SplVar19, 2); table({ :i :j | let r = +(-(m, i), 1); let c = +(-(n, j), 1); determinant(submatrix(self,without(asList(upOrDownTo(1, m)),r), without(asList(upOrDownTo(1, n)),c))) },Span(1, m, 1), Span(1, n, 1)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar20 = assertIsOfSize(singularValueDecomposition(self), 3); let u = at(__SplVar20, 1); let s = at(__SplVar20, 2); let v = at(__SplVar20, 3); let i = deepCollect(s, { :x | if(isZero(x), { 0 }, { /(1, x) }) }); dot(dot(v,i),transpose(u)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :data :dimension :delay :metric | let n = -(size(data), (*((-(dimension, 1)), delay))); let m = reshape([0],[n, n]); let __SplVar22 = assertIsOfSize(caseOf(metric,[->('Manhattan', { [abs:/1, sum:/1] }), ->('Euclidean', { [square:/1, { :x | sqrt(sum(x)) }] }), ->('Supremum', { [abs:/1, max:/1] })]), 2); let f = at(__SplVar22, 1); let g = at(__SplVar22, 2); toDo(1, n, { :i | toDo(i, n, { :j | let x = collect(upOrDownTo(0, -(dimension, 1)), { :k | value(f,-(at(data, +(i, (*(k, delay)))), at(data, +(j, (*(k, delay)))))) }); atPut(at(m, i), j, atPut(at(m, j), i, value(g,x))) }) }); m }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :tolerance | let a = self; let __SplVar24 = assertIsOfSize(shape(a), 2); let m = at(__SplVar24, 1); let n = at(__SplVar24, 2); let loopMax = *(100, max(m,n)); let loopCount = 0; let u = identityMatrix(m); let s = conjugateTranspose(a); let v = identityMatrix(n); let err = Infinity; let ss = nil; whileTrue({ &(>(err, tolerance), { <(loopCount, loopMax) }) }, { var q, e, f;({ let __SplVar25 = qrDecomposition(conjugateTranspose(s)); q := at(__SplVar25, 1); s := at(__SplVar25, 2) } . ()); u := dot(u,q); ({ let __SplVar26 = qrDecomposition(conjugateTranspose(s)); q := at(__SplVar26, 1); s := at(__SplVar26, 2) } . ()); v := dot(v,q); e := norm(catenate(upperTriangularize(deepCopy(s),1))); f := norm(diagonal(s)); err := if((||(=(f, 0), (=(f, 1)))), { 0 }, { /(e, f) }); loopCount := +(loopCount, 1) }); ss := diagonal(s); s := reshape([0],[m, n]); toDo(1, size(ss), { :n | let ssn = at(ss, n); atPut(at(s, n), n, abs(ssn)); ifTrue((<(ssn, 0)), { toDo(1, m, { :i | atPut(at(u, i), n, negate(at(at(u, i), n))) }) }) }); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar27 = assertIsOfSize(singularValueDecompositionGolubReinsch(self), 3); let u = at(__SplVar27, 1); let q = at(__SplVar27, 2); let v = at(__SplVar27, 3); let s = diagonalMatrix(q,0, shape(self)); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SplVar28 = assertIsOfSize(shape(self), 2); let m = at(__SplVar28, 1); let n = at(__SplVar28, 2); toDo(1, m, { :k | let x = at(at(self, k), i); atPut(at(self, k), i, at(at(self, k), j)); atPut(at(self, k), j, x) }); self }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SplVar29 = assertIsOfSize(shape(self), 2); let m = at(__SplVar29, 1); let n = at(__SplVar29, 2); swapWith(self,i, j) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :k | let m = assertIsMatrix(self,'List>>upperTriangularize'); let __SplVar30 = assertIsOfSize(shape(m), 2); let r = at(__SplVar30, 1); let c = at(__SplVar30, 2); toDo((-(2, k)), r, { :i | toDo(1, +(-(i, 1), k), { :j | atPut(at(m, i), j, 0) }) }); m }",
);
