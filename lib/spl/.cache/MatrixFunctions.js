...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __genSym29 = _at_2(__SPL15, 1);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _tolerance) {
    ...
    let _a = _self;
    let __SPL22 = _assertIsOfSize_2(_shape_1(_a), 2);
    ...
    _whileTrue_2(
      ...
      sl.annotateFunction(function () {
        ...
        sl.annotateFunction(function () {
          ...
          let __SPL23 = _qrDecomposition_1(_conjugateTranspose_1(_s));
          /* Statements */
          _q = _at_2(__SPL23, 1);
          return _s = _at_2(__SPL23, 2);
        }, [])();
        _u = _dot_2(_u, _q);
        sl.annotateFunction(function () {
          /* ArityCheck */
  ...
  "{ :self :tolerance | let a = self; let __SPL22 = assertIsOfSize(shape(a), 2); let m = at(__SPL22, 1); let n = at(__SPL22, 2); let loopMax = *(100, max(m,n)); let loopCount = 0; let u = identityMatrix(m); let s = conjugateTranspose(a); let v = identityMatrix(n); let err = Infinity; let ss = nil; whileTrue({ &(>(err, tolerance), { <(loopCount, loopMax) }) }, { var q, e, f;({ let __SPL23 = qrDecomposition(conjugateTranspose(s)); q := at(__SPL23, 1); s := at(__SPL23, 2) } . ()); u := dot(u,q); ({ let __SPL24 = qrDecomposition(conjugateTranspose(s)); q := at(__SPL24, 1); s := at(__SPL24, 2) } . ()); v := dot(v,q); e := norm(catenate(upperTriangularize(deepCopy(s),1))); f := norm(diagonal(s)); err := if((||(=(f, 0), (=(f, 1)))), { 0 }, { /(e, f) }); loopCount := +(loopCount, 1) }); ss := diagonal(s); s := reshape([0],[m, n]); toDo(1, size(ss), { :n | let ssn = at(ss, n); atPut(at(s, n), n, abs(ssn)); ifTrue((<(ssn, 0)), { toDo(1, m, { :i | atPut(at(u, i), n, negated(at(at(u, i), n))) }) }) }); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL25 = _assertIsOfSize_2(
      _singularValueDecompositionGolubReinsch_1(_self),
      3,
    );
    let _u = _at_2(__SPL25, 1);
    let _q = _at_2(__SPL25, 2);
    let _v = _at_2(__SPL25, 3);
    let _s = _diagonalMatrix_3(_q, 0, _shape_1(_self));
    /* Statements */
    return [_u, _s, _v];
  }, ["self"]),
  "{ :self | let __SPL25 = assertIsOfSize(singularValueDecompositionGolubReinsch(self), 3); let u = at(__SPL25, 1); let q = at(__SPL25, 2); let v = at(__SPL25, 3); let s = diagonalMatrix(q,0, shape(self)); [u, s, v] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SPL26 = assertIsOfSize(shape(self), 2); let m = at(__SPL26, 1); let n = at(__SPL26, 2); toDo(1, m, { :k | let x = at(at(self, k), i); atPut(at(self, k), i, at(at(self, k), j)); atPut(at(self, k), j, x) }); self }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :i :j | let __SPL27 = assertIsOfSize(shape(self), 2); let m = at(__SPL27, 1); let n = at(__SPL27, 2); swapWith(self,i, j) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _k) {
    ...
    let __SPL28 = _assertIsOfSize_2(_shape_1(_m), 2);
    let _r = _at_2(__SPL28, 1);
    let _c = _at_2(__SPL28, 2);
    /* Statements */
    _toDo_3(
      _hyphenMinus_2(2, _k),
      _r,
  ...
  "{ :self :k | let m = assertIsMatrix(self,'List>>upperTriangularize'); let __SPL28 = assertIsOfSize(shape(m), 2); let r = at(__SPL28, 1); let c = at(__SPL28, 2); toDo((-(2, k)), r, { :i | toDo(1, +(-(i, 1), k), { :j | atPut(at(m, i), j, 0) }) }); m }",
);
