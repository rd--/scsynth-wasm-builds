...
sl.addMethodToExistingType(
  ...
  "{ :self :p :o | if(isCollection(p), { collect(p, { :each | quantile(self,each, o) }) }, { let y = self; let n = size(y); if((=(p, 0)), { at(y, 1) }, { if((=(p, 1)), { at(y, n) }, { let __SplVar1 = assertIsOfSize(at(o, 1), 2); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); let __SplVar2 = assertIsOfSize(at(o, 2), 2); let c = at(__SplVar2, 1); let d = at(__SplVar2, 2); let r = +(a, (*((+(n, b)), p))); let f = fractionalPart(r); let i0 = max(floor(r),1); let i1 = min(ceiling(r),n); +(at(y, i0), (*((-(at(y, i1), at(y, i0))), (+(c, (*(d, f))))))) }) }) }) }",
);
