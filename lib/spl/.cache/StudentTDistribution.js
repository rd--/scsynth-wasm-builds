...
sl.addMethodToExistingType(
  ...
  "{ :self | let mu = mu(self); let sigma = sigma(self); let nu = nu(self); { :x | let a = *(square(sigma), nu); let b = square((-(x, mu))); let t = /(a, (+(a, b))); let r = *((/(1, 2)), betaRegularized(t,/(nu, 2), /(1, 2))); if((<=(x, mu)), { r }, { -(1, r) }) } }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let mu = mu(self); let sigma = sigma(self); let nu = nu(self); { :x | let d = /(square((-(x, mu))), square(sigma)); /((^((/(nu, (+(d, nu)))), (/((+(nu, 1)), 2)))), (*(*(sigma, sqrt(nu)), beta((/(nu, 2)),/(1, 2))))) } }",
);
