sl.extendTypeOrTraitWithMethod(
  ...
  "{ :a :v | if(isVector(v), { let __SplVar1 = assertIsOfSize(v, 2); let sigma = at(__SplVar1, 1); let tau = at(__SplVar1, 2); let x = *(a, (/(sinh(tau), (-(cosh(tau), cos(sigma)))))); let y = *(a, (/(sin(sigma), (-(cosh(tau), cos(sigma)))))); [x, y] }, { collect(v, { :each | fromBipolarCoordinates(a, each) }) }) }",
);
