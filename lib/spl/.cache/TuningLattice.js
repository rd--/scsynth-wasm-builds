...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :primes :unitVector | let primesList = tuningLatticeVertexCoordinates(self,primes); let edgeList = tuningLatticeEdges(self,primesList); let coordinateList = collect(primesList, { :each | let v = first(unitVector,size(each)); sum(withCollect(each, v, { :count :unit | *(count, unit) })) }); vertexCoordinates(Graph(asList(upOrDownTo(1, size(primesList))), edgeList),coordinateList) }",
);
