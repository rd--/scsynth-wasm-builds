...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :primes | if(includesAllOf(primes,latticePrimes(self,includes(primes,2))), { join(collect(latticeVector(self,primes), { :each | padLeft(asString(each),2, ' ') }),' ') }, { '*' }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self, _vertexCoordinates) {
    ...
    _combinationsAtATimeDo_3(_indices, 2, function (_each) {
      ...
      let __SPL217 = _assertIsOfSize_2(_each, 2);
      let _i = _at_2(__SPL217, 1);
      let _j = _at_2(__SPL217, 2);
      /* Statements */
      return _ifTrue_2(
        _equalsSign_2(
  ...
  "{ :self :vertexCoordinates | let indices = iota(size(self)); let answer = []; combinationsAtATimeDo(indices, 2, { :each | let __SPL217 = assertIsOfSize(each, 2); let i = at(__SPL217, 1); let j = at(__SPL217, 2); ifTrue((=(manhattanDistance(at(vertexCoordinates, i),at(vertexCoordinates, j)), 1)), { add(answer,copy(each)) }) }); answer }",
);
