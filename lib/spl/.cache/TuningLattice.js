...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _vertexCoordinates) {
    ...
    _combinationsAtATimeDo_3(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL399 = _assertIsOfSize_2(_each, 2);
        let _i = _at_2(__SPL399, 1);
        let _j = _at_2(__SPL399, 2);
        /* Statements */
        return _ifTrue_2(
          _equalsSign_2(
  ...
  "{ :self :vertexCoordinates | let indices = iota(size(self)); let answer = []; combinationsAtATimeDo(indices, 2, { :each | let __SPL399 = assertIsOfSize(each, 2); let i = at(__SPL399, 1); let j = at(__SPL399, 2); ifTrue((=(manhattanDistance(at(vertexCoordinates, i),at(vertexCoordinates, j)), 1)), { add(answer,copy(each)) }) }); answer }",
);
