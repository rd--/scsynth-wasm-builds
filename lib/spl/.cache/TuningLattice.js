...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :primes | if(includesAllOf(primes,latticePrimes(self,includes(primes,2))), { let pf1 = primeFactors(numerator(self)); let pf2 = negate(primeFactors(denominator(self))); let pf3 = ++(pf1, pf2); collect(primes, { :each | -(occurrencesOf(pf3,each), occurrencesOf(pf3,negate(each))) }) }, { error(self,'Fraction>>latticeVector: incomplete primes') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _vertexCoordinates) {
    ...
    _combinationsAtATimeDo_3(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL486 = _assertIsOfSize_2(_each, 2);
        let _i = _at_2(__SPL486, 1);
        let _j = _at_2(__SPL486, 2);
        /* Statements */
        return _ifTrue_2(
          _equalsSign_2(
  ...
  "{ :self :vertexCoordinates | let indices = iota(size(self)); let answer = []; combinationsAtATimeDo(indices, 2, { :each | let __SPL486 = assertIsOfSize(each, 2); let i = at(__SPL486, 1); let j = at(__SPL486, 2); ifTrue((=(manhattanDistance(at(vertexCoordinates, i),at(vertexCoordinates, j)), 1)), { add(answer,copy(each)) }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :primes | let upperLimit = last(primes); if((<=(primeLimit(self), upperLimit)), { primes }, { let tuningPrimes = latticePrimes(self,false); if((<=(size(tuningPrimes), size(primes))), { tuningPrimes }, { nil }) }) }",
);
