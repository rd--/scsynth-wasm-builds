...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :vertexCoordinates | let indices = iota(size(self)); let answer = []; combinationsAtATimeDo(indices, 2, { :each | let __SPL5 = assertIsOfSize(each, 2); let i = at(__SPL5, 1); let j = at(__SPL5, 2); ifTrue((=(manhattanDistance(at(vertexCoordinates, i),at(vertexCoordinates, j)), 1)), { add(answer,copy(each)) }) }); answer }",
);
