...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let one = one(self); if(isInteger(self), { product(collect(upOrDownTo(one, self), { :k | ^(k, k) })) }, { error('@Integer>>hyperfactorial: not implemented for non-integer') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_n, _k) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL1 = _assertIsOfSize_2(_k, 1);
  ...
  "{ :n :k | if(isList(k), { let __SPL1 = assertIsOfSize(k, 1); let m = at(__SPL1, 1); /(sum(divisors(n), { :d | *(*(eulerPhi(d), stirlingS2(/(n, d), m)), factorial(m)) }), n) }, { sum(divisors(n), { :d | /(*(eulerPhi(d), (^(k, (/(n, d))))), n) }) }) }",
);
