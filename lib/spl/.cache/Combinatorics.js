...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_n, _k) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL67 = _assertIsOfSize_2(_k, 1);
        let _m = _at_2(__SPL67, 1);
        /* Statements */
        return _solidus_2(
          _sum_2(
            _divisors_1(_n),
  ...
  "{ :n :k | if(isList(k), { let __SPL67 = assertIsOfSize(k, 1); let m = at(__SPL67, 1); /(sum(divisors(n), { :d | *(*(eulerPhi(d), stirlingS2(/(n, d), m)), factorial(m)) }), n) }, { sum(divisors(n), { :d | /(*(eulerPhi(d), (^(k, (/(n, d))))), n) }) }) }",
);
