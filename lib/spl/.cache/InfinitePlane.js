...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(terms(self), 4); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); let c = at(__SplVar1, 3); let d = at(__SplVar1, 4); let z = sqrt((+(+((*(a, a)), (*(b, b))), (*(c, c))))); let n = /([a, b, c], z); let p = /(d, z); [n, p] }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if((=(size(self), 4)), { let __SplVar2 = assertIsOfSize(self, 4); let a = at(__SplVar2, 1); let b = at(__SplVar2, 2); let c = at(__SplVar2, 3); let d = at(__SplVar2, 4); InfinitePlane(a, b, c, d) }, { if((=(size(self), 3)), { let __SplVar3 = assertIsOfSize(self, 3); let p1 = at(__SplVar3, 1); let p2 = at(__SplVar3, 2); let p3 = at(__SplVar3, 3); InfinitePlane(p1, p2, p2) }, { error(self,'List>>asInfinitePlane') }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :p1 :p2 :p3 | let __SplVar4 = assertIsOfSize(p1, 3); let x1 = at(__SplVar4, 1); let y1 = at(__SplVar4, 2); let z1 = at(__SplVar4, 3); let __SplVar5 = assertIsOfSize(p2, 3); let x2 = at(__SplVar5, 1); let y2 = at(__SplVar5, 2); let z2 = at(__SplVar5, 3); let __SplVar6 = assertIsOfSize(p3, 3); let x3 = at(__SplVar6, 1); let y3 = at(__SplVar6, 2); let z3 = at(__SplVar6, 3); let a = +(+((*(y1, (-(z2, z3)))), (*(y2, (-(z3, z1))))), (*(y3, (-(z1, z2))))); let b = +(+((*(z1, (-(x2, x3)))), (*(z2, (-(x3, x1))))), (*(z3, (-(x1, x2))))); let c = +(+((*(x1, (-(y2, y3)))), (*(x2, (-(y3, y1))))), (*(x3, (-(y1, y2))))); let d = negate(sum((*([a, b, c], p1)))); ifTrue(allSatisfy([a, b, c],isInteger:/1), { let z = gcd([a, b, c]); ({ let __SplVar7 = /([a, b, c, d], z); a := at(__SplVar7, 1); b := at(__SplVar7, 2); c := at(__SplVar7, 3); d := at(__SplVar7, 4) } . ()) }); ifTrue(allSatisfy([a, b, c],isNonPositive:/1), { ({ let __SplVar8 = negate([a, b, c, d]); a := at(__SplVar8, 1); b := at(__SplVar8, 2); c := at(__SplVar8, 3); d := at(__SplVar8, 4) } . ()) }); InfinitePlane(a, b, c, d) }",
);
