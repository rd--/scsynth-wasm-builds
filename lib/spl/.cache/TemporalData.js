...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let t = []; let v = []; do(self, { :each | let __SplVar1 = assertIsOfSize(transpose(each), 2); let i = at(__SplVar1, 1); let j = at(__SplVar1, 2); add(t,i); add(v,j) }); TemporalData(v, t) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :aBlock:/1 :timeSpecificationList | TemporalData(collect(timeSpecificationList, { :timeSpecification | let __SplVar2 = assertIsOfSize(timeSpecification, 3); let tMin = at(__SplVar2, 1); let tMax = at(__SplVar2, 2); let tStep = at(__SplVar2, 3); let timeList = asList(thenTo(tMin, +(tMin, tStep), tMax)); transpose([timeList, aBlock(timeList)]) })) }",
);
