...
sl.addMethodToExistingType(
  ...
  "{ :self | let v = valueList(self); let t = timeList(self); if((=(size(t), 1)), { TimeSeries(at(v, 1), at(t, 1)) }, { if(allEqual(t), { TimeSeries(transpose(valueList(self)), anyOne(timeList(self))) }, { error(self,'asTimeSeries: non-uniform timeList') }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | withCollect(timeList(self), valueList(self), { :i :j | transpose([i, j]) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _do_2(
      ...
      sl.annotateFunction(function (_each) {
        ...
        let __SPL15 = _assertIsOfSize_2(_transpose_1(_each), 2);
  ...
  "{ :self | let t = []; let v = []; do(self, { :each | let __SPL15 = assertIsOfSize(transpose(each), 2); let i = at(__SPL15, 1); let j = at(__SPL15, 2); add(t,i); add(v,j) }); TemporalData(v, t) }",
);
...
sl.extendTypeOrTraitWithMethod(
    ...
    return _TemporalData_1(
      _collect_2(
        ...
        sl.annotateFunction(function (_timeSpecification) {
          ...
          let __SPL16 = _assertIsOfSize_2(_timeSpecification, 3);
          let _tMin = _at_2(__SPL16, 1);
          let _tMax = _at_2(__SPL16, 2);
          let _tStep = _at_2(__SPL16, 3);
  ...
  "{ :aBlock:/1 :timeSpecificationList | TemporalData(collect(timeSpecificationList, { :timeSpecification | let __SPL16 = assertIsOfSize(timeSpecification, 3); let tMin = at(__SPL16, 1); let tMax = at(__SPL16, 2); let tStep = at(__SPL16, 3); let timeList = asList(thenTo(tMin, +(tMin, tStep), tMax)); transpose([timeList, aBlock(timeList)]) })) }",
);
