    ...
    let __SPL1 = _assertIsOfSize_2(
      _transposed_1(_collect_2(
        ...
        sl.annotateFunction(function (_each) {
          ...
          return _transposed_1(_each);
        }, ["each"]),
      )),
      2,
    );
    let _t = _at_2(__SPL1, 1);
    let _v = _at_2(__SPL1, 2);
    /* Statements */
    return _TemporalData_2(_v, _t);
  }, ["self"]),
  "{ :self | let __SPL1 = assertIsOfSize(transposed(collect(self, { :each | transposed(each) })), 2); let t = at(__SPL1, 1); let v = at(__SPL1, 2); TemporalData(v, t) }",
);
...
sl.extendTypeOrTraitWithMethod(
    ...
    return _TemporalData_1(
      _collect_2(
        ...
        sl.annotateFunction(function (_timeSpecification) {
          ...
          let __SPL2 = _assertIsOfSize_2(_timeSpecification, 3);
          let _tMin = _at_2(__SPL2, 1);
          let _tMax = _at_2(__SPL2, 2);
          let _tStep = _at_2(__SPL2, 3);
  ...
  "{ :aBlock:/1 :timeSpecificationList | TemporalData(collect(timeSpecificationList, { :timeSpecification | let __SPL2 = assertIsOfSize(timeSpecification, 3); let tMin = at(__SPL2, 1); let tMax = at(__SPL2, 2); let tStep = at(__SPL2, 3); let timeList = asList(thenTo(tMin, +(tMin, tStep), tMax)); transposed([timeList, aBlock(timeList)]) })) }",
);
