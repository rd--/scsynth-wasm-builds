...
sl.addMethodToExistingType(
  ...
  "{ :self :subject :event :delay :aBlock:/0 | let timerId = valueEveryMilliseconds(aBlock:/0,*(asSeconds(delay), 1000)); let frame = addFrame(self,subject, event); addEventListener(frame, 'close', { :unusedEvent | cancel(timerId) }); frame }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :subject :event | let frame = Frame(self, subject); frame(subject, frame); zIndex(frame, +(max(zIndices(self)), 1)); ifNotNil(event, { moveTo(frame,x(event), y(event)) }); basicInclude(frameSet(self),frame); appendChild(container(self),outerElement(frame)); frame }",
);
