...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _Span_3(_start_1(_self), _stop_1(_self), _step_1(_self));
  }, ["self"]),
  "{ :self | Span(start(self), stop(self), step(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :operand :aBlock:/2 | &(isRange(operand), { &(aBlock(start(self), start(operand)), { &(aBlock(stop(self), stop(operand)), { aBlock(step(self), step(operand)) }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | Range(negate(start(self)), negate(stop(self)), negate(step(self))) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | if((&(=(step(self), 1), { not(isEmpty(self)) })), { format('%:%',[start(self), stop(self)]) }, { format('%:%:%',[printString(start(self)), printString(step(self)), printString(stop(self))]) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let start = start(self); start(self, last(self)); stop(self, start); step(self, -(0, step(self))); self }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | ifTrue((<(step(self), 0)), { reverseInPlace(self) }); self }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(self, 3); let start = at(__SplVar1, 1); let stop = at(__SplVar1, 2); let step = at(__SplVar1, 3); Range(start, stop, step) }",
);
