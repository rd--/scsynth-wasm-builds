...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let _start = _at_2(__SPL1, 1);
    let _stop = _at_2(__SPL1, 2);
    let _step = _at_2(__SPL1, 3);
    /* Statements */
    return _Range_3(_start, _stop, _step);
  }, ["self"]),
  "{ :self | let __SPL1 = assertIsOfSize(self, 3); let start = at(__SPL1, 1); let stop = at(__SPL1, 2); let step = at(__SPL1, 3); Range(start, stop, step) }",
);
