...
sl.addMethodToExistingType(
  ...
  "{ :self | let c = 1; let __SPL1 = assertIsOfSize(center(self), 2); let r = at(__SPL1, 1); let i = at(__SPL1, 2); let alpha = Complex(negate(r), i); let d = -(square(abs(alpha)), square(radius(self))); GeneralisedCircle(c, alpha, d) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SPL2 = assertIsOfSize(center(self), 2); let a = at(__SPL2, 1); let b = at(__SPL2, 2); let rSquared = square(radius(self)); { :x :y | -(+(square((-(x, a))), square((-(y, b)))), rSquared) } }",
);
...
sl.extendTypeOrTraitWithMethod(
        ...
        return _caseOf_2(_k, [
          ...
          _hyphenMinusGreaterThanSign_2(
            ...
            sl.annotateFunction(function () {
              ...
              let _z1 = _at_2(__SPL4, 1);
              let _z2 = _at_2(__SPL4, 2);
              let _z3 = _at_2(__SPL4, 3);
              let _w = _solidus_2(
  ...
  "{ :self | let k = size(self); if((|(<(k, 2), { >(k, 3) })), { error(self,'circleThrough') }, { caseOf(k,[->(2, { let __SPL3 = assertIsOfSize(self, 2); let a = at(__SPL3, 1); let b = at(__SPL3, 2); let c = /((+(a, b)), 2); let r = /(euclideanDistance(a,b), 2); Circle(c, r) }), ->(3, { let __SPL4 = assertIsOfSize(collect(self,asComplex:/1), 3); let z1 = at(__SPL4, 1); let z2 = at(__SPL4, 2); let z3 = at(__SPL4, 3); let w = /((-(z3, z1)), (-(z2, z1))); let c = +(/(*((-(z2, z1)), (-(w, (^(abs(w), 2))))), (*(Complex(0, 2), imaginary(w)))), z1); let r = abs((-(z1, c))); Circle(asList(c), r) })]) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL5 = _assertIsOfSize_2(_asList_1(_self), 2);
    let _h = _at_2(__SPL5, 1);
    let _k = _at_2(__SPL5, 2);
    let _r = _solidus_2(1, _asterisk_2(2, _square_1(_k)));
    /* Statements */
    return _Circle_2([_solidus_2(_h, _k), _r], _r);
  }, ["self"]),
  "{ :self | let __SPL5 = assertIsOfSize(asList(self), 2); let h = at(__SPL5, 1); let k = at(__SPL5, 2); let r = /(1, (*(2, square(k)))); Circle([/(h, k), r], r) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _reference) {
    ...
    let __SPL6 = _assertIsOfSize_2(_center_1(_self), 2);
    let _x = _at_2(__SPL6, 1);
    let _y = _at_2(__SPL6, 2);
    let _a = _radius_1(_self);
    let __SPL7 = _assertIsOfSize_2(_center_1(_reference), 2);
  ...
  "{ :self :reference | let __SPL6 = assertIsOfSize(center(self), 2); let x = at(__SPL6, 1); let y = at(__SPL6, 2); let a = radius(self); let __SPL7 = assertIsOfSize(center(reference), 2); let x0 = at(__SPL7, 1); let y0 = at(__SPL7, 2); let k = radius(reference); let s = /(square(k), (-(+(square((-(x, x0))), square((-(y, y0)))), square(a)))); Circle([+(x0, (*(s, (-(x, x0))))), +(y0, (*(s, (-(y, y0)))))], *(abs(s), a)) }",
);
