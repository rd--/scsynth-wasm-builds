...
sl.extendTypeOrTraitWithMethod(
        ...
        return _caseOf_2(_k, [
          ...
          _hyphenMinusGreaterThanSign_2(
            ...
            sl.annotateFunction(function () {
              ...
              let _z1 = _at_2(__SPL2, 1);
              let _z2 = _at_2(__SPL2, 2);
              let _z3 = _at_2(__SPL2, 3);
              let _w = _solidus_2(
  ...
  "{ :self | let k = size(self); if((|(<(k, 2), { >(k, 3) })), { error(self,'circleThrough') }, { caseOf(k,[->(2, { let __SPL1 = assertIsOfSize(self, 2); let a = at(__SPL1, 1); let b = at(__SPL1, 2); let c = /((+(a, b)), 2); let r = /(euclideanDistance(a,b), 2); Circle(c, r) }), ->(3, { let __SPL2 = assertIsOfSize(collect(self,asComplex:/1), 3); let z1 = at(__SPL2, 1); let z2 = at(__SPL2, 2); let z3 = at(__SPL2, 3); let w = /((-(z3, z1)), (-(z2, z1))); let c = +(/(*((-(z2, z1)), (-(w, (^(abs(w), 2))))), (*(Complex(0, 2), imaginary(w)))), z1); let r = abs((-(z1, c))); Circle(asList(c), r) })]) }) }",
);
