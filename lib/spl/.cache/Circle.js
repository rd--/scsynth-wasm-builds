...
sl.addMethodToExistingType(
  ...
  "{ :self | let c = 1; let __SplVar1 = assertIsOfSize(center(self), 2); let r = at(__SplVar1, 1); let i = at(__SplVar1, 2); let alpha = Complex(negate(r), i); let d = -(square(abs(alpha)), square(radius(self))); GeneralisedCircle(c, alpha, d) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(center(self), 2); let a = at(__SplVar2, 1); let b = at(__SplVar2, 2); let rSquared = square(radius(self)); { :x :y | -(+(square((-(x, a))), square((-(y, b)))), rSquared) } }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let k = size(self); if((|(<(k, 2), { >(k, 3) })), { error(self,'circleThrough') }, { caseOf(k,[->(2, { let __SplVar3 = assertIsOfSize(self, 2); let a = at(__SplVar3, 1); let b = at(__SplVar3, 2); let c = /((+(a, b)), 2); let r = /(euclideanDistance(a,b), 2); Circle(c, r) }), ->(3, { let __SplVar4 = assertIsOfSize(collect(self,asComplex:/1), 3); let z1 = at(__SplVar4, 1); let z2 = at(__SplVar4, 2); let z3 = at(__SplVar4, 3); let w = /((-(z3, z1)), (-(z2, z1))); let c = +(/(*((-(z2, z1)), (-(w, (^(abs(w), 2))))), (*(Complex(0, 2), imaginary(w)))), z1); let r = abs((-(z1, c))); Circle(asList(c), r) })]) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar5 = assertIsOfSize(asList(self), 2); let h = at(__SplVar5, 1); let k = at(__SplVar5, 2); let r = /(1, (*(2, square(k)))); Circle([/(h, k), r], r) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :reference | let __SplVar6 = assertIsOfSize(center(self), 2); let x = at(__SplVar6, 1); let y = at(__SplVar6, 2); let a = radius(self); let __SplVar7 = assertIsOfSize(center(reference), 2); let x0 = at(__SplVar7, 1); let y0 = at(__SplVar7, 2); let k = radius(reference); let s = /(square(k), (-(+(square((-(x, x0))), square((-(y, y0)))), square(a)))); Circle([+(x0, (*(s, (-(x, x0))))), +(y0, (*(s, (-(y, y0)))))], *(abs(s), a)) }",
);
