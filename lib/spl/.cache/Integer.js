...
sl.addMethodToExistingTrait(
  ...
  "{ :a :n | let t = 0; let t1 = 1; let r = if((<(n, 0)), { negate(n) }, { n }); let r1 = if((<(a, 0)), { %((-(n, (%(negate(a), n)))), n) }, { %(a, n) }); whileTrue({ !=(r1, 0) }, { let quotient = //(r, r1); ({ let __SPL7 = [t1, -(t, (*(quotient, t1)))]; t := at(__SPL7, 1); t1 := at(__SPL7, 2) } . ()); ({ let __SPL8 = [r1, -(r, (*(quotient, r1)))]; r := at(__SPL8, 1); r1 := at(__SPL8, 2) } . ()) }); if((>(r, 1)), { error('@Integer>>modularInverse: not invertible') }, { ifTrue((<(t, 0)), { t := +(t, n) }); ifTrue((<(n, 0)), { t := +(t, n) }); t }) }",
);
