...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let absoluteTime = absoluteTime(system); let sessionTime = sessionTime(system); let timeDifference = -(absoluteTime, sessionTime); schedule({ :currentTime | let next = next(self); ifNil(next, { nil }, { let dur = at(next, 'dur'); let events = multiChannelExpand(next); let packets = gather(events, { :each | asPatternEventMessages(each,+(currentTime, timeDifference)) }); do(packets, { :each | sendOsc(scSynth(system),each) }); dur }) }) }",
);
      ...
      return _withAndCollect_4(
        ...
        sl.annotateFunction(function (_low, _high, _step) {
          ...
          _next = _foldBetweenAnd_3(
            _plusSign_2(
              _next,
              _aBlock_3(_randomNumberGenerator, [_negated_1(_step), _step], []),
            ),
            _low,
            _high,
          );
