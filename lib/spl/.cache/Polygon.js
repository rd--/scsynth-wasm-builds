...
sl.addMethodToExistingType(
    ...
    return _Polygon_1(
      _collect_2(
        ...
        sl.annotateFunction(function (_each) {
          ...
          return _plusSign_2(
            _asterisk_2(_hyphenMinus_2(_each, _c), _scale),
            _c,
          );
        }, ["each"]),
      ),
  ...
  "{ :self :scale | let c = centroid(self); Polygon(collect(vertexCoordinates(self), { :each | +((*((-(each, c)), scale)), c) })) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :p :q | if(isCoprime(p, q), { let v = circlePoints(p,[0, 0], 1, 0); let i = mod(asList(thenTo(1, +(1, q), *(p, q))),p, 1); Polygon(++(atAll(v,i), [first(v)])) }, { error([p, q],'starPolygon: not coprime') }) }",
);
