...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(dimensions(self), 2); let _ = at(__SplVar1, 1); let m = at(__SplVar1, 2); if((=(m, 2)), { let __SplVar2 = assertIsOfSize(transpose(self), 2); let keys = at(__SplVar2, 1); let values = at(__SplVar2, 2); let answer = Dictionary(); keys(answer, keys); values(answer, values); answer }, { error(self,'List>>listToDictionary') }) }",
);
