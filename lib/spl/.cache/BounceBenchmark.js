...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | initializeSlots(newBounceBall(),%(next(self), 500), %(next(self), 500), -((%(next(self), 300)), 150), -((%(next(self), 300)), 150)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ Benchmark('Bounce', asMap([->(100, 1331)]), { :ballCount | let random = SomRandom(); let bounces = 0; let balls = fill(ballCount, { :unusedIndex | BounceBall(random) }); toDo(1, 50, { :i | do(balls, { :ball | ifTrue(bounce(ball), { bounces := +(bounces, 1) }) }) }); bounces }) }",
);
