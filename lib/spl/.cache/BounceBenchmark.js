...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ Benchmark('Bounce', Map([[100, 1331]]), { :ballCount | let random = SomRandom(); let bounces = 0; let balls = fill(ballCount, { :unusedIndex | BounceBall(random) }); toDo(1, 50, { :i | do(balls, { :ball | ifTrue(bounce(ball), { bounces := +(bounces, 1) }) }) }); bounces }) }",
);
