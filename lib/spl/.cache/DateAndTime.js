...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject :aBlock:/2 | &(isDateAndTime(anObject), { aBlock(absoluteTime(self), absoluteTime(anObject)) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL2 = _assertIsOfSize_2(_self, 6);
    let _year = _at_2(__SPL2, 1);
    let _month = _at_2(__SPL2, 2);
    let _dayOfMonth = _at_2(__SPL2, 3);
    let _hour = _at_2(__SPL2, 4);
    let _minute = _at_2(__SPL2, 5);
    let _second = _at_2(__SPL2, 6);
    /* Statements */
    return _DateAndTime_6(_year, _month, _dayOfMonth, _hour, _minute, _second);
  }, ["self"]),
  "{ :self | let __SPL2 = assertIsOfSize(self, 6); let year = at(__SPL2, 1); let month = at(__SPL2, 2); let dayOfMonth = at(__SPL2, 3); let hour = at(__SPL2, 4); let minute = at(__SPL2, 5); let second = at(__SPL2, 6); DateAndTime(year, month, dayOfMonth, hour, minute, second) }",
);
