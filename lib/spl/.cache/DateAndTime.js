...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL1 = assertIsOfSize(self, 6); let year = at(__SPL1, 1); let month = at(__SPL1, 2); let dayOfMonth = at(__SPL1, 3); let hour = at(__SPL1, 4); let minute = at(__SPL1, 5); let second = at(__SPL1, 6); DateAndTime(year, month, dayOfMonth, hour, minute, second) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | &(includes([24, 29],size(self)), { matchesRegExp(self,'^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9](.[0-9]+)?([-+][0-9][0-9]:[0-9][0-9]|Z)?$') }) }",
);
