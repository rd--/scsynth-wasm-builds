...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL562 = _assertIsOfSize_2(_self, 6);
    let _year = _at_2(__SPL562, 1);
    let _month = _at_2(__SPL562, 2);
    let _dayOfMonth = _at_2(__SPL562, 3);
    let _hour = _at_2(__SPL562, 4);
    let _minute = _at_2(__SPL562, 5);
    let _second = _at_2(__SPL562, 6);
    /* Statements */
    return _DateAndTime_6(_year, _month, _dayOfMonth, _hour, _minute, _second);
  }, ["self"]),
  "{ :self | let __SPL562 = assertIsOfSize(self, 6); let year = at(__SPL562, 1); let month = at(__SPL562, 2); let dayOfMonth = at(__SPL562, 3); let hour = at(__SPL562, 4); let minute = at(__SPL562, 5); let second = at(__SPL562, 6); DateAndTime(year, month, dayOfMonth, hour, minute, second) }",
);
