...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL469 = _assertIsOfSize_2(_self, 6);
    let _year = _at_2(__SPL469, 1);
    let _month = _at_2(__SPL469, 2);
    let _dayOfMonth = _at_2(__SPL469, 3);
    let _hour = _at_2(__SPL469, 4);
    let _minute = _at_2(__SPL469, 5);
    let _second = _at_2(__SPL469, 6);
    /* Statements */
    return _DateAndTime_6(_year, _month, _dayOfMonth, _hour, _minute, _second);
  }, ["self"]),
  "{ :self | let __SPL469 = assertIsOfSize(self, 6); let year = at(__SPL469, 1); let month = at(__SPL469, 2); let dayOfMonth = at(__SPL469, 3); let hour = at(__SPL469, 4); let minute = at(__SPL469, 5); let second = at(__SPL469, 6); DateAndTime(year, month, dayOfMonth, hour, minute, second) }",
);
