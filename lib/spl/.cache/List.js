...
sl.addMethodToExistingType(
  ...
  "{ :self | self := /(self, norm(self)); caseOf(size(self), [->(2, { let __SPL1 = assertIsOfSize(self, 2); let a = at(__SPL1, 1); let b = at(__SPL1, 2); [[-(square(a), square(b)), *(*(2, a), b)], [*(*(2, a), b), -(square(b), square(a))]] }), ->(3, { let __SPL2 = assertIsOfSize(self, 3); let a = at(__SPL2, 1); let b = at(__SPL2, 2); let c = at(__SPL2, 3); [[-(1, (*(2, square(a)))), *(*(-2, a), b), *(*(-2, a), c)], [*(*(-2, a), b), -(1, (*(2, square(b)))), *(*(-2, b), c)], [*(*(-2, a), c), *(*(-2, b), c), -(1, (*(2, square(c))))]] })], { error('List>>reflectionMatrix: not two or three list') }) }",
);
