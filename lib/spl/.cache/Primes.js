...
sl.extendTypeOrTraitWithMethod(
          ...
          sl.annotateFunction(function () {
            ...
            return _ampersand_2(
              ...
              sl.annotateFunction(function () {
                ...
                return _not_1(
                  _isPrime_1(
                    _hyphenMinus_2(_n, _asterisk_2(2, _squared_1(_k))),
                  ),
                );
              }, []),
            );
  ...
  "{ :n | &(isPrime(n), { let k = floor(sqrt((/(n, 2)))); whileTrue({ &(>(k, 0), { not(isPrime((-(n, (*(2, squared(k))))))) }) }, { k := -(k, 1) }); isZero(k) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if((<=(self, 1)), { [] }, { valueWithReturn({ :return:/1 | let index = 1; let prime = 2; let k = self; let answer = []; repeat({ prime := prime(index); whileTrue({ =(%(k, prime), 0) }, { add(answer,prime); k := //(k, prime); ifTrue((=(k, 1)), { return(answer) }) }); ifTrue((>(squared(prime), k)), { add(answer,k); return(answer) }); index := +(index, 1) }); answer }) }) }",
);
