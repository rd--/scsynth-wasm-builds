...
sl.addMethodToExistingType(
  ...
  "{ :self :r :t :n | let sigma = sigma(self); let __SplVar1 = assertIsOfSize(start(self), 2); let t1 = at(__SplVar1, 1); let x1 = at(__SplVar1, 2); let __SplVar2 = assertIsOfSize(end(self), 2); let t2 = at(__SplVar2, 1); let x2 = at(__SplVar2, 2); let a = { :y :t | /((-(x2, y)), (-(t2, t))) }; let b = { :y :t | sigma }; let __SplVar3 = assertIsOfSize(t, 3); let tMin = at(__SplVar3, 1); let tMax = at(__SplVar3, 2); let dt = at(__SplVar3, 3); if((|(!=(t1, tMin), { !=(t2, tMax) })), { error(self,'randomFunction: invalid t') }, { TemporalData(!({ let __SplVar4 = assertIsOfSize(eulerMaruyamaMethod(r,a:/2, b:/2, t, x1), 2); let p = at(__SplVar4, 1); let q = at(__SplVar4, 2); let k = size(q); atPut(q, k, x2); transpose([p, q]) }, n)) }) }",
);
