...
sl.addMethodToExistingType(
  ...
  "{ :self :anObject :aBlock:/2 | &(isHelp(anObject), { equalByAtNamedSlots(self,anObject, ['origin', 'source'], aBlock:/2) }) }",
);
...
sl.addMethodToExistingType(
      ...
      sl.annotateFunction(function (_each) {
        ...
        return _ifTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL24 = _assertIsOfSize_2(_at_2(_each, "sourcePosition"), 2);
            let _start = _at_2(__SPL24, 1);
            let _end = _at_2(__SPL24, 2);
            /* Statements */
            return _ifTrue_2(
  ...
  "{ :self | let answer = []; do(markdown(self), { :each | ifTrue((=(at(each, 'type'), 'codeBlock')), { let __SPL24 = assertIsOfSize(at(each, 'sourcePosition'), 2); let start = at(__SPL24, 1); let end = at(__SPL24, 2); ifTrue(isCodeFence(at(lines(self), at(start, 1))), { add(answer,Range(at(start, 1), at(end, 1), 1)) }) }) }); answer }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | readCommaSeparatedField(self,'Guides: ') }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | isNotEmpty(documentationTests(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | isNotEmpty(rationale(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | isNotEmpty(references(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | isNotEmpty(signatures(self)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :index | linesFromWhile(self, index, { :each :unusedIndex | isNotEmpty(each) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :prefix | cached(self, prefix, { let lines = lines(self); let limit = size(lines); let index = findFirst(lines, { :each | =(each, prefix) }); if(isPositive(index), { linesFromWhile(self, +(index, 1), { :each :unusedIndex | isNotEmpty(each) }) }, { [] }) }) }",
);
