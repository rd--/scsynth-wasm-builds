...
sl.addMethodToExistingType(
      ...
      sl.annotateFunction(function (_each) {
        ...
        return _ifTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL238 = _assertIsOfSize_2(_at_2(_each, "sourcePosition"), 2);
            let _start = _at_2(__SPL238, 1);
            let _end = _at_2(__SPL238, 2);
            /* Statements */
            return _ifTrue_2(
  ...
  "{ :self | let answer = []; do(markdown(self), { :each | ifTrue((=(at(each, 'type'), 'codeBlock')), { let __SPL238 = assertIsOfSize(at(each, 'sourcePosition'), 2); let start = at(__SPL238, 1); let end = at(__SPL238, 2); ifTrue(isCodeFence(at(lines(self), at(start, 1))), { add(answer,Range(at(start, 1), at(end, 1), 1)) }) }) }); answer }",
);
