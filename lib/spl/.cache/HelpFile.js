...
sl.addMethodToExistingType(
    ...
    _do_2(_markdown_1(_self), function (_each) {
      ...
      return _ifTrue_2(
        ...
        function () {
          ...
          let __SPL174 = _assertIsOfSize_2(_at_2(_each, "sourcePosition"), 2);
          let _start = _at_2(__SPL174, 1);
          let _end = _at_2(__SPL174, 2);
          /* Statements */
          return _ifTrue_2(
  ...
  "{ :self | let answer = []; do(markdown(self), { :each | ifTrue((=(at(each, 'type'), 'codeBlock')), { let __SPL174 = assertIsOfSize(at(each, 'sourcePosition'), 2); let start = at(__SPL174, 1); let end = at(__SPL174, 2); ifTrue(isCodeFence(at(lines(self), at(start, 1))), { add(answer,Range(at(start, 1), at(end, 1), 1)) }) }) }); answer }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let verbose = false; ifTrue(verbose, { postLine(['writeImageFiles', name(self)]) }); do(definitionCodeBlocks(self), { :each | evaluate(system,at(each, 'contents')) }); do(codeBlocksWithAttribute(self,'png'), { :each | let fileName = codeBlockImageFileName(self,each, 'png'); postLine(fileName); writePng(evaluate(system,at(each, 'contents')),fileName) }); do(codeBlocksWithAttribute(self,'svg'), { :each | let fileName = codeBlockImageFileName(self,each, 'svg'); postLine(fileName); writeSvg(evaluate(system,at(each, 'contents')),fileName) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "HelpFile",
  "helpFilesDo",
  ["self", "kind", "pattern", "aBlock:/1"],
  function (_self, _kind, _pattern, _aBlock_1) {
    /* ArityCheck */
    if (arguments.length !== 4) {
      const errorMessage =
        "Arity: expected 4, _self, _kind, _pattern, _aBlock_1";
      throw new Error(errorMessage);
    } /* Statements */
    return _do_2(
      _select_2(
        ...
        function (_each) {
          ...
          return _ampersand_2(_endsWith_2(_each, ".help.sl"), function () {
            ...
            return _matchesRegExp_2(_pathBasename_1(_each), _pattern);
          });
        },
      ),
      function (_each) {
  ...
  "{ :self :kind :pattern :aBlock:/1 | do(select(readDirectoryFileNames(splFileName(self,++('Help/', kind))), { :each | &(endsWith(each,'.help.sl'), { matchesRegExp(pathBasename(each),pattern) }) }), { :each | let text = readTextFile(each); let help = HelpFile(asFileUrl(each), text); aBlock(help) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self, _pattern, _aBlock_1) {
    /* ArityCheck */
    if (arguments.length !== 3) {
