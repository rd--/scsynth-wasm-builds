...
sl.addMethodToExistingType(
  ...
  "{ :self | let answer = []; do(markdown(self), { :each | ifTrue((=(at(each, 'type'), 'codeBlock')), { let __SplVar1 = assertIsOfSize(at(each, 'sourcePosition'), 2); let start = at(__SplVar1, 1); let end = at(__SplVar1, 2); ifTrue(isCodeFence(at(lines(self), at(start, 1))), { add(answer,Range(at(start, 1), at(end, 1), 1)) }) }) }); answer }",
);
