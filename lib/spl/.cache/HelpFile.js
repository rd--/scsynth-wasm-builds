...
sl.addMethodToExistingType(
      ...
      sl.annotateFunction(function (_each) {
        ...
        return _ifTrue_2(
          ...
          sl.annotateFunction(function () {
            ...
            let __SPL226 = _assertIsOfSize_2(_at_2(_each, "sourcePosition"), 2);
            let _start = _at_2(__SPL226, 1);
            let _end = _at_2(__SPL226, 2);
            /* Statements */
            return _ifTrue_2(
  ...
  "{ :self | let answer = []; do(markdown(self), { :each | ifTrue((=(at(each, 'type'), 'codeBlock')), { let __SPL226 = assertIsOfSize(at(each, 'sourcePosition'), 2); let start = at(__SPL226, 1); let end = at(__SPL226, 2); ifTrue(isCodeFence(at(lines(self), at(start, 1))), { add(answer,Range(at(start, 1), at(end, 1), 1)) }) }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :kind :pattern :verbose :aBlock:/1 | do(select(sort(readDirectoryFileNames(splFileName(self,++('Help/', kind)))), { :each | &(endsWith(each,'.help.sl'), { matchesRegExp(pathBasename(each),pattern) }) }), { :each | let text = readTextFile(each); let help = HelpFile(asFileUrl(each), text); ifTrue(verbose, { postLine(each) }); aBlock(help) }) }",
);
