...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SplVar1 = _assertIsOfSize_2(_asList_1(_self), 4);
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(asList(self), 4); let d = at(__SplVar1, 1); let h = at(__SplVar1, 2); let m = at(__SplVar1, 3); let s = at(__SplVar1, 4); format('P%DT%H%M%S',[d, h, m, s]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar2 = assertIsOfSize(self, 4); let d = at(__SplVar2, 1); let h = at(__SplVar2, 2); let m = at(__SplVar2, 3); let s = at(__SplVar2, 4); let b = [24, 60, 60]; Duration(mixedRadixDecode(self,b)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :elseClause:/0 | if(isIso8601DurationString(self), { let __SplVar3 = assertIsOfSize(asList(parseCalendarDuration(self)), 6); let years = at(__SplVar3, 1); let months = at(__SplVar3, 2); let days = at(__SplVar3, 3); let hours = at(__SplVar3, 4); let minutes = at(__SplVar3, 5); let seconds = at(__SplVar3, 6); if((>(+(years, months), 0)), { elseClause() }, { Duration(mixedRadixDecode([days, hours, minutes, seconds],[24, 60, 60])) }) }, { elseClause() }) }",
);
