...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL470 = _assertIsOfSize_2(_asList_1(_self), 4);
    let _d = _at_2(__SPL470, 1);
    let _h = _at_2(__SPL470, 2);
    let _m = _at_2(__SPL470, 3);
    let _s = _at_2(__SPL470, 4);
    /* Statements */
    return _format_2("P%DT%H%M%S", [_d, _h, _m, _s]);
  }, ["self"]),
  "{ :self | let __SPL470 = assertIsOfSize(asList(self), 4); let d = at(__SPL470, 1); let h = at(__SPL470, 2); let m = at(__SPL470, 3); let s = at(__SPL470, 4); format('P%DT%H%M%S',[d, h, m, s]) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL471 = _assertIsOfSize_2(_self, 4);
    let _d = _at_2(__SPL471, 1);
    let _h = _at_2(__SPL471, 2);
    let _m = _at_2(__SPL471, 3);
    let _s = _at_2(__SPL471, 4);
    let _b = [24, 60, 60];
    /* Statements */
    return _Duration_1(_mixedRadixDecode_2(_self, _b));
  }, ["self"]),
  "{ :self | let __SPL471 = assertIsOfSize(self, 4); let d = at(__SPL471, 1); let h = at(__SPL471, 2); let m = at(__SPL471, 3); let s = at(__SPL471, 4); let b = [24, 60, 60]; Duration(mixedRadixDecode(self,b)) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _elseClause_0) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL472 = _assertIsOfSize_2(
          _asList_1(_parseCalendarDuration_1(_self)),
          6,
        );
        let _years = _at_2(__SPL472, 1);
        let _months = _at_2(__SPL472, 2);
        let _days = _at_2(__SPL472, 3);
        let _hours = _at_2(__SPL472, 4);
        let _minutes = _at_2(__SPL472, 5);
        let _seconds = _at_2(__SPL472, 6);
        /* Statements */
        return _if_3(
  ...
  "{ :self :elseClause:/0 | if(isIso8601DurationString(self), { let __SPL472 = assertIsOfSize(asList(parseCalendarDuration(self)), 6); let years = at(__SPL472, 1); let months = at(__SPL472, 2); let days = at(__SPL472, 3); let hours = at(__SPL472, 4); let minutes = at(__SPL472, 5); let seconds = at(__SPL472, 6); if((>(+(years, months), 0)), { elseClause() }, { Duration(mixedRadixDecode([days, hours, minutes, seconds],[24, 60, 60])) }) }, { elseClause() }) }",
);
