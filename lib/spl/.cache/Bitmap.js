...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL176 = _assertIsOfSize_2(_shape_1(_contents_1(_self)), 2);
    let _rowCount = _at_2(__SPL176, 1);
    let _columnCount = _at_2(__SPL176, 2);
  ...
  "{ :self | let __SPL176 = assertIsOfSize(shape(contents(self)), 2); let rowCount = at(__SPL176, 1); let columnCount = at(__SPL176, 2); let header = ['P1', format('% %',[columnCount, rowCount])]; let rows = collect(contents(self), { :each | unwords(collect(each, { :item | printString(asBit(item)) })) }); unlines((++(++(header, rows), ['']))) }",
);
