...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL1 = _assertIsOfSize_2(_shape_1(_contents_1(_self)), 2);
    let _rowCount = _at_2(__SPL1, 1);
    let _columnCount = _at_2(__SPL1, 2);
  ...
  "{ :self | let __SPL1 = assertIsOfSize(shape(contents(self)), 2); let rowCount = at(__SPL1, 1); let columnCount = at(__SPL1, 2); let header = ['P1', format('% %',[columnCount, rowCount])]; let rows = collect(contents(self), { :each | unwords(collect(each, { :item | printString(asBit(item)) })) }); unlines((++(++(header, rows), ['']))) }",
);
