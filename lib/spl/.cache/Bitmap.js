...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(shape(contents(self)), 2); let rowCount = at(__SplVar1, 1); let columnCount = at(__SplVar1, 2); let header = ['P1', format('% %',[columnCount, rowCount])]; let rows = collect(contents(self), { :each | unwords(collect(each, { :item | printString(asBit(item)) })) }); unlines((++(++(header, rows), ['']))) }",
);
