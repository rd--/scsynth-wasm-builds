...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :coefficientMatrix :x :y | ifEmpty(coefficientMatrix, { 0 }, { let __SplVar1 = assertIsOfSize(shape(coefficientMatrix), 2); let m = at(__SplVar1, 1); let n = at(__SplVar1, 2); let answer = nil; toDo(1, m, { :i | let r = at(coefficientMatrix, i); toDo(1, n, { :j | let a = at(r, j); ifFalse(isZero(a), { let p = -(i, 1); let q = -(j, 1); let r = which([->({ &(=(p, 0), { =(q, 0) }) }, { a }), ->({ =(p, 0) }, { *(a, (^(y, q))) }), ->({ =(q, 0) }, { *(a, (^(x, p))) }), ->({ true }, { *(*(a, (^(x, p))), (^(y, q))) })]); answer := ifNil(answer, { r }, { +(answer, r) }) }) }) }); answer }) }",
);
