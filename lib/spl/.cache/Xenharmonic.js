...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_p, _q) {
    ...
    return _sum_1(_sum_1(_table_3(
      sl.annotateFunction(function (_a, _b) {
        ...
        let __SPL1 = _assertIsOfSize_2(_a, 2);
        let _f1 = _at_2(__SPL1, 1);
        let _u1 = _at_2(__SPL1, 2);
        let __SPL2 = _assertIsOfSize_2(_b, 2);
        let _f2 = _at_2(__SPL2, 1);
        let _u2 = _at_2(__SPL2, 2);
        /* Statements */
  ...
  "{ :p :q | sum(sum(table({ :a :b | let __SPL1 = assertIsOfSize(a, 2); let f1 = at(__SPL1, 1); let u1 = at(__SPL1, 2); let __SPL2 = assertIsOfSize(b, 2); let f2 = at(__SPL2, 1); let u2 = at(__SPL2, 2); plompLeveltDissonance(f1, f2, u1, u2) },p, q))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL3 = _assertIsOfSize_2(_asList_1(_withoutFactorsOfTwo_1(_self)), 2);
    let _n = _at_2(__SPL3, 1);
    let _d = _at_2(__SPL3, 2);
    /* Statements */
    return _max_2(_n, _d);
  }, ["self"]),
  "{ :self | let __SPL3 = assertIsOfSize(asList(withoutFactorsOfTwo(self)), 2); let n = at(__SPL3, 1); let d = at(__SPL3, 2); max(n,d) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :octaveRatio | let exponent = negated(floor(log(asFloat(self),asFloat(octaveRatio)))); *(self, (^(Fraction(2L, 1L), exponent))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :p | if(isOne(p), { 0 }, { if(isPrime(p), { *(/(squared((-(p, 1))), p), 2) }, { sum(collect(primeFactors(p), { :n | *(/(squared((-(n, 1))), n), 2) })) }) }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | ifFalse(isInteger(self), { error(self,'octaveReduced: not integer') }); octaveReduced(Fraction(self, 1),2) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | collect(self,octaveReduced:/1) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let n = asList(thenTo(1, 3, self)); let o = sort(collect(n, { :i | octaveReduced(Fraction(i, 1)) })); let u = /(1, o); table({ :i :j | octaveReduced((*(i, j))) },u, o) }",
);
