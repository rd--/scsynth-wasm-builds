sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let ratios = asRatios(self); let primesVector = latticeDerivedPrimesVector(self,[3, 5, 7, 11, 13, 17, 19, 23]); let div = createElement('div'); appendChildren(div,[asHtmlTable([['Size', asString(size(self))], ['Limit', asString(primeLimit(self))], ['Description', description(self)], ['Octave', asString(octave(self))], ['Primes', asString(latticePrimes(self,false))]]), ifNil(primesVector, { TextParagraph('No drawing') }, { latticeDrawing(self,primesVector, gradyLatticeCoordinates(1)) }), asHtmlTable(transpose([asList(upOrDownTo(1, size(self))), ratios, collect(ratios, { :each | ifNil(primesVector, { '*' }, { latticeVectorString(each,primesVector) }) }), round(asCents(self)), asIntegers(self)]))]); div }",
);
