sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let ratios = asRatios(self); let primesVector = tuningLatticeDerivedPrimesVector(self,[3, 5, 7, 11, 13, 17, 19, 23]); let div = createElement('div'); let overviewTable = Table([['Size', asString(size(self))], ['Limit', asString(primeLimit(self))], ['Description', description(self)], ['Octave', asString(octave(self))], ['Primes', asString(tuningLatticePrimes(self,false))]]); let tuningTable = Table(transpose([asList(upOrDownTo(1, size(self))), ratios, collect(ratios, { :each | ifNil(primesVector, { '*' }, { tuningLatticeVectorString(each,primesVector) }) }), round(asCents(self)), asIntegers(self)])); appendChildren(div,[parseHtml(document(window(system)),asHtml(overviewTable)), ifNil(primesVector, { TextParagraph('No drawing') }, { tuningLatticeDrawing(self,primesVector, gradyTuningLatticeCoordinates(1)) }), parseHtml(document(window(system)),asHtml(tuningTable))]); div }",
);
