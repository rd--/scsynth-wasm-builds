...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _options) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let _p1 = _at_2(__SPL1, 1);
        let _p2 = _at_2(__SPL1, 2);
        let __SPL2 = _assertIsOfSize_2(_p1, 2);
        let _x1 = _at_2(__SPL2, 1);
        let _y1 = _at_2(__SPL2, 2);
        let __SPL3 = _assertIsOfSize_2(_p2, 2);
  ...
  '{ :self :options | if((=(vertexCount(self), 2)), { let precision = at(options, \'precision\'); let __SPL1 = assertIsOfSize(vertexCoordinates(self), 2); let p1 = at(__SPL1, 1); let p2 = at(__SPL1, 2); let __SPL2 = assertIsOfSize(p1, 2); let x1 = at(__SPL2, 1); let y1 = at(__SPL2, 2); let __SPL3 = assertIsOfSize(p2, 2); let x2 = at(__SPL3, 1); let y2 = at(__SPL3, 2); format(\'<line x1="%" y1="%" x2="%" y2="%" />\',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), printStringToFixed(x2,precision), printStringToFixed(y2,precision)]) }, { format(\'<polyline points="%" />\',[asSvgPointList(vertexCoordinates(self),options)]) }) }',
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL4 = _assertIsOfSize_2(_at_2(_self, 1), 2);
    let _x1 = _at_2(__SPL4, 1);
    let _y1 = _at_2(__SPL4, 2);
    let __SPL5 = _assertIsOfSize_2(_at_2(_self, 2), 2);
  ...
  "{ :self | let __SPL4 = assertIsOfSize(at(self, 1), 2); let x1 = at(__SPL4, 1); let y1 = at(__SPL4, 2); let __SPL5 = assertIsOfSize(at(self, 2), 2); let x2 = at(__SPL5, 1); let y2 = at(__SPL5, 2); let dx = abs((-(x2, x1))); let sx = if((<(x1, x2)), { 1 }, { -1 }); let dy = -(abs((-(y2, y1)))); let sy = if((<(y1, y2)), { 1 }, { -1 }); let e = +(dx, dy); let answer = []; whileTrue({ add(answer,[x1, y1]); if((&((==(x1, x2)), { ==(y1, y2) })), { false }, { let e2 = *(2, e); ifTrue((>=(e2, dy)), { e := +(e, dy); x1 := +(x1, sx) }); ifTrue((<=(e2, dx)), { e := +(e, dx); y1 := +(y1, sy) }); true }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL6 = _assertIsOfSize_2(_at_2(_self, 1), 2);
    let _x0 = _at_2(__SPL6, 1);
    let _y0 = _at_2(__SPL6, 2);
    let __SPL7 = _assertIsOfSize_2(_at_2(_self, 2), 2);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_n, _m, _centre, _radii, _initialAngle) {
    ...
    let __SPL8 = _assertIsOfSize_2(_radii, 2);
    let _radiusIncrement = _at_2(__SPL8, 1);
    let _initialRadial = _at_2(__SPL8, 2);
