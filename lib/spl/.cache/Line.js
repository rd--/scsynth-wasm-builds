...
sl.addMethodToExistingType(
  ...
  "{ :self | let __SplVar1 = assertIsOfSize(vertexCoordinates(self), 2); let a = at(__SplVar1, 1); let b = at(__SplVar1, 2); lineEquation(a,b) }",
);
...
sl.addMethodToExistingType(
  ...
  '{ :self :options | if((=(vertexCount(self), 2)), { let precision = at(options, \'precision\'); let __SplVar2 = assertIsOfSize(vertexCoordinates(self), 2); let p1 = at(__SplVar2, 1); let p2 = at(__SplVar2, 2); let __SplVar3 = assertIsOfSize(p1, 2); let x1 = at(__SplVar3, 1); let y1 = at(__SplVar3, 2); let __SplVar4 = assertIsOfSize(p2, 2); let x2 = at(__SplVar4, 1); let y2 = at(__SplVar4, 2); format(\'<line x1="%" y1="%" x2="%" y2="%" />\',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), printStringToFixed(x2,precision), printStringToFixed(y2,precision)]) }, { format(\'<polyline points="%" />\',[asSvgPointList(vertexCoordinates(self),options)]) }) }',
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SplVar5 = assertIsOfSize(at(self, 1), 2); let x1 = at(__SplVar5, 1); let y1 = at(__SplVar5, 2); let __SplVar6 = assertIsOfSize(at(self, 2), 2); let x2 = at(__SplVar6, 1); let y2 = at(__SplVar6, 2); let dx = abs((-(x2, x1))); let sx = if((<(x1, x2)), { 1 }, { -1 }); let dy = -(abs((-(y2, y1)))); let sy = if((<(y1, y2)), { 1 }, { -1 }); let e = +(dx, dy); let answer = []; whileTrue({ add(answer,[x1, y1]); if((&((==(x1, x2)), { ==(y1, y2) })), { false }, { let e2 = *(2, e); ifTrue((>=(e2, dy)), { e := +(e, dy); x1 := +(x1, sx) }); ifTrue((<=(e2, dx)), { e := +(e, dx); y1 := +(y1, sy) }); true }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :aRectangle | let __SplVar10 = assertIsOfSize(vertexCoordinates(self), 2); let a = at(__SplVar10, 1); let b = at(__SplVar10, 2); let c = lowerLeft(aRectangle); let d = upperRight(aRectangle); let __SplVar11 = assertIsOfSize(cohenSutherlandAlgorithm(at(a, 1), at(a, 2), at(b, 1), at(b, 2), at(c, 1), at(c, 2), at(d, 1), at(d, 2)), 5); let accept = at(__SplVar11, 1); let x1 = at(__SplVar11, 2); let y1 = at(__SplVar11, 3); let x2 = at(__SplVar11, 4); let y2 = at(__SplVar11, 5); if(accept, { Line([[x1, y1], [x2, y2]]) }, { nil }) }",
);
