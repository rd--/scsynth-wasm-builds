...
sl.addMethodToExistingType(
  ...
  function (_self, _options) {
    ...
    return _if_3(_equalsSign_2(_vertexCount_1(_self), 2), function () {
      ...
      let __SPL86 = _assertIsOfSize_2(_vertexCoordinates_1(_self), 2);
      let _p1 = _at_2(__SPL86, 1);
      let _p2 = _at_2(__SPL86, 2);
      let __SPL87 = _assertIsOfSize_2(_p1, 2);
      let _x1 = _at_2(__SPL87, 1);
      let _y1 = _at_2(__SPL87, 2);
      let __SPL88 = _assertIsOfSize_2(_p2, 2);
  ...
  '{ :self :options | if((=(vertexCount(self), 2)), { let precision = at(options, \'precision\'); let __SPL86 = assertIsOfSize(vertexCoordinates(self), 2); let p1 = at(__SPL86, 1); let p2 = at(__SPL86, 2); let __SPL87 = assertIsOfSize(p1, 2); let x1 = at(__SPL87, 1); let y1 = at(__SPL87, 2); let __SPL88 = assertIsOfSize(p2, 2); let x2 = at(__SPL88, 1); let y2 = at(__SPL88, 2); format(\'<line x1="%" y1="%" x2="%" y2="%" />\',[printStringToFixed(x1,precision), printStringToFixed(y1,precision), printStringToFixed(x2,precision), printStringToFixed(y2,precision)]) }, { format(\'<polyline points="%" />\',[asSvgPointList(vertexCoordinates(self),options)]) }) }',
);
...
sl.addMethodToExistingType(
  ...
  function (_self) {
    ...
    let _c1 = 0;
    let _c2 = _c1;
    let __genSym91 = _whileTrue_2(function () {
      /* ArityCheck */
      if (arguments.length !== 0) {
        const errorMessage = "Arity: expected 0, ";
        throw new Error(errorMessage);
...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    let __SPL89 = _assertIsOfSize_2(_at_2(_self, 1), 2);
    let _x1 = _at_2(__SPL89, 1);
    let _y1 = _at_2(__SPL89, 2);
    let __SPL90 = _assertIsOfSize_2(_at_2(_self, 2), 2);
    let _x2 = _at_2(__SPL90, 1);
    let _y2 = _at_2(__SPL90, 2);
  ...
  "{ :self | let __SPL89 = assertIsOfSize(at(self, 1), 2); let x1 = at(__SPL89, 1); let y1 = at(__SPL89, 2); let __SPL90 = assertIsOfSize(at(self, 2), 2); let x2 = at(__SPL90, 1); let y2 = at(__SPL90, 2); let dx = abs((-(x2, x1))); let sx = if((<(x1, x2)), { 1 }, { -1 }); let dy = -(abs((-(y2, y1)))); let sy = if((<(y1, y2)), { 1 }, { -1 }); let e = +(dx, dy); let answer = []; whileTrue({ add(answer,[x1, y1]); if((&((==(x1, x2)), { ==(y1, y2) })), { false }, { let e2 = *(2, e); ifTrue((>=(e2, dy)), { e := +(e, dy); x1 := +(x1, sx) }); ifTrue((<=(e2, dx)), { e := +(e, dx); y1 := +(y1, sy) }); true }) }); answer }",
);
