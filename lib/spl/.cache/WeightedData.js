...
sl.addMethodToExistingType(
  ...
  "{ :self | let x = inputData(self); let w = weights(self); sqrt(sum((/((*(square(x), w)), sum(w))))) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let x = inputData(self); let w = weights(self); let wSum = sum(w); let v = sum(square(w)); let n = size(x); let mu = mean(self); *((/(wSum, (-(square(wSum), v)))), sum((*(w, (^((-(x, mu)), 2)))))) }",
);
