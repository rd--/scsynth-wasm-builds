...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        return _uncheckedAsRecord_1(_matrix);
      }, []),
      sl.annotateFunction(function () {
        /* ArityCheck */
        if (arguments.length !== 0) {
  ...
  "{ :self | let matrix = collect(self,asList:/1); if((&((|(isAssociationList(self), { &(isMatrix(matrix), { =(second(shape(matrix)), 2) }) })), { allSatisfy(matrix, { :each | isString(first(each)) }) })), { uncheckedAsRecord(matrix) }, { error(self,'List>>asRecord: not of correct shape or invalid keys') }) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | if(allSatisfy(self, { :each | &(isList(each), { &(=(size(each), 2), { isString(at(each, 1)) }) }) }), { uncheckedAsRecord(self) }, { error(self,'List>>Record: not two column matrix or keys not uniformly String values') }) }",
);
