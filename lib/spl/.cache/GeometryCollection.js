...
sl.addMethodToExistingType(
  ...
  "{ :self :circle | GeometryCollection(collect(contents(self), { :each | circleInversion(each,circle) })) }",
);
...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL161 = _assertIsOfSize_2(
      _nub_1(_collect_2(_contents_1(_self), _embeddingDimension_1)),
      1,
    );
    let _n = _at_2(__SPL161, 1);
    /* Statements */
    return _n;
  }, ["self"]),
  "{ :self | let __SPL161 = assertIsOfSize(nub(collect(contents(self),embeddingDimension:/1)), 1); let n = at(__SPL161, 1); n }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :options | unlines(collect(contents(self), { :each | svgFragment(each,options) })) }",
);
