...
sl.extendTypeOrTraitWithMethod(
    ...
    return _LineDrawing_2([
      _collect_2(
        ...
        sl.annotateFunction(function (_y) {
          ...
          let _x1 = _at_2(__SPL38, 1);
          let _x2 = _at_2(__SPL38, 2);
          /* Statements */
  ...
  "{ :self | let chordDegrees = /(ratioToCents(self), 100); let upper = +(max(chordDegrees), 1); let lineDegrees = [2, 5, 9, 12, 16, 19, 23]; let allLines = takeWhile(collectCatenate(Span(0, 3, 1), { :each | +(*(each, 24), lineDegrees) }), { :each | <=(each, upper) }); let staffLines = [19, 23, 26, 29, 33, 40, 43, 47, 50, 53]; LineDrawing([collect(allLines, { :y | let __SPL38 = assertIsOfSize(if(includes(staffLines,y), { [0, 15] }, { [5, 10] }), 2); let x1 = at(__SPL38, 1); let x2 = at(__SPL38, 2); Line([[x1, y], [x2, y]]) }), collect(chordDegrees, { :y | Ellipse([7.5, y], [/(4, 5), /(2, 3)]) })], Record([['height', 300]])) }",
);
        ...
        sl.annotateFunction(function (_libraryItem) {
          ...
          return _collect_2(
            ...
            sl.annotateFunction(function (_each) {
              ...
              let __SPL39 = _assertIsOfSize_2(_each, 4);
              let _f2 = _at_2(__SPL39, 1);
              let _f1 = _at_2(__SPL39, 2);
              let _m = _at_2(__SPL39, 3);
              let _sd = _at_2(__SPL39, 4);
              /* Statements */
