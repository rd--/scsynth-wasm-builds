...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let chordDegrees = /(ratioToCents(self), 100); let upper = +(max(chordDegrees), 1); let lineDegrees = [2, 5, 9, 12, 16, 19, 23]; let allLines = takeWhile(collectCatenate(Span(0, 3, 1), { :each | +(*(each, 24), lineDegrees) }), { :each | <=(each, upper) }); let staffLines = [19, 23, 26, 29, 33, 40, 43, 47, 50, 53]; LineDrawing([collect(allLines, { :y | let __SplVar1 = assertIsOfSize(if(includes(staffLines,y), { [0, 15] }, { [5, 10] }), 2); let x1 = at(__SplVar1, 1); let x2 = at(__SplVar1, 2); Line([[x1, y], [x2, y]]) }), collect(chordDegrees, { :y | Ellipse([7.5, y], [/(4, 5), /(2, 3)]) })], Record([['height', 300]])) }",
);
