...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :window :max :coefficients | discretize((--(-0.5, 0.5)),self, caseOf(window, [->(1, { hammingWindow:/1 }), ->(2, { hannWindow:/1 }), ->(3, { bartlettWindow:/1 }), ->(4, { blackmanWindow:/1 }), ->(5, { blackmanHarrisWindow:/1 }), ->(6, { let sigma = *(at(coefficients, 1), 0.1); { :x | gaussianWindow(x,sigma) } }), ->(7, { let alpha = at(coefficients, 1); { :x | kaiserWindow(x,alpha) } }), ->(8, { dirichletWindowWindow:/1 }), ->(9, { let alpha = at(coefficients, 1); { :x | sincWindow(x,alpha) } })], { error(self,'gen20: unknown window') })) }",
);
