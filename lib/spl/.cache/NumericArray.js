...
sl.addMethodToExistingType(
  ...
  "{ :self :i :j | if((=(rank(self), 2)), { let __SPL52 = assertIsOfSize(shape(self), 2); let m = at(__SPL52, 1); let n = at(__SPL52, 2); let linearIndex = +((*((-(i, 1)), n)), j); at(contents(self),linearIndex) }, { error(self,'NumericArray>>at') }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :i :j :x | if((=(rank(self), 2)), { let __SPL53 = assertIsOfSize(shape(self), 2); let m = at(__SPL53, 1); let n = at(__SPL53, 2); let linearIndex = +((*((-(i, 1)), n)), j); atPut(contents(self),linearIndex, x) }, { error(self,'NumericArray>>atPut') }) }",
);
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _ampersand_2(
      ...
      sl.annotateFunction(function () {
        ...
        let _n = _at_2(__SPL54, 1);
        let _m = _at_2(__SPL54, 2);
        /* Statements */
        return _equalsSign_2(_n, _m);
      }, []),
    );
  }, ["self"]),
  "{ :self | &((=(rank(self), 2)), { let __SPL54 = assertIsOfSize(shape(self), 2); let n = at(__SPL54, 1); let m = at(__SPL54, 2); =(n, m) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | if(isMatrix(self), { let __SPL56 = assertIsOfSize(shape(self), 2); let m = at(__SPL56, 1); let n = at(__SPL56, 2); let c = contents(self); let a = new(species(c),*(m, n)); let k = 1; toDo(1, n, { :i | toDo(1, m, { :j | atPut(a, k, at(c, +((*((-(j, 1)), n)), i))); k := +(k, 1) }) }); NumericArray(a, [n, m]) }, { error(self,'NumericArray>>transposed: not matrix') }) }",
);
