...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :b1 :b2 | let __SplVar1 = assertIsOfSize([size(b1), size(b2)], 2); let m = at(__SplVar1, 1); let n = at(__SplVar1, 2); let c = !({ [] }, [-(m, 1), -(n, 1)]); do(self, { :e | let __SplVar2 = assertIsOfSize(e, 2); let e1 = at(__SplVar2, 1); let e2 = at(__SplVar2, 2); ifTrue((&(>=(e1, at(b1, 1)), { &(<=(e1, at(b1, m)), { &(>=(e2, at(b2, 1)), { <=(e2, at(b2, n)) }) }) })), { let i = min(binarySearchLeftmost(b1,e1),-(m, 1)); let j = min(binarySearchLeftmost(b2,e2),-(n, 1)); add(at(at(c, i), j),e) }) }); c }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :b | let __SplVar3 = assertIsOfSize(b, 3); let start = at(__SplVar3, 1); let stop = at(__SplVar3, 2); let step = at(__SplVar3, 3); histogramListFor(self,asList(Range(start, stop, step))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let k = +(ceiling(sqrt(size(self))), 1); let __SplVar4 = assertIsOfSize(minMax(self), 2); let min = at(__SplVar4, 1); let max = at(__SplVar4, 2); let b = findDivisions((--(min, max)),k); histogramListFor(self,asList(b)) }",
);
