...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :b | let __SPL3 = assertIsOfSize(b, 3); let start = at(__SPL3, 1); let stop = at(__SPL3, 2); let step = at(__SPL3, 3); histogramListFor(self,asList(Range(start, stop, step))) }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    let __SPL4 = _assertIsOfSize_2(_minMax_1(_self), 2);
    let _min = _at_2(__SPL4, 1);
    let _max = _at_2(__SPL4, 2);
    let _b = _findDivisions_2(_hyphenMinusHyphenMinus_2(_min, _max), _k);
    /* Statements */
    return _histogramListFor_2(_self, _asList_1(_b));
  }, ["self"]),
  "{ :self | let k = +(ceiling(sqrt(size(self))), 1); let __SPL4 = assertIsOfSize(minMax(self), 2); let min = at(__SPL4, 1); let max = at(__SPL4, 2); let b = findDivisions((--(min, max)),k); histogramListFor(self,asList(b)) }",
);
