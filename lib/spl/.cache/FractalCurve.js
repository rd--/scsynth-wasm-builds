sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(
    function (_stepLetters, _plusMinus, _rule, _initialCondition, _count) {
      ...
      let __SPL126 = _assertIsOfSize_2(_plusMinus, 2);
      let _plusAngle = _at_2(__SPL126, 1);
      let _minusAngle = _at_2(__SPL126, 2);
      /* Statements */
      _do_2(
        _contents_1(
          _select_2(
  ...
  "{ :stepLetters :plusMinus :rule :initialCondition :count | let angle = 0; let answer = [[0, 0]]; let keepLetters = ++(stepLetters, '+-'); let __SPL126 = assertIsOfSize(plusMinus, 2); let plusAngle = at(__SPL126, 1); let minusAngle = at(__SPL126, 2); do(contents(select(last(substitutionSystem(asMap((++(rule, [->('+', '+'), ->('-', '-')]))),initialCondition, count)), { :each | includes(keepLetters,each) })), { :each | if(includes(stepLetters,each), { add(answer,+(last(answer), angleVector(angle))) }, { angle := +(angle, if((=(each, '+')), { plusAngle }, { minusAngle })) }) }); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _collect_2(
      ...
      sl.annotateFunction(function (_n) {
        ...
        let _k = _size_1(_b);
        let __SPL127 = _assertIsOfSize_2(
          _deinterleave_2(
        ...
        let _y = _at_2(__SPL127, 1);
        let _x = _at_2(__SPL127, 2);
        /* Statements */
        return _collect_2(
          [_x, _y],
          sl.annotateFunction(function (_each) {
  ...
  "{ :self | collect(upOrDownTo(0, -(self, 1)), { :n | let b = integerDigits(n,2); let k = size(b); let __SPL127 = assertIsOfSize(deinterleave(padLeft(b,[+(k, (%(k, 2)))], 0),2), 2); let y = at(__SPL127, 1); let x = at(__SPL127, 2); collect([x, y], { :each | fromDigits(each,2) }) }) }",
);
