sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(
    function (_stepLetters, _plusMinus, _rule, _initialCondition, _count) {
      ...
      let __SPL1 = _assertIsOfSize_2(_plusMinus, 2);
      let _plusAngle = _at_2(__SPL1, 1);
      let _minusAngle = _at_2(__SPL1, 2);
      /* Statements */
      _do_2(
        _contents_1(
          _select_2(
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _collect_2(
      ...
      sl.annotateFunction(function (_n) {
        ...
        let _y = _at_2(__SPL2, 1);
        let _x = _at_2(__SPL2, 2);
        /* Statements */
        return _collect_2(
          [_x, _y],
          sl.annotateFunction(function (_each) {
  ...
  "{ :self | collect(upOrDownTo(0, -(self, 1)), { :n | let b = integerDigits(n,2); let k = size(b); let __SPL2 = assertIsOfSize(deinterleave(padLeft(b,[+(k, (%(k, 2)))], 0),2), 2); let y = at(__SPL2, 1); let x = at(__SPL2, 2); collect([x, y], { :each | fromDigits(each,2) }) }) }",
);
