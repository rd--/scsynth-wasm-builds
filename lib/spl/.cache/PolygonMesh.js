...
sl.addMethodToExistingTrait(
  ...
  "{ :self | let answer = []; do(faceIndices(self), { :each | toDo(1, size(each), { :i | add(answer,sortBy([at(each,i), atWrap(each,+(i, 1))],<=)) }) }); nub(answer) }",
);
...
sl.addMethodToExistingTrait(
  ...
  "{ :self | counts(collect(faceIndices(self),size:/1)) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | let v = vertexCoordinates(self); let w = sortBy(nub(v),<=); PolygonMesh(w, sortBy(nub(reject(collect(faceIndices(self), { :each | deleteAdjacentDuplicates(lexicographicallyLeastRotation(collect(each, { :i | indexOf(w,at(v, i)) }))) }), { :each | <=(size(each), 2) })),<=)) }",
);
