...
sl.addMethodToExistingTrait(
  ...
  "union",
  ["self", "aSet"],
  sl.annotateFunction(function (_self, _aSet) {
    /* ArityCheck */
    if (arguments.length !== 2) {
      const errorMessage = "Arity: expected 2, _self, _aSet";
      throw new Error(errorMessage);
    } /* Temporaries */
    let _answer = _copy_1(_self);
    /* Statements */
    _includeAll_2(_answer, _aSet);
    return _answer;
  }, ["self", "aSet"]),
  "{ :self :aSet | let answer = copy(self); includeAll(answer,aSet); answer }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _aCollection, _aBlock_2) {
    ...
    let _set = _asSet_2(_self, _aBlock_2);
    /* Statements */
    _includeAll_2(_set, _aCollection);
    return _asList_1(_set);
  }, ["self", "aCollection", "aBlock:/2"]),
  "{ :self :aCollection :aBlock:/2 | let set = asSet(self,aBlock:/2); includeAll(set,aCollection); asList(set) }",
);
