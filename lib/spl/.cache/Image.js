...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _maxDepth) {
    ...
    let __SPL265 = _assertIsOfSize_2(_shape_1(_contents_1(_self)), 3);
    let _rowCount = _at_2(__SPL265, 1);
    let _columnCount = _at_2(__SPL265, 2);
    let _pixelCount = _at_2(__SPL265, 3);
    let _header = [
      "P3",
  ...
  "{ :self :maxDepth | let __SPL265 = assertIsOfSize(shape(contents(self)), 3); let rowCount = at(__SPL265, 1); let columnCount = at(__SPL265, 2); let pixelCount = at(__SPL265, 3); let header = ['P3', format('% %',[columnCount, rowCount]), printString(maxDepth)]; let rows = collect(contents(self), { :each | unwords(collect(each, { :pixel | if(isValidRgb(pixel), { unwords(collect(rounded((*(first(pixel,3), maxDepth))),printString:/1)) }, { format('% % %',[maxDepth, maxDepth, maxDepth]) }) })) }); unlines((++(++(header, rows), ['']))) }",
);
