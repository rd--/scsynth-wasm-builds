...
sl.addMethodToExistingType(
  ...
  "{ :self :maxDepth | let __SplVar1 = assertIsOfSize(shape(contents(self)), 3); let rowCount = at(__SplVar1, 1); let columnCount = at(__SplVar1, 2); let pixelCount = at(__SplVar1, 3); let header = ['P3', format('% %',[columnCount, rowCount]), printString(maxDepth)]; let rows = collect(contents(self), { :each | unwords(collect(each, { :pixel | if(isValidRgb(pixel), { unwords(collect(round((*(first(pixel,3), maxDepth))),printString:/1)) }, { format('% % %',[maxDepth, maxDepth, maxDepth]) }) })) }); unlines((++(++(header, rows), ['']))) }",
);
