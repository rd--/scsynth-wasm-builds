...
sl.addMethodToExistingType(
  ...
  function (_self, _maxDepth) {
    ...
    let __SPL153 = _assertIsOfSize_2(_shape_1(_contents_1(_self)), 3);
    let _rowCount = _at_2(__SPL153, 1);
    let _columnCount = _at_2(__SPL153, 2);
    let _pixelCount = _at_2(__SPL153, 3);
    let _header = [
      "P3",
  ...
  "{ :self :maxDepth | let __SPL153 = assertIsOfSize(shape(contents(self)), 3); let rowCount = at(__SPL153, 1); let columnCount = at(__SPL153, 2); let pixelCount = at(__SPL153, 3); let header = ['P3', format('% %',[columnCount, rowCount]), printString(maxDepth)]; let rows = collect(contents(self), { :each | join(collect(each, { :pixel | if(isValidRgb(pixel), { join(collect(rounded((*(first(pixel,3), maxDepth))),printString:/1),' ') }, { format('% % %',[maxDepth, maxDepth, maxDepth]) }) }),' ') }); unlines((++(++(header, rows), ['']))) }",
);
