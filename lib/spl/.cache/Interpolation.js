...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :x :y | if((=(size(x), size(y))), { let k = size(x); let slope = []; let count = 0; let m = nil; toDo(1, k, { :i | let x1 = at(x, i); let y1 = at(y, i); toDo((+(i, 1)), k, { :j | let x2 = at(x, j); let y2 = at(y, j); ifTrue((~=(x1, x2)), { add(slope,/((-(y2, y1)), (-(x2, x1)))); count := +(count, 1) }) }) }); m := median(slope); postLine(count); [median(collect(upOrDownTo(1, k), { :i | -(at(y, i), (*(m, at(x, i)))) })), m] }, { error(xList,'theilSenEstimator: invalid input') }) }",
);
