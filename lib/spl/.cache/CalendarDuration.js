...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _elseClause_0) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL559 = _assertIsOfSize_2(
          _uncheckedParseIso8601DurationAsList_1(_self),
          7,
        );
        let _years = _at_2(__SPL559, 1);
        let _months = _at_2(__SPL559, 2);
        let _weeks = _at_2(__SPL559, 3);
        let _days = _at_2(__SPL559, 4);
        let _hours = _at_2(__SPL559, 5);
        let _minutes = _at_2(__SPL559, 6);
        let _seconds = _at_2(__SPL559, 7);
        /* Statements */
  ...
  "{ :self :elseClause:/0 | if(isIso8601DurationString(self), { let __SPL559 = assertIsOfSize(uncheckedParseIso8601DurationAsList(self), 7); let years = at(__SPL559, 1); let months = at(__SPL559, 2); let weeks = at(__SPL559, 3); let days = at(__SPL559, 4); let hours = at(__SPL559, 5); let minutes = at(__SPL559, 6); let seconds = at(__SPL559, 7); days := +(days, (*(weeks, 7))); CalendarDuration([years, months, days, hours, minutes, seconds]) }, { elseClause() }) }",
);
