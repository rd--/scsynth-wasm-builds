...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | matchesRegExp(self,'^P([0-9]+Y)?([0-9]+M)?([0-9]+W)?([0-9]+D)?(T([0-9]+H)?([0-9]+M)?([0-9]+(.[0-9]+)S)?)?$') }",
);
...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self, _elseClause_0) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let _years = _at_2(__SPL1, 1);
        let _months = _at_2(__SPL1, 2);
        let _weeks = _at_2(__SPL1, 3);
        let _days = _at_2(__SPL1, 4);
        let _hours = _at_2(__SPL1, 5);
        let _minutes = _at_2(__SPL1, 6);
        let _seconds = _at_2(__SPL1, 7);
        /* Statements */
  ...
  "{ :self :elseClause:/0 | if(isIso8601DurationString(self), { let __SPL1 = assertIsOfSize(uncheckedParseIso8601DurationAsList(self), 7); let years = at(__SPL1, 1); let months = at(__SPL1, 2); let weeks = at(__SPL1, 3); let days = at(__SPL1, 4); let hours = at(__SPL1, 5); let minutes = at(__SPL1, 6); let seconds = at(__SPL1, 7); days := +(days, (*(weeks, 7))); CalendarDuration([years, months, days, hours, minutes, seconds]) }, { elseClause() }) }",
);
