...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self :elseClause:/0 | if(isIso8601DurationString(self), { let __SplVar1 = assertIsOfSize(uncheckedParseIso8601DurationAsList(self), 7); let years = at(__SplVar1, 1); let months = at(__SplVar1, 2); let weeks = at(__SplVar1, 3); let days = at(__SplVar1, 4); let hours = at(__SplVar1, 5); let minutes = at(__SplVar1, 6); let seconds = at(__SplVar1, 7); days := +(days, (*(weeks, 7))); CalendarDuration([years, months, days, hours, minutes, seconds]) }, { elseClause() }) }",
);
