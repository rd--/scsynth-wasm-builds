...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _appendChildren_2(_projectionsA, [
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL547 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL547, 1);
        let _y = _at_2(__SPL547, 2);
        let _z = _at_2(__SPL547, 3);
        /* Statements */
        return [_x, _y];
      }, ["each"])),
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SPL547 = assertIsOfSize(each, 3); let x = at(__SPL547, 1); let y = at(__SPL547, 2); let z = at(__SPL547, 3); [x, y] }), scaledDrawing({ :each | let __SPL548 = assertIsOfSize(each, 3); let x = at(__SPL548, 1); let y = at(__SPL548, 2); let z = at(__SPL548, 3); [y, z] }), scaledDrawing({ :each | let __SPL549 = assertIsOfSize(each, 3); let x = at(__SPL549, 1); let y = at(__SPL549, 2); let z = at(__SPL549, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
