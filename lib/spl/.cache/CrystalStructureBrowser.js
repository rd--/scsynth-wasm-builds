...
sl.extendTypeOrTraitWithMethod(
  ...
  function (_self) {
    ...
    _appendChildren_2(_projectionsA, [
      _scaledDrawing_1(function (_each) {
        ...
        let __SPL243 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL243, 1);
        let _y = _at_2(__SPL243, 2);
        let _z = _at_2(__SPL243, 3);
        /* Statements */
        return [_x, _y];
      }),
      _scaledDrawing_1(function (_each) {
        ...
        let __SPL244 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL244, 1);
        let _y = _at_2(__SPL244, 2);
        let _z = _at_2(__SPL244, 3);
        /* Statements */
        return [_y, _z];
      }),
      _scaledDrawing_1(function (_each) {
        ...
        let __SPL245 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL245, 1);
        let _y = _at_2(__SPL245, 2);
        let _z = _at_2(__SPL245, 3);
        /* Statements */
        return [_x, _z];
      }),
    ]);
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SPL243 = assertIsOfSize(each, 3); let x = at(__SPL243, 1); let y = at(__SPL243, 2); let z = at(__SPL243, 3); [x, y] }), scaledDrawing({ :each | let __SPL244 = assertIsOfSize(each, 3); let x = at(__SPL244, 1); let y = at(__SPL244, 2); let z = at(__SPL244, 3); [y, z] }), scaledDrawing({ :each | let __SPL245 = assertIsOfSize(each, 3); let x = at(__SPL245, 1); let y = at(__SPL245, 2); let z = at(__SPL245, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
