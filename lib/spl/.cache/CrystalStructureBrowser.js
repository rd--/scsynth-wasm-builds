...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SplVar1 = assertIsOfSize(each, 3); let x = at(__SplVar1, 1); let y = at(__SplVar1, 2); let z = at(__SplVar1, 3); [x, y] }), scaledDrawing({ :each | let __SplVar2 = assertIsOfSize(each, 3); let x = at(__SplVar2, 1); let y = at(__SplVar2, 2); let z = at(__SplVar2, 3); [y, z] }), scaledDrawing({ :each | let __SplVar3 = assertIsOfSize(each, 3); let x = at(__SplVar3, 1); let y = at(__SplVar3, 2); let z = at(__SplVar3, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
