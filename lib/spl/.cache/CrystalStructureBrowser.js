...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _appendChildren_2(_projectionsA, [
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL320 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL320, 1);
        let _y = _at_2(__SPL320, 2);
        let _z = _at_2(__SPL320, 3);
        /* Statements */
        return [_x, _y];
      }, ["each"])),
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL321 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL321, 1);
        let _y = _at_2(__SPL321, 2);
        let _z = _at_2(__SPL321, 3);
        /* Statements */
        return [_y, _z];
      }, ["each"])),
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL322 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL322, 1);
        let _y = _at_2(__SPL322, 2);
        let _z = _at_2(__SPL322, 3);
        /* Statements */
        return [_x, _z];
      }, ["each"])),
    ]);
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SPL320 = assertIsOfSize(each, 3); let x = at(__SPL320, 1); let y = at(__SPL320, 2); let z = at(__SPL320, 3); [x, y] }), scaledDrawing({ :each | let __SPL321 = assertIsOfSize(each, 3); let x = at(__SPL321, 1); let y = at(__SPL321, 2); let z = at(__SPL321, 3); [y, z] }), scaledDrawing({ :each | let __SPL322 = assertIsOfSize(each, 3); let x = at(__SPL322, 1); let y = at(__SPL322, 2); let z = at(__SPL322, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
