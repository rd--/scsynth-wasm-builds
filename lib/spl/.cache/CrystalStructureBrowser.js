...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _appendChildren_2(_projectionsA, [
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL118 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL118, 1);
        let _y = _at_2(__SPL118, 2);
        let _z = _at_2(__SPL118, 3);
        /* Statements */
        return [_x, _y];
      }, ["each"])),
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL119 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL119, 1);
        let _y = _at_2(__SPL119, 2);
        let _z = _at_2(__SPL119, 3);
        /* Statements */
        return [_y, _z];
      }, ["each"])),
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL120 = _assertIsOfSize_2(_each, 3);
        let _x = _at_2(__SPL120, 1);
        let _y = _at_2(__SPL120, 2);
        let _z = _at_2(__SPL120, 3);
        /* Statements */
        return [_x, _z];
      }, ["each"])),
    ]);
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SPL118 = assertIsOfSize(each, 3); let x = at(__SPL118, 1); let y = at(__SPL118, 2); let z = at(__SPL118, 3); [x, y] }), scaledDrawing({ :each | let __SPL119 = assertIsOfSize(each, 3); let x = at(__SPL119, 1); let y = at(__SPL119, 2); let z = at(__SPL119, 3); [y, z] }), scaledDrawing({ :each | let __SPL120 = assertIsOfSize(each, 3); let x = at(__SPL120, 1); let y = at(__SPL120, 2); let z = at(__SPL120, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
