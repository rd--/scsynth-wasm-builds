...
sl.extendTypeOrTraitWithMethod(
  ...
  sl.annotateFunction(function (_self) {
    ...
    _appendChildren_2(_projectionsA, [
      _scaledDrawing_1(sl.annotateFunction(function (_each) {
        ...
        let __SPL1 = _assertIsOfSize_2(_each, 3);
  ...
  "{ :self | let container = createElement('div'); let description = createElement('p'); let projectionsA = createElement('p'); let projectionsB = createElement('p'); let scaledDrawing = { :projection | asSvgElement(self,12, asUnaryBlock(projection)) }; textContent(description, description(self)); appendChildren(projectionsA,[scaledDrawing({ :each | let __SPL1 = assertIsOfSize(each, 3); let x = at(__SPL1, 1); let y = at(__SPL1, 2); let z = at(__SPL1, 3); [x, y] }), scaledDrawing({ :each | let __SPL2 = assertIsOfSize(each, 3); let x = at(__SPL2, 1); let y = at(__SPL2, 2); let z = at(__SPL2, 3); [y, z] }), scaledDrawing({ :each | let __SPL3 = assertIsOfSize(each, 3); let x = at(__SPL3, 1); let y = at(__SPL3, 2); let z = at(__SPL3, 3); [x, z] })]); appendChildren(projectionsB,[scaledDrawing(namedAxonometricProjection('CabinetOblique',pi(Fraction(1L, 6L)))), scaledDrawing(namedAxonometricProjection('Isometric'))]); appendChildren(container,[description, projectionsA, projectionsB]); container }",
);
