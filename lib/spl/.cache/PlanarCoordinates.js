...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _PlanarCoordinates_1(_reversed_1(_coordinates_1(_self)));
  }, ["self"]),
  "{ :self | PlanarCoordinates(reversed(coordinates(self))) }",
);
  ...
  sl.annotateFunction(function (_self) {
    ...
    return _if_3(
      ...
      sl.annotateFunction(function () {
        ...
        let __SPL175 = _assertIsOfSize_2(_self, 2);
        let _x = _at_2(__SPL175, 1);
        let _y = _at_2(__SPL175, 2);
        /* Statements */
        return _initializeSlots_2(_newPlanarCoordinates_0(), [_x, _y]);
      }, []),
    );
  }, ["self"]),
  "{ :self | if(isMatrix(self), { collect(self,PlanarCoordinates:/1) }, { let __SPL175 = assertIsOfSize(self, 2); let x = at(__SPL175, 1); let y = at(__SPL175, 2); initializeSlots(newPlanarCoordinates(),[x, y]) }) }",
);
