...
sl.extendTypeOrTraitWithMethod(
  ...
  "{ :self | let __SPL2 = assertIsOfSize(asFloat(self), 3); let hue = at(__SPL2, 1); let saturation = at(__SPL2, 2); let brightness = at(__SPL2, 3); let s = max(min(saturation,1),0); let v = max(min(brightness,1),0); let h = %((*(hue, 360)), 360); let i = //(h, 60); let f = /((%(h, 60)), 60); let p = *((-(1, s)), v); let q = *((-(1, (*(s, f)))), v); let t = *((-(1, (*(s, (-(1, f)))))), v); caseOf(i, [->(0, { [v, t, p] }), ->(1, { [q, v, p] }), ->(2, { [p, v, t] }), ->(3, { [p, q, v] }), ->(4, { [t, p, v] }), ->(5, { [v, p, q] })], { error('hsvToRgb','implementation error') }) }",
);
