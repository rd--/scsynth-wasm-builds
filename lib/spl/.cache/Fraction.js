...
sl.addMethodToExistingType(
  ...
  sl.annotateFunction(function (_self, _aFraction) {
    ...
    let __SPL31 = _assertIsOfSize_2(_asList_1(_self), 2);
    let _a = _at_2(__SPL31, 1);
    let _b = _at_2(__SPL31, 2);
    let __SPL32 = _assertIsOfSize_2(_asList_1(_aFraction), 2);
    let _c = _at_2(__SPL32, 1);
    let _d = _at_2(__SPL32, 2);
    /* Statements */
  ...
  "{ :self :aFraction | let __SPL31 = assertIsOfSize(asList(self), 2); let a = at(__SPL31, 1); let b = at(__SPL31, 2); let __SPL32 = assertIsOfSize(asList(aFraction), 2); let c = at(__SPL32, 1); let d = at(__SPL32, 2); =(-((*(b, c)), (*(a, d))), 1) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :maxDenominator | if((<(maxDenominator, 1)), { error(self,'limitDenominator: illegal maxDenominator') }, { if((<=(denominator(self), maxDenominator)), { self }, { let p0 = 0; let q0 = 1; let p1 = 1; let q1 = 0; let n = numerator(self); let d = denominator(self); let continue = true; let k = nil; let bound1 = nil; let bound2 = nil; whileTrue({ continue }, { let a = //(n, d); let q2 = +(q0, (*(a, q1))); if((>(q2, maxDenominator)), { continue := false }, { ({ let __SPL33 = [p1, q1, +(p0, (*(a, p1))), q2, d, -(n, (*(a, d)))]; p0 := at(__SPL33, 1); q0 := at(__SPL33, 2); p1 := at(__SPL33, 3); q1 := at(__SPL33, 4); n := at(__SPL33, 5); d := at(__SPL33, 6) } . ()) }) }); k := //((-(maxDenominator, q0)), q1); bound1 := ReducedFraction(+(p0, (*(k, p1))), +(q0, (*(k, q1)))); bound2 := ReducedFraction(p1, q1); if((<=(abs((-(bound2, self))), abs((-(bound1, self))))), { bound2 }, { bound1 }) }) }) }",
);
