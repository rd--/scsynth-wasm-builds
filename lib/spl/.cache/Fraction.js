...
sl.addMethodToExistingType(
  ...
  function (_self, _aFraction) {
    ...
    let __SPL1 = _assertIsOfSize_2(_asList_1(_self), 2);
    let _a = _at_2(__SPL1, 1);
    let _b = _at_2(__SPL1, 2);
    let __SPL2 = _assertIsOfSize_2(_asList_1(_aFraction), 2);
  ...
  "{ :self :aFraction | let __SPL1 = assertIsOfSize(asList(self), 2); let a = at(__SPL1, 1); let b = at(__SPL1, 2); let __SPL2 = assertIsOfSize(asList(aFraction), 2); let c = at(__SPL2, 1); let d = at(__SPL2, 2); =(-((*(b, c)), (*(a, d))), 1) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self :maxDenominator | if((<(maxDenominator, 1)), { error(self,'limitDenominator: illegal maxDenominator') }, { if((<=(denominator(self), maxDenominator)), { self }, { let p0 = 0; let q0 = 1; let p1 = 1; let q1 = 0; let n = numerator(self); let d = denominator(self); let continue = true; let k = nil; let bound1 = nil; let bound2 = nil; whileTrue({ continue }, { let a = //(n, d); let q2 = +(q0, (*(a, q1))); if((>(q2, maxDenominator)), { continue := false }, { ({ let __SPL3 = [p1, q1, +(p0, (*(a, p1))), q2, d, -(n, (*(a, d)))]; p0 := at(__SPL3, 1); q0 := at(__SPL3, 2); p1 := at(__SPL3, 3); q1 := at(__SPL3, 4); n := at(__SPL3, 5); d := at(__SPL3, 6) } . ()) }) }); k := //((-(maxDenominator, q0)), q1); bound1 := ReducedFraction(+(p0, (*(k, p1))), +(q0, (*(k, q1)))); bound2 := ReducedFraction(p1, q1); if((<=(abs((-(bound2, self))), abs((-(bound1, self))))), { bound2 }, { bound1 }) }) }) }",
);
...
sl.addMethodToExistingType(
  ...
  "{ :self | join([basicPrintString(numerator(self),10), basicPrintString(denominator(self),10)],'/') }",
);
