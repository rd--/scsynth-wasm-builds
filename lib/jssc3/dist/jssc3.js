...
function audioBufferInterleavedChannelData(anAudioBuffer) {
    if (anAudioBuffer.numberOfChannels === 1) {
        ...
        anAudioBuffer.length * anAudioBuffer.numberOfChannels;
...
function encodeFloat32(aNumber, litteEndian) {
    return encodeUsing(4, (b)=>b.setFloat32(0, aNumber, litteEndian));
}
function encodeFloat32Array(inputArray, litteEndian) {
...
function SelectX(which, array) {
    return XFade2(Multiplexer(RoundTo(which, 2), array), Multiplexer(Add(Trunc(which, 2), 1), array), Fold2(Sub(Mul(which, 2), 1), 1), 1);
}
function UnitCps(a) {
...
function b_allocMemcpyFloat32Array(bufferNumber, numberOfFrames, numberOfChannels, sampleRate, data) {
    return b_allocMemcpy(bufferNumber, numberOfFrames, numberOfChannels, sampleRate, encodeFloat32Array(data, true), 0);
}
function b_allocMemcpyArray(bufferNumber, sampleRate, data) {
    const numberOfFrames = data.length;
    return b_allocMemcpyFloat32Array(bufferNumber, numberOfFrames, 1, sampleRate, new Float32Array(data));
}
...
function wasmConnect(scSynth, wasm) {
    ...
    setTimeout(function() {
        wasm.oscDriver[langPort] = {
            receive: function(_unusedAddress, data) {
...
function userProgramMenuInit(selectId, setProgram) {
...
export { userProgramMenuInit as userProgramMenuInit };
